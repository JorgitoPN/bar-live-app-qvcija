...
replaceModule(1259, (mapModule) => {
  ...
  const selectionEffect = '(0,l.useEffect)(()=>{if(!e0||!a0?.id||!s.current||!ve)return;const e1=Number(a0?.latitud??a0?.coordenadas?.lat),n1=Number(a0?.longitud??a0?.coordenadas?.lng);if(!Number.isFinite(e1)||!Number.isFinite(n1))return;const t1=JSON.stringify(a0).replace(/</g,"\\u003c").replace(/\\u2028/g,"\\u2028").replace(/\\u2029/g,"\\u2029"),i1=JSON.stringify({lng:n1,lat:e1});s.current.injectJavaScript("(function(){try{if(typeof window.flyToLocation===\\\"function\\\")window.flyToLocation("+e1+","+n1+",16);setTimeout(function(){if(typeof window.showPopupFromNative===\\\"function\\\")window.showPopupFromNative("+t1+","+i1+");function compactMiniPopup(){try{var content=document.querySelector(\\\".maplibregl-popup-content\\\");if(!content)return;content.style.width=\\\"190px\\\";content.style.minWidth=\\\"190px\\\";content.style.maxWidth=\\\"190px\\\";content.style.borderRadius=\\\"9px\\\";content.style.boxShadow=\\\"0 3px 10px rgba(15,23,42,.20)\\\";var root=content.firstElementChild;if(root){root.style.display=\\\"flex\\\";root.style.alignItems=\\\"stretch\\\";root.style.width=\\\"100%\\\";root.style.minHeight=\\\"88px\\\";root.style.maxHeight=\\\"92px\\\";root.style.background=\\\"#fff\\\";}var img=content.querySelector(\\\".popup-img\\\");if(img){img.style.width=\\\"52px\\\";img.style.minWidth=\\\"52px\\\";img.style.maxWidth=\\\"52px\\\";img.style.height=\\\"92px\\\";img.style.minHeight=\\\"92px\\\";img.style.maxHeight=\\\"92px\\\";img.style.objectFit=\\\"cover\\\";img.style.flex=\\\"0 0 52px\\\"}var info=content.querySelector(\\\".popup-info\\\");if(info){info.style.padding=\\\"6px 7px\\\";info.style.flex=\\\"1\\\";info.style.minWidth=\\\"0\\\";info.style.display=\\\"flex\\\";info.style.flexDirection=\\\"column\\\";info.style.justifyContent=\\\"center\\\";}var title=content.querySelector(\\\".popup-title\\\");if(title){title.style.fontSize=\\\"12.5px\\\";title.style.marginBottom=\\\"3px\\\";title.style.lineHeight=\\\"1.15\\\";title.style.whiteSpace=\\\"nowrap\\\";title.style.overflow=\\\"hidden\\\";title.style.textOverflow=\\\"ellipsis\\\"}var rating=content.querySelector(\\\".popup-rating\\\");if(rating){rating.style.display=\\\"none\\\"}var category=content.querySelector(\\\".popup-category\\\");if(category){category.style.display=\\\"none\\\"}var button=content.querySelector(\\\".popup-btn\\\");if(button){button.style.marginTop=\\\"2px\\\";button.style.padding=\\\"5px 6px\\\";button.style.fontSize=\\\"10px\\\";button.style.borderRadius=\\\"6px\\\";button.style.lineHeight=\\\"1.05\\\";button.style.minHeight=\\\"24px\\\";var buttonText=button.querySelector(\\\"span\\\");if(buttonText)buttonText.textContent=\\\"Ver local\\\"}content.querySelectorAll(\\\".popup-banner\\\").forEach(function(el){el.style.display=\\\"none\\\"});content.querySelectorAll(\\\"[style*=\\\\\\\"font-size:12px\\\\\\\"]\\\").forEach(function(el){el.style.fontSize=\\\"9.5px\\\";el.style.marginBottom=\\\"3px\\\";el.style.lineHeight=\\\"1.15\\\";el.style.whiteSpace=\\\"nowrap\\\";el.style.overflow=\\\"hidden\\\";el.style.textOverflow=\\\"ellipsis\\\";var inner=el.querySelector(\\\"span:last-child\\\");if(inner){inner.style.whiteSpace=\\\"nowrap\\\";inner.style.overflow=\\\"hidden\\\";inner.style.textOverflow=\\\"ellipsis\\\"}});if(window.currentPopup&&typeof window.currentPopup.setOffset===\\\"function\\\")window.currentPopup.setOffset(8);requestAnimationFrame(function(){try{var popup=window.currentPopup&&window.currentPopup.getElement?window.currentPopup.getElement():document.querySelector(\\\".maplibregl-popup\\\");var mapEl=document.getElementById(\\\"map\\\");var mapObj=window.currentPopup&&window.currentPopup._map;if(!popup||!mapEl||!mapObj||typeof mapObj.panBy!==\\\"function\\\")return;var pr=popup.getBoundingClientRect(),mr=mapEl.getBoundingClientRect(),dx=pr.left+pr.width/2-(mr.left+mr.width/2),dy=pr.top+pr.height/2-(mr.top+mr.height/2);if(Math.abs(dx)>2||Math.abs(dy)>2)mapObj.panBy([dx,dy],{duration:180})}catch(_){}})}catch(_){}}compactMiniPopup();setTimeout(compactMiniPopup,140)},180)}catch(error){console.error(\\\"[ExploreMiniMap] selection error\\\",error)}})();true;")},[e0,a0?.id,a0?.latitud,a0?.longitud,a0?.coordenadas?.lat,a0?.coordenadas?.lng,t0,le]);const Re=';
...
for (const needle of ['ExploreMiniMap','embeddedRoot','selectedVenue:a0','compactMiniPopup','190px','root.style.display']) {
