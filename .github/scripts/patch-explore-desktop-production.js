...
replaceModule(1259, (mapModule) => {
  ...
  const selectionEffect = '(0,l.useEffect)(()=>{if(!e0||!a0?.id||!s.current||!ve)return;const e1=Number(a0?.latitud??a0?.coordenadas?.lat),n1=Number(a0?.longitud??a0?.coordenadas?.lng);if(!Number.isFinite(e1)||!Number.isFinite(n1))return;s.current.injectJavaScript("(function(){try{if(window.currentPopup){try{window.currentPopup.remove()}catch(_){}window.currentPopup=null}if(typeof window.flyToLocation===\\\"function\\\")window.flyToLocation("+e1+","+n1+",16)}catch(error){console.error(\\\"[ExploreMiniMap] selection error\\\",error)}})();true;")},[e0,a0?.id,a0?.latitud,a0?.longitud,a0?.coordenadas?.lat,a0?.coordenadas?.lng,t0,le]);const Re=';
...
for (const needle of ['ExploreMiniMap','embeddedRoot','selectedVenue:a0','currentPopup.remove']) {
