...
replaceModule(1259, (mapModule) => {
  ...
  const selectionEffect = '(0,l.useEffect)(()=>{if(!e0||!a0?.id||!s.current||!ve)return;const e1=Number(a0?.latitud??a0?.coordenadas?.lat),n1=Number(a0?.longitud??a0?.coordenadas?.lng);if(!Number.isFinite(e1)||!Number.isFinite(n1))return;const t1=JSON.stringify(a0).replace(/</g,"\\\\u003c").replace(/\\u2028/g,"\\\\u2028").replace(/\\u2029/g,"\\\\u2029"),i1=JSON.stringify({lng:n1,lat:e1});s.current.injectJavaScript("(function(){try{if(typeof window.flyToLocation===\\\"function\\\")window.flyToLocation("+e1+","+n1+",16);setTimeout(function(){if(typeof window.showPopupFromNative===\\\"function\\\")window.showPopupFromNative("+t1+","+i1+")},180)}catch(error){console.error(\\\"[ExploreMiniMap] selection error\\\",error)}})();true;")},[e0,a0?.id,a0?.latitud,a0?.longitud,a0?.coordenadas?.lat,a0?.coordenadas?.lng,t0,le]);const Re=';
...
for (const needle of ['ExploreMiniMap','embeddedRoot','selectedVenue:a0']) {
