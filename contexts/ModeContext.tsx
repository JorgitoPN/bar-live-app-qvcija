...
/**
 * ✅ MODE CONTEXT v289.0 - ANDROID PERFORMANCE OPTIMIZATION
 * 
...
export function ModeProvider({ children }: { children: ReactNode }) {
  ...
  const loadOwnedLocals = useCallback(async () => {
    ...
    try {
      ...
      console.log('[ModeContext v289.0] 🔄 Loading owned locals for user:', user.id);
      ...
      if (error) {
        console.error('[ModeContext v289.0] ❌ Error loading owned locals:', error);
        return;
      }
      ...
      console.log('[ModeContext v289.0] ✅ Loaded', locals.length, 'active owned locals');
  ...
  useEffect(() => {
    const initializeMode = async () => {
      try {
        console.log('[ModeContext v289.0] 🔄 Initializing from AsyncStorage...');
        ...
        if (savedMode && (savedMode === 'cliente' || savedMode === 'propietario' || savedMode === 'admin')) {
          if (user) {
            ...
            if (isValidMode) {
              console.log('[ModeContext v289.0] ✅ Restored mode from storage:', savedMode);
              setCurrentModeState(savedMode as UserMode);
            } else {
              console.log('[ModeContext v289.0] ⚠️ Invalid mode for user, resetting to cliente');
          ...
          console.log('[ModeContext v289.0] ℹ️ No saved mode, defaulting to cliente');
          setCurrentModeState('cliente');
          await AsyncStorage.setItem(MODE_STORAGE_KEY, 'cliente');
        }

        if (savedProfileId && savedProfileType) {
          console.log('[ModeContext v289.0] ✅ Restoring active profile:', savedProfileId, savedProfileType);
          
          if (savedProfileType === 'local' && user) {
            console.log('[ModeContext v289.0] 🔄 Verifying local ownership for:', savedProfileId);
            ...
            if (ownershipError || !ownershipData) {
              console.error('[ModeContext v289.0] ❌ User does not own saved local, resetting to client profile');
              ...
              if (localError || !localData) {
                console.error('[ModeContext v289.0] ❌ Error loading local data, resetting to client profile:', localError);
                setActiveProfileIdState(user.id);
                setActiveProfileTypeState('cliente');
                setActiveLocalData(null);
                setCurrentModeState('cliente');
                ...
                console.log('[ModeContext v289.0] ✅ Loaded local data:', localData.nombre);
                ...
                if (savedMode !== 'propietario') {
                  console.log('[ModeContext v289.0] ⚠️ Mode was not propietario, correcting...');
            ...
            console.log('[ModeContext v289.0] ⚠️ Invalid saved profile, resetting to default');
            if (user) {
              setActiveProfileIdState(user.id);
              setActiveProfileTypeState('cliente');
              setActiveLocalData(null);
          ...
          console.log('[ModeContext v289.0] ℹ️ No saved profile, defaulting to client profile');
          setActiveProfileIdState(user.id);
          setActiveProfileTypeState('cliente');
          setActiveLocalData(null);
          await AsyncStorage.setItem(ACTIVE_PROFILE_STORAGE_KEY, user.id);
        ...
        console.log('[ModeContext v289.0] ✅ Initialization complete');
      } catch (error) {
        console.error('[ModeContext v289.0] ❌ Error initializing mode:', error);
        if (user) {
          setCurrentModeState('cliente');
          setActiveProfileIdState(user.id);
          setActiveProfileTypeState('cliente');
  ...
  // ✅ CRITICAL FIX v289.0: LAZY LOADING - Only load when switching to propietario mode
  ...
  const setCurrentMode = useCallback(async (mode: UserMode) => {
    try {
      console.log('[ModeContext v289.0] 🔄 Setting mode to:', mode);
      ...
      if (user) {
        ...
        if (!isValidMode) {
          console.warn('[ModeContext v289.0] ⚠️ Invalid mode for user:', mode, userRole);
          return;
        }
      }
      ...
      console.log('[ModeContext v289.0] ✅ Mode saved to storage:', mode);

      // ✅ FIX v289.0: Load owned locals ONLY when switching to propietario
      if (mode === 'propietario' && user && ownedLocals.length === 0) {
        console.log('[ModeContext v289.0] 🔍 Loading owned locals for propietario mode...');
        ...
        setTimeout(async () => {
          ...
          if (!error && data && data.length > 0) {
            ...
            if (firstLocal) {
              console.log('[ModeContext v289.0] ✅ Auto-selecting first local:', firstLocal.nombre);
              await switchToLocalProfile(firstLocal.id);
            }
          } else {
            console.log('[ModeContext v289.0] ℹ️ User has no active locals, staying in cliente mode');
      ...
      if (mode === 'cliente' && user) {
        console.log('[ModeContext v289.0] 🔄 Mode changed to cliente, switching to client profile');
        await switchToClientProfile();
      }
    } catch (error) {
  ...
  const switchToClientProfile = useCallback(async () => {
    if (!user) {
      console.warn('[ModeContext v289.0] ⚠️ Cannot switch to client profile: no user');
      return;
    }

    try {
      console.log('[ModeContext v289.0] 🔄 Switching to client profile:', user.id);
      ...
      console.log('[ModeContext v289.0] ✅ State updated - Mode: cliente, Profile:', user.nombre);
      ...
      try {
        ...
        console.error('[ModeContext v289.0] ⚠️ Error persisting to storage (state is still updated):', storageError);
      }
    } catch (error) {
      console.error('[ModeContext v289.0] ❌ Error switching to client profile:', error);
    }
  }, [user]);

  const switchToLocalProfile = useCallback(async (localId: string) => {
    if (!user) {
      console.warn('[ModeContext v289.0] ⚠️ Cannot switch to local profile: no user');
      return;
    }

    try {
      console.log('[ModeContext v289.0] 🔄 Switching to local profile:', localId);
      ...
      if (ownershipError || !ownershipData) {
        console.error('[ModeContext v289.0] ❌ User does not own this local:', ownershipError);
        return;
      }
      ...
      if (localError || !localData) {
        console.error('[ModeContext v289.0] ❌ Error loading local data:', localError);
        return;
      }
      ...
      console.log('[ModeContext v289.0] ✅ State updated - Mode: propietario, Profile:', localData.nombre);
      ...
      console.error('[ModeContext v289.0] ❌ Error switching to local profile:', error);
    }
  }, [user]);
