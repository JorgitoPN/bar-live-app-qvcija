...
const getLogServerUrl = (): string | null => {
  ...
  try {
    ...
    console.log('[errorLogger] Error getting log server URL:', e);
  }
...
const flushLogs = async () => {
  ...
  for (const log of logsToSend) {
    try {
      ...
      console.log('[errorLogger] Error flushing logs:', e);
    }
  }
};
...
const sendErrorToParent = (level: string, message: string, data: any) => {
  ...
  try {
    ...
    console.log('[errorLogger] Error sending to parent:', error);
  }
};
