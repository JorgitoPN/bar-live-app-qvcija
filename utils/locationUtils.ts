...
import { calcularDistancia, getOptimizedUserLocation, getCachedLocation, prewarmLocationCache } from '@/utils/locationUtils';
...
/**
 * 🗺️ MAPA SCREEN v460.0 - ULTRA-FAST LOCATION DETECTION
 * 
 * CRITICAL FIXES v460.0:
 * - 🔥 INSTANT LOCATION: Sub-second detection with aggressive caching
 * - 🔥 PARALLEL REQUESTS: Last known + current position simultaneously
 * - 🔥 PRE-WARMING: Location cache starts loading on component mount
 * - 🔥 REDUCED TIMEOUT: 3 seconds (from 5s)
 * - 🔥 LOW ACCURACY: Speed over precision on all platforms
 * - ✅ Blue dot appears almost instantly on map load
 * 
...
export default function MapaScreen() {
  ...
  const handleCategoriaChange = useCallback((categoriaId: string) => {
    console.log('🗺️ [MAPA v460.0] Cambiando categoría a:', categoriaId);
  ...
  const handleEstadoChange = useCallback((estado: 'todos' | 'no_cerrados') => {
    console.log('🗺️ [MAPA v460.0] Cambiando estado a:', estado);
    setFiltroEstado(estado);
  }, []);
  
  const handleToggleFiltros = useCallback(() => {
    console.log('🗺️ [MAPA v460.0] 🔍 Opening advanced filters');
    setMostrarFiltros(prev => !prev);
  }, []);
  
  const handleCloseFiltros = useCallback(() => {
    console.log('🗺️ [MAPA v460.0] ✅ Closing advanced filters');
    setMostrarFiltros(false);
  }, []);

  const handleClearAdvancedFilters = useCallback(() => {
    console.log('🗺️ [MAPA v460.0] 🧹 Clearing advanced filters');
    limpiarFiltros();
  }, [limpiarFiltros]);
...
<div id="map"></div>
<script>
console.log('🗺️ [MAPA v460.0] Inicializando MapLibre GL JS - Ultra-Fast Location Detection');
...
function loadCategoryIcons() {
  ...
  console.log('🗺️ [MAPA v460.0] Iconos de categorías cargados');
}
...
map.on('load', function() {
  console.log('🗺️ [MAPA v460.0] Mapa cargado, añadiendo source GeoJSON');
  
  setTimeout(function() {
    map.resize();
    console.log('🗺️ [MAPA v460.0] ✅ map.resize() ejecutado');
  }, 100);
  ...
  console.log('🗺️ [MAPA v460.0] ✅ Adding cluster-count layer with MAXIMUM visibility');
  map.addLayer({
    id: 'cluster-count',
    type: 'symbol',
    source: 'locales-source',
  ...
  console.log('🗺️ [MAPA v460.0] ✅ Cluster count layer added');
  ...
  console.log('🗺️ [MAPA v460.0] ✅ All layers added');
  
  window.loadLocales();
  
  if (window.pendingAdvancedFilters) {
    console.log('🗺️ [MAPA v460.0] Applying pending advanced filters');
...
window.loadLocales = async function() {
  try {
    console.log('🗺️ [MAPA v460.0] Cargando locales desde Supabase...');
    ...
    console.log('🗺️ [MAPA v460.0] Locales cargados:', locales.length);
    
    window.allLocales = locales;
    window.applyFilters();
  } catch (error) {
    console.error('🗺️ [MAPA v460.0] Error cargando locales:', error);
  }
};

window.applyAdvancedFilters = function(filterCriteria) {
  console.log('🗺️ [MAPA v460.0] 🔍 Setting advanced filters:', filterCriteria);
...
window.applyFilters = function() {
  ...
  console.log('🗺️ [MAPA v460.0] Aplicando filtros:', window.filtros);
  console.log('🗺️ [MAPA v460.0] Advanced filters:', window.advancedFilters);
  ...
  console.log('🗺️ [MAPA v460.0] Locales filtrados:', filteredLocales.length);
  ...
  if (source) {
    ...
    console.log('🗺️ [MAPA v460.0] ✅ GeoJSON actualizado con', geojson.features.length, 'marcadores');
  }
};
...
window.updateUserLocation = function(lat, lng) {
  console.log('🗺️ [MAPA v460.0] 📍 Actualizando ubicación del usuario:', lat, lng);
  ...
  if (!window.userMarker) {
    ...
    console.log('🗺️ [MAPA v460.0] ✅ Marcador de usuario creado');
  } else {
    window.userMarker.setLngLat([lng, lat]);
    console.log('🗺️ [MAPA v460.0] ✅ Marcador de usuario actualizado');
  }
};
...
function showPopupForFeature(feature, coordinates) {
  ...
  console.log('🗺️ [MAPA v460.0] ✅ Mostrando popup para local:', properties.name);
...
map.on('click', function(e) {
  console.log('🗺️ [MAPA v460.0] 🎯 Click detectado');
  
  var clusterFeatures = map.queryRenderedFeatures(e.point, { layers: ['clusters'] });
  if (clusterFeatures.length > 0) {
    console.log('🗺️ [MAPA v460.0] 🔵 CLUSTER detectado - ejecutando zoom-in');
  ...
  if (detectado) {
    console.log('🗺️ [MAPA v460.0] 🎉 Local encontrado:', detectado.nombre);
...
console.log('🗺️ [MAPA v460.0] ✅ Map initialization complete with ULTRA-FAST location detection');
</script>
</body>
</html>`;
  }, [userLocation, popupWidth, popupImageHeight, markerScale, userMarkerSize]);

  // ✅ v460.0: Pre-warm location cache on mount
  useEffect(() => {
    console.log('🗺️ [MAPA v460.0] 🔥 Pre-warming location cache on mount');
    prewarmLocationCache();
  }, []);

  // ✅ v460.0: ULTRA-FAST location detection
  useEffect(() => {
    (async () => {
      try {
        console.log('🗺️ [MAPA v460.0] 🎯 Starting ULTRA-FAST location detection');
        
        // Check cache first (instant)
        const cached = getCachedLocation();
        if (cached) {
          console.log('🗺️ [MAPA v460.0] ⚡ Using cached location (INSTANT)');
          setUserLocation({
            lat: cached.latitude,
        ...
        // Get optimized location (fast)
        const location = await getOptimizedUserLocation();
        
        if (location) {
          console.log('🗺️ [MAPA v460.0] ✅ Location obtained:', location.coords.latitude, location.coords.longitude);
          setUserLocation({
            lat: location.coords.latitude,
            lng: location.coords.longitude,
          });
        } else {
          console.log('🗺️ [MAPA v460.0] ⚠️ Using default location (Madrid)');
          setUserLocation({ lat: 40.4168, lng: -3.7038 });
        }
      } catch (error) {
        console.error('🗺️ [MAPA v460.0] ❌ Error obteniendo ubicación:', error);
        setUserLocation({ lat: 40.4168, lng: -3.7038 });
      }
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    console.log('🗺️ [MAPA v460.0] 🔍 Applying advanced filters to map:', globalFiltros);
    ...
    requestAnimationFrame(() => {
      webViewRef.current?.injectJavaScript(`
        (function() {
          console.log('🗺️ [MAPA v460.0] Applying advanced filters in WebView:', ${JSON.stringify(filterCriteria)});
  ...
  useEffect(() => {
    ...
    console.log('🗺️ [MAPA v460.0] 📍 Inyectando ubicación del usuario');
      ...
      webViewRef.current?.injectJavaScript(`
        (function() {
          try {
            ...
            console.error('🗺️ [MAPA v460.0] ❌ Error actualizando ubicación:', error);
          }
        })();
        true;
      `);
  ...
  const centerOnUser = useCallback(() => {
    if (userLocation && webViewRef.current && isMapReady) {
      console.log('🗺️ [MAPA v460.0] Centrando en ubicación del usuario');
      webViewRef.current.injectJavaScript(`
  ...
  const handleWebViewMessage = useCallback((event: any) => {
    try {
      ...
      if (data.type === 'navigate' && data.id) {
        console.log('🗺️ [MAPA v460.0] Navegando a local:', data.id);
        router.push(`/detalle/local?id=${data.id}`);
      } else if (data.type === 'map_ready') {
        console.log('🗺️ [MAPA v460.0] Mapa listo');
        setIsMapReady(true);
      }
    } catch (error) {
      console.error('🗺️ [MAPA v460.0] Error procesando mensaje:', error);
    }
  }, [router]);
