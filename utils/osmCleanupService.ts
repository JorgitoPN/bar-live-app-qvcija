...
/**
 * 🗑️ SERVICIO DE LIMPIEZA DE LOCALES OSM ENRIQUECIDOS v2.1
 * 
 * LINT FIXES v2.1:
 * - ✅ FIXED: Changed Array<{...}> to {...}[] for TypeScript array types (lines 29, 90)
...
export async function limpiarOSMEnriquecidos(dryRun: boolean = false): Promise<OSMCleanupResult> {
  console.log('[OSM Cleanup Service v2.1] 🔄 Starting catalog migration...');
  console.log('[OSM Cleanup Service v2.1] Dry run:', dryRun);
  console.log('[OSM Cleanup Service v2.1] Action: Change source_type from osm to google');
  ...
  try {
    ...
    if (fetchError) {
      console.error('[OSM Cleanup Service v2.1] ❌ Error fetching locales:', fetchError);
      throw fetchError;
    }

    console.log('[OSM Cleanup Service v2.1] Found locales to migrate:', localesAMover?.length || 0);

    if (!localesAMover || localesAMover.length === 0) {
      console.log('[OSM Cleanup Service v2.1] ✅ No locales to migrate');
      return {
        success: true,
        localesMovidos: 0,
        espacioLiberadoMB: 0,
    ...
    if (dryRun) {
      ...
      console.log('[OSM Cleanup Service v2.1] 📊 Simulation mode - no changes made');
      return {
        success: true,
    ...
    for (let i = 0; i < localesAMover.length; i += batchSize) {
      ...
      console.log(`[OSM Cleanup Service v2.1] Migrating batch ${i / batchSize + 1}:`, ids.length, 'locales');
      ...
      if (updateError) {
        console.error('[OSM Cleanup Service v2.1] ❌ Error migrating batch:', updateError);
        throw updateError;
      }
      ...
      console.log(`[OSM Cleanup Service v2.1] ✅ Migrated ${totalMigrated}/${localesAMover.length} locales`);
    }

    // Calculate space freed (rough estimate: 5KB per local)
    const espacioLiberadoMB = Math.round((totalMigrated * 5) / 1024);

    console.log('[OSM Cleanup Service v2.1] ✅ Migration completed');
    console.log('[OSM Cleanup Service v2.1] Total migrated:', totalMigrated);
    console.log('[OSM Cleanup Service v2.1] Catalog separation complete');
    ...
    console.error('[OSM Cleanup Service v2.1] ❌ Error:', error);
    return {
      success: false,
      localesMovidos: 0,
      espacioLiberadoMB: 0,
...
export async function limpiarLocalOSMSiEnriquecido(localId: string): Promise<boolean> {
  try {
    console.log(`[OSM Cleanup Service v2.1] Checking local ${localId}...`);
    ...
    if (fetchError || !local) {
      console.error('[OSM Cleanup Service v2.1] Local not found:', fetchError);
      return false;
    }

    // Only migrate if it's OSM, enriched, and active
    if (local.source_type !== 'osm' || !local.enriquecido || !local.activo) {
      console.log('[OSM Cleanup Service v2.1] Local does not meet migration criteria:', {
    ...
    console.log(`[OSM Cleanup Service v2.1] Migrating enriched OSM local to Google catalog: ${local.nombre}`);
    ...
    if (updateError) {
      console.error('[OSM Cleanup Service v2.1] ❌ Error migrating local:', updateError);
      return false;
    }

    console.log('[OSM Cleanup Service v2.1] ✅ Local migrated to Google catalog successfully');
    return true;
  } catch (error) {
    console.error('[OSM Cleanup Service v2.1] ❌ Error:', error);
    return false;
...
export async function obtenerEstadisticasOSMEnriquecidos(): Promise<{
  ...
  try {
    ...
    console.error('[OSM Cleanup Service v2.1] Error getting statistics:', error);
    return {
      totalOSM: 0,
      osmEnriquecidos: 0,
      osmActivos: 0,
...
export async function estaLimpiezaAutomaticaHabilitada(): Promise<boolean> {
  try {
    ...
    console.error('[OSM Cleanup Service v2.1] Error checking auto-cleanup status:', error);
...
export async function configurarLimpiezaAutomatica(enabled: boolean): Promise<boolean> {
  try {
    console.log('[OSM Cleanup Service v2.1] Setting auto-migration:', enabled);
    ...
    if (error) {
      console.error('[OSM Cleanup Service v2.1] Error setting auto-migration:', error);
      return false;
    }

    console.log('[OSM Cleanup Service v2.1] ✅ Auto-migration configured successfully');
    return true;
  } catch (error) {
    console.error('[OSM Cleanup Service v2.1] Error:', error);
    return false;
