...
/**
 * ✅ SALA VIRTUAL v101.0 - ANDROID SCALING STANDARDIZATION
 * 
 ...
 * - ✅ Login required and owner button hidden maintained
 */
...
export default function SalaVirtualScreen() {
    ...
    const checkOwnership = async () => {
      ...
      try {
        ...
        if (localData && localData.propietario_id === user.id) {
          setIsOwner(true);
          console.log('[SalaVirtual v101.0] ✅ User IS OWNER of this local');
        } else {
          setIsOwner(false);
          console.log('[SalaVirtual v101.0] ✅ User is NOT owner of this local');
        }
      } catch (error) {
        console.error('[SalaVirtual v101.0] Error checking ownership:', error);
        setIsOwner(false);
      }
    };
  ...
  const checkClosingTime = useCallback(() => {
    ...
    if (estadoLocal.estaAbierto && estadoLocal.tiempoRestante) {
      ...
      console.log(`[SalaVirtual v101.0] ⏰ Tiempo hasta cierre: ${totalMinutes} minutos`);
      ...
      console.log('[SalaVirtual v101.0] ❌ Local cerrado, expulsando usuarios');
      setLocalClosed(true);
  ...
  const loadLocalData = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual v101.0] No localId provided');
      setLoading(false);
    ...
    try {
      console.log('[SalaVirtual v101.0] Loading local:', localId);
      ...
      if (error) {
        console.error('[SalaVirtual v101.0] Error loading local:', error);
        setLoading(false);
      ...
      console.log('[SalaVirtual v101.0] Local loaded:', data);
      setLocal(data);
      
      const estadoLocal = getEstadoLocal(data);
      const isOpen = estadoLocal.estaAbierto === true;
      
      console.log('[SalaVirtual v101.0] Estado del local:', estadoLocal);
      ...
      console.error('[SalaVirtual v101.0] Error:', error);
      setLoading(false);
    }
  }, [localId, router]);
  ...
  const checkUserCheckin = useCallback(async () => {
    if (!user || !localId) {
      console.log('[SalaVirtual v101.0] No user or localId');
      return false;
    }
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('[SalaVirtual v101.0] Error checking checkin:', error);
        return false;
      }

      const checkedIn = !!data;
      setIsCheckedIn(checkedIn);
      console.log('[SalaVirtual v101.0] User checked in:', checkedIn);
  ...
  const handleCheckIn = useCallback(async () => {
    ...
    try {
      setCheckingIn(true);
      console.log('[SalaVirtual v101.0] 🔄 Starting check-in for user:', user.id);

      setIsCheckedIn(true);
      console.log('[SalaVirtual v101.0] ✅ Optimistic check-in: user is now in_room: true');
      ...
      if (closeError) {
        console.error('[SalaVirtual v101.0] ❌ Error closing previous check-ins:', closeError);
        setIsCheckedIn(false);
        throw new Error('No se pudo cerrar la sesión anterior');
      }

      console.log('[SalaVirtual v101.0] ✅ All previous check-ins closed');
      ...
      if (error) {
        console.error('[SalaVirtual v101.0] ❌ Error inserting checkin:', error);
        setIsCheckedIn(false);
        throw new Error('No se pudo entrar en la sala');
      }

      console.log('[SalaVirtual v101.0] ✅ Checked in successfully');
      ...
      if (presenceChannelRef.current) {
        try {
          ...
          console.log('[SalaVirtual v101.0] ✅ Broadcasted user joined');
        } catch (broadcastError) {
          console.error('[SalaVirtual v101.0] Error broadcasting user joined:', broadcastError);
        }
      }
      ...
      console.error('[SalaVirtual v101.0] ❌ Unexpected error during checkin:', error);
  ...
  const handleAutoCheckOut = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v101.0] 🔄 Auto checking out user on modal close:', user.id);
      ...
      if (error) {
        console.error('[SalaVirtual v101.0] ❌ Error auto checking out:', error);
        return;
      }
      ...
      if (presenceChannelRef.current) {
        try {
          ...
          console.error('[SalaVirtual v101.0] Error broadcasting user left:', broadcastError);
        }
      }

      console.log('[SalaVirtual v101.0] ✅ Auto checked out successfully');
    } catch (error) {
      console.error('[SalaVirtual v101.0] ❌ Error in auto checkout:', error);
    }
  }, [user, localId, isCheckedIn]);

  const handleModalClose = useCallback(async () => {
    console.log('[SalaVirtual v101.0] 🚪 Modal closing, auto checking out user...');
  ...
  const loadMessages = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v101.0] ✅ Volatile chat: Starting with empty message array (no database load)');
      setMessages([]);
      setLoading(false);
      console.log('[SalaVirtual v101.0] ✅ Chat initialized as volatile (ephemeral)');
    } catch (error) {
      console.error('[SalaVirtual v101.0] Error:', error);
      setLoading(false);
    }
  ...
  const updateActiveUsers = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual v101.0] Error loading active users:', error);
        return;
      }
      ...
      console.log('[SalaVirtual v101.0] ⚡ Active users updated:', users.length);
    } catch (error) {
      console.error('[SalaVirtual v101.0] Error:', error);
    }
  }, [localId]);
  ...
  const subscribeToUpdates = useCallback(() => {
    ...
    console.log('[SalaVirtual v101.0] 📡 Subscribing to real-time updates (volatile chat mode + user list)');
    ...
    const chatChannel = supabase
      ...
      .on('broadcast', { event: 'message_created' }, (payload) => {
        console.log('[SalaVirtual v101.0] ⚡ INSTANT new volatile message received:', payload);
        
        if (payload.payload.usuario_id === user.id) {
          console.log('[SalaVirtual v101.0] ⏭️ Own message, skipping');
          return;
        }
      ...
      .on('broadcast', { event: 'message_deleted' }, (payload) => {
        console.log('[SalaVirtual v101.0] 🗑️ Message deleted:', payload.payload.id);
        setMessages((prev) => prev.filter(m => m.id !== payload.payload.id));
      })
      .on('broadcast', { event: 'user_typing' }, (payload) => {
        console.log('[SalaVirtual v101.0] ⌨️ User typing:', payload.payload.usuario_id);
      ...
      .on('broadcast', { event: 'room_closing_soon' }, (payload) => {
        console.log('[SalaVirtual v101.0] ⏰ Room closing soon');
        Alert.alert(
      ...
      .on('broadcast', { event: 'room_closed' }, () => {
        console.log('[SalaVirtual v101.0] 🔒 Room closed');
        Alert.alert(
          'Sala Virtual Cerrada',
          'El local ha cerrado. Has sido expulsado de la sala virtual.',
          [{ text: 'OK', onPress: () => router.back() }]
      ...
      .subscribe((status) => {
        console.log('[SalaVirtual v101.0] 📡 Chat channel status:', status);
      });
    ...
    const presenceChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual v101.0] ⚡ INSTANT user joined (postgres):', payload.new);
          updateActiveUsers();
        }
      )
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual v101.0] ⚡ User check-in updated (postgres):', payload.new);
          updateActiveUsers();
        }
      )
      .on('broadcast', { event: 'user_joined' }, () => {
        console.log('[SalaVirtual v101.0] ⚡ User joined (broadcast)');
        updateActiveUsers();
      })
      .on('broadcast', { event: 'user_left' }, () => {
        console.log('[SalaVirtual v101.0] ⚡ User left (broadcast)');
        updateActiveUsers();
      })
      .subscribe((status) => {
        console.log('[SalaVirtual v101.0] 📡 Presence channel status:', status);
      });
    ...
    return () => {
      console.log('[SalaVirtual v101.0] 🔄 Unsubscribing from real-time updates');
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (!checkedIn && !localClosed && user) {
        console.log('[SalaVirtual v101.0] 🔄 Auto check-in starting...');
        const success = await handleCheckIn();
        if (!success) {
          console.log('[SalaVirtual v101.0] ❌ Auto check-in failed, aborting initialization');
          return;
        }
        console.log('[SalaVirtual v101.0] ✅ Auto check-in successful');
      }
  ...
  const sendMessage = useCallback(async () => {
    ...
    if (!isCheckedIn) {
      console.error('[SalaVirtual v101.0] ❌ User not checked in, cannot send message');
    ...
    try {
      setSending(true);
      console.log('[SalaVirtual v101.0] 📤 Sending volatile message (broadcast only)...');
      ...
      if (chatChannelRef.current) {
        ...
        console.log('[SalaVirtual v101.0] ✅ Volatile message broadcasted successfully');
      }
    } catch (error) {
      console.error('[SalaVirtual v101.0] ❌ Error:', error);
  ...
  const deleteMessage = useCallback(async (messageId: string) => {
    ...
    try {
      ...
      console.log('[SalaVirtual v101.0] Message deleted successfully (volatile)');
    } catch (error) {
      console.error('[SalaVirtual v101.0] Error:', error);
    }
  }, [user]);
  ...
  const sendEmoticon = useCallback(async (recipientId: string, emoticon: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual v101.0] Error sending emoticon:', error);
