...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * 🚨 SALA VIRTUAL v8.0 - KEYBOARD FIX COMPLETE (iOS & Android)
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * ✅ PROBLEMA RESUELTO - iOS Keyboard Coverage:
 * - Keyboard.addListener detecta apertura/cierre manualmente
 * - KeyboardAvoidingView behavior='padding' con keyboardVerticalOffset=90
 * - ScrollView forzado al final cuando teclado se abre
 * - El campo de texto queda completamente visible al escribir
 * 
 * ✅ PROBLEMA RESUELTO - Android Keyboard & Touch Buttons:
 * - Keyboard.addListener detecta apertura/cierre manualmente
 * - KeyboardAvoidingView behavior='height' empuja contenido hacia arriba
 * - ScrollView forzado al final cuando teclado se abre
 * - El campo de texto es completamente visible y accesible
 * - No queda oculto por los botones de navegación del teléfono
 * 
 * ARQUITECTURA:
 * - Listeners de teclado para detectar apertura/cierre
 * - iOS: behavior='padding' + keyboardVerticalOffset=90
 * - Android: behavior='height' (empuja contenido hacia arriba)
 * - ScrollView auto-scroll al final cuando teclado se abre
 * - Input siempre visible y accesible en ambas plataformas
 * 
 * ═══════════════════════════════════════════════════════════════════════════
 */

console.log("✅ SALA VIRTUAL v8.0 - KEYBOARD FIX iOS/Android COMPLETE");
...
export default function SalaVirtualEnhancedScreen() {
  ...
  console.log('[SalaVirtual v8.0] 🎯 INITIAL TAB from params:', initialTab);
  console.log('[SalaVirtual v8.0] 🔄 Redirect param:', redirectParam);
  ...
  useEffect(() => {
    console.log('[SalaVirtual v8.0] 🔐 Checking authentication status...');
    console.log('[SalaVirtual v8.0] 👤 User:', user ? 'authenticated' : 'NOT authenticated');
    
    if (!user) {
      console.log('[SalaVirtual v8.0] 🚫 User not authenticated - showing login modal');
      setShowLoginModal(true);
      setLoading(false);
    } else {
      console.log('[SalaVirtual v8.0] ✅ User authenticated - proceeding to load room');
    }
  }, [user]);

  useEffect(() => {
    console.log('[SalaVirtual v8.0] 🎬 Component mounted');
    isMounted.current = true;
    
    return () => {
      console.log('[SalaVirtual v8.0] 🧹 Component unmounting');
      isMounted.current = false;
    };
  }, []);

  // ✅ FIXED v8.0: Keyboard listeners for manual detection
  useEffect(() => {
    console.log('[SalaVirtual v8.0] 🎹 Setting up keyboard listeners');
  ...
  const loadReadMessagesFromStorage = useCallback(async (localId: string, userId: string): Promise<Set<string>> => {
    try {
      ...
      if (stored) {
        ...
        console.log('[SalaVirtual v8.0] 🔵 Loaded read partners from storage:', readPartners);
        return new Set(readPartners);
      }
      
      console.log('[SalaVirtual v8.0] 🔵 No stored read partners found');
      return new Set();
    } catch (error) {
      console.error('[SalaVirtual v8.0] ❌ Error loading from storage:', error);
      return new Set();
    }
  }, [getReadMessagesKey]);
  ...
  const saveReadMessagesToStorage = useCallback(async (localId: string, userId: string, partnerId: string) => {
    try {
      ...
      if (!readPartners.includes(partnerId)) {
        ...
        console.log('[SalaVirtual v8.0] 🔵 Saved read status for partner:', partnerId);
      }
    } catch (error) {
      console.error('[SalaVirtual v8.0] ❌ Error saving to storage:', error);
    }
  }, [getReadMessagesKey]);

  const fetchUserProfile = useCallback(async (userId: string): Promise<UserProfile | null> => {
    try {
      console.log('[SalaVirtual v8.0] 🔍 Fetching user profile from database for userId:', userId);
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error fetching user profile:', error);
        return null;
      }

      console.log('[SalaVirtual v8.0] ✅ User profile fetched successfully');
      console.log('[SalaVirtual v8.0] 👤 Name:', data.nombre);
      console.log('[SalaVirtual v8.0] 🖼️ Avatar:', data.avatar || 'NO AVATAR');
      console.log('[SalaVirtual v8.0] 📝 Username:', data.username || 'NO USERNAME');
      ...
      console.error('[SalaVirtual v8.0] ❌ Error in fetchUserProfile:', error);
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      console.log('[SalaVirtual v8.0] 🌓 Checking day/night mode:', newMode);
      if (isMounted.current) {
        setMode(newMode);
      }
    }, 60000);
    ...
    (async () => {
      ...
      if (status === 'granted') {
        ...
        if (isMounted.current) {
          ...
          console.log('[SalaVirtual v8.0] ✅ User location obtained');
        }
      }
    })();
  }, []);
  ...
  const checkClosingTime = useCallback(() => {
    ...
    if (estadoLocal.estaAbierto && estadoLocal.tiempoRestante) {
      ...
      console.log('[SalaVirtual v8.0] ⏰ Time until closing:', totalMinutes, 'minutes');
  ...
  const loadLocalData = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual v8.0] ❌ No localId provided');
      if (isMounted.current) {
        setLoading(false);
      }
      return;
    ...
    try {
      console.log('[SalaVirtual v8.0] 🏠 Loading local data for:', localId);
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error loading local:', error);
        if (isMounted.current) {
          setLoading(false);
        }
        return;
      ...
      console.log('[SalaVirtual v8.0] ✅ Local loaded:', data.nombre);
      ...
      if (!isOpen) {
        console.log('[SalaVirtual v8.0] 🔒 Local is closed');
        setLocalClosed(true);
        setLoading(false);
      } else {
        console.log('[SalaVirtual v8.0] ✅ Local is open');
        setLocalClosed(false);
      }
    } catch (error) {
      console.error('[SalaVirtual v8.0] ❌ Error:', error);
      if (isMounted.current) {
        setLoading(false);
      }
    }
  ...
  const checkUserCheckin = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v8.0] 🔍 Checking if user is checked in...');
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('[SalaVirtual v8.0] ❌ Error checking checkin:', error);
        return false;
      }

      const checkedIn = !!data;
      console.log('[SalaVirtual v8.0] ✅ User checked in status:', checkedIn);
  ...
  const handleCheckIn = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v8.0] 🚪 User checking in...');
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error inserting checkin:', error);
        if (isMounted.current) {
      ...
      console.log('[SalaVirtual v8.0] ✅ User checked in successfully');
      ...
      console.error('[SalaVirtual v8.0] ❌ Error during checkin:', error);
      if (isMounted.current) {
        setCheckingIn(false);
      }
      return false;
  ...
  const handleCheckOut = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v8.0] 🚪 BOTÓN CERRAR PULSADO - Manual checkout');
      console.log('[SalaVirtual v8.0] 🚪 User checking out...');
      ...
      console.log('[SalaVirtual v8.0] ✅ User checked out successfully');

      router.back();
      
      console.log('[SalaVirtual v8.0] ✅ Navigation executed with back()');
    } catch (error) {
      console.error('[SalaVirtual v8.0] ❌ Error checking out:', error);
    }
  }, [user, localId, router]);

  const loadMessages = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual v8.0] ❌ No localId for loadMessages');
      return;
    }
    
    try {
      console.log('[SalaVirtual v8.0] 🔥 EPHEMERAL CHAT - NOT LOADING OLD MESSAGES');
      console.log('[SalaVirtual v8.0] ✅ Starting with empty message list (session-only messages)');
      ...
      console.log('[SalaVirtual v8.0] ✅ Room initialized with empty chat (ephemeral mode)');
    } catch (error) {
      console.error('[SalaVirtual v8.0] ❌ Error initializing messages:', error);
      if (isMounted.current) {
        setLoading(false);
      }
    }
  ...
  const triggerReceivedAnimation = useCallback((messageText: string, tipo: string) => {
    console.log('[SalaVirtual v8.0] 🎬 Triggering received animation for tipo:', tipo);
    
    if (tipo !== 'privado') {
      console.log('[SalaVirtual v8.0] ⏭️ Not a private message, skipping animation');
      return;
    }
    ...
    console.log('[SalaVirtual v8.0] 🎬 Showing received animation with emoji:', emoji);
  ...
  const syncMessages = useCallback(async () => {
    ...
    try {
      ...
      if (publicError) {
        console.error('[SalaVirtual v8.0] ❌ Error syncing public messages:', publicError);
      ...
      if (privateError) {
        console.error('[SalaVirtual v8.0] ❌ Error syncing private messages:', privateError);
      ...
      console.error('[SalaVirtual v8.0] ❌ Error syncing messages:', error);
    }
  ...
  const updateActiveUsers = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error loading active users:', error);
        return;
      }
  ...
  const markPrivateMessagesAsRead = useCallback(async (partnerId: string) => {
    ...
    try {
      console.log('[SalaVirtual v8.0] 🔵 Marking private messages as read from:', partnerId);
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error marking messages as read:', error);
      }
  ...
  const loadPrivateChats = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error loading private chats:', error);
        return;
      }
  ...
  useEffect(() => {
    ...
    return () => {
      console.log('[SalaVirtual v8.0] 🧹 Cleanup: Cleaning up subscriptions only');
    };
  ...
  const sendPublicMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error saving message to DB:', error);
  ...
  const sendPredefinedMessage = useCallback(async (recipientId: string, messageText: string) => {
    ...
    try {
      ...
      if (insertError) {
        console.error('[SalaVirtual v8.0] ❌ Error saving private message to DB:', insertError);
      } else {
        setTimeout(() => {
          pendingMessageIds.current.delete(pendingId);
        }, 1000);
      ...
      console.error('[SalaVirtual v8.0] ❌ Error sending predefined message:', error);
  ...
  const sendPrivateMessage = useCallback(async (recipientId: string, content: string) => {
    ...
    try {
      ...
      console.error('[SalaVirtual v8.0] ❌ Error sending private message:', error);
    }
  ...
  const openPrivateChat = useCallback(async (chat: PrivateChat) => {
    ...
    try {
      ...
      console.log('[SalaVirtual v8.0] 💬 Opening private chat with:', displayName);
      console.log('[SalaVirtual v8.0] 📥 LOADING PRIVATE MESSAGES - Messages persist during session');
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error loading private messages:', error);
        if (isMounted.current) {
        ...
        console.log('[SalaVirtual v8.0] ✅ Loaded', messages.length, 'private messages');
      }
    } catch (error) {
      console.error('[SalaVirtual v8.0] ❌ Error opening private chat:', error);
    }
  ...
  const confirmDeleteMessage = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual v8.0] ❌ Error deleting message:', error);
  ...
  const handleUserPress = async (selectedUser: ActiveUser) => {
    ...
    console.log('[SalaVirtual v8.0] 👤 User pressed:', displayName);
    
    if (selectedUser.id === user?.id) {
      console.log('[SalaVirtual v8.0] ⚠️ Cannot interact with self');
      return;
    }
  ...
  const handleViewProfile = useCallback(async () => {
    ...
    console.log('[SalaVirtual v8.0] 🚀 Starting profile navigation');
    console.log('[SalaVirtual v8.0] 👤 Target user ID:', selectedUser.id);
    ...
    try {
      ...
      console.log('[SalaVirtual v8.0] ✅ Navigation executed successfully');
    } catch (error) {
      console.error('[SalaVirtual v8.0] ❌ Error navigating:', error);
    }
  }, [selectedUser, localId, router, closeBottomSheet, activeTab]);

  const handlePrivateChatUserPress = useCallback(async () => {
    if (!selectedPrivateChat) {
      console.log('[SalaVirtual v8.0] ⚠️ No selected private chat');
      return;
    }
    
    console.log('[SalaVirtual v8.0] 🚀 Navigating to profile from private chat header');
    console.log('[SalaVirtual v8.0] 👤 Target user ID:', selectedPrivateChat.userId);
    ...
    try {
      ...
      console.log('[SalaVirtual v8.0] ✅ Navigation executed successfully from private chat');
    } catch (error) {
      console.error('[SalaVirtual v8.0] ❌ Error navigating:', error);
    }
  ...
  const contentPaddingBottom = useMemo(() => {
    ...
    const dynamicPadding = Math.max(insets.bottom, 8);
  ...
  const handleLoginSuccess = useCallback(async () => {
    console.log('[SalaVirtual v8.0] ✅ Login successful - reloading room');
    if (isMounted.current) {
...
const styles = StyleSheet.create({
  ...
  coverGlowCircle: {
    ...
    height: 200,
    borderRadius: 100,
    opacityI'll fix the keyboard and fullscreen layout issues for the Virtual Room, Gallery, and Posts screens.

<write file="app/detalle/sala-virtual-enhanced.tsx">
import React, { useState, useEffect, useCallback, useRef, useMemo } from 'react';
import {
  View,
  Text,
 ...
 * 
 * ✅ PROBLEMA RESUELTO - iOS Keyboard Coverage:
 * - Keyboard.addListener para detectar apertura/cierre manualmente
 * - KeyboardAvoidingView behavior='padding' con keyboardVerticalOffset=90
...
export default function SalaVirtualEnhancedScreen() {
  ...
  // ✅ FIXED v8.0: Manual keyboard listeners for precise control
  useEffect(() => {
    console.log('[SalaVirtual v8.0] 🎹 Setting up keyboard listeners');
  ...
  const contentPaddingBottom = useMemo(() => {
    ...
    // ✅ FIXED v8.0: Dynamic padding based on keyboard state
    const dynamicPadding = isKeyboardVisible 
      ? (Platform.OS === 'ios' ? 0 : 8) // iOS: 0 when keyboard open, Android: 8
      : Math.max(insets.bottom, Platform.OS === 'ios' ? 8 : 16); // iOS: 8, Android: 16 when closed
  ...
  // ✅ FIXED v8.0: iOS keyboard positioning - input field sits directly on keyboard
  // ✅ FIXED v8.0: Android keyboard positioning - input field visible above keyboard with proper spacing
...
const styles = StyleSheet.create({
  ...
  privateChatLastMessage: {
    font<write file="app/detalle/sala-virtual-enhanced.tsx">
import React, { useState, useEffect, useCallback, useRef, useMemo } from 'react';
import {
  View,
  Text,
...
export default function SalaVirtualEnhancedScreen() {
  ...
  useEffect(() => {
    const keyboardDidShowListener = Keyboard.addListener(
      'keyboardDidShow',
      (e) => {
        console.log('[SalaVirtual] ⬆️ Keyboard opened, height:', e.endCoordinates.height);
        if (isMounted.current) {
          setKeyboardHeight(e.endCoordinates.height);
          setIsKeyboardVisible(true);
    ...
    const keyboardDidHideListener = Keyboard.addListener(
      'keyboardDidHide',
      () => {
        console.log('[SalaVirtual] ⬇️ Keyboard closed');
        if (isMounted.current) {
          setKeyboardHeight(0);
          setIsKeyboardVisible(false);
        }
  ...
  const loadReadMessagesFromStorage = useCallback(async (localId: string, userId: string): Promise<Set<string>> => {
    try {
      ...
      console.error('[SalaVirtual] Error loading from storage:', error);
      return new Set();
    }
  }, [getReadMessagesKey]);
  ...
  const saveReadMessagesToStorage = useCallback(async (localId: string, userId: string, partnerId: string) => {
    try {
      ...
      console.error('[SalaVirtual] Error saving to storage:', error);
    }
  }, [getReadMessagesKey]);
  ...
  const fetchUserProfile = useCallback(async (userId: string): Promise<UserProfile | null> => {
    try {
      ...
      if (error) {
        console.error('[SalaVirtual] Error fetching user profile:', error);
        return null;
      }
      ...
      console.error('[SalaVirtual] Error in fetchUserProfile:', error);
      return null;
    }
  }, []);
  ...
  const loadMessages = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual] No localId for loadMessages');
      return;
    }
    ...
    try {
      ...
      console.error('[SalaVirtual] Error initializing messages:', error);
      if (isMounted.current) {
        setLoading(false);
      }
    }
  ...
  const syncMessages = useCallback(async () => {
    ...
    try {
      ...
      if (publicError) {
        console.error('[SalaVirtual] Error syncing public messages:', publicError);
      } else if (publicData && publicData.length > 0) {
        const newMessages: Message[] = publicData
          .filter(msg => {
            if (!msg.usuario) {
      ...
      if (privateError) {
        console.error('[SalaVirtual] Error syncing private messages:', privateError);
      } else if (privateData && privateData.length > 0) {
        if (privateData.length > 0) {
          lastPrivateMessageTimestampRef.current = privateData[privateData.length - 1].created_at;
        }
      ...
      console.error('[SalaVirtual] Error syncing messages:', error);
    }
  }, [localId, user, selectedPrivateChat, triggerReceivedAnimation]);
  ...
  const markPrivateMessagesAsRead = useCallback(async (partnerId: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual] Error marking messages as read:', error);
      }
  ...
  const loadPrivateChats = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual] Error loading private chats:', error);
        return;
      }
  ...
  useEffect(() => {
    ...
    return () => {
      console.log('[SalaVirtual] Cleanup: Cleaning up subscriptions only');
    };
  }, [localId, loadLocalData, checkUserCheckin, handleCheckIn, loadMessages, subscribeToUpdates, subscribeToTypingEvents, updateActiveUsers, localClosed, user]);
  ...
  const sendPublicMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual] Error saving message to DB:', error);
  ...
  const sendPredefinedMessage = useCallback(async (recipientId: string, messageText: string) => {
    ...
    try {
      ...
      if (insertError) {
        console.error('[SalaVirtual] Error saving private message to DB:', insertError);
      } else {
        setTimeout(() => {
          pendingMessageIds.current.delete(pendingId);
        }, 1000);
      ...
      console.error('[SalaVirtual] Error sending predefined message:', error);
  ...
  const sendPrivateMessage = useCallback(async (recipientId: string, content: string) => {
    ...
    try {
      ...
      console.error('[SalaVirtual] Error sending private message:', error);
    }
  }, [user, localId, activeUsers, isTyping, handleTypingStop, fetchUserProfile]);
  ...
  const openPrivateChat = useCallback(async (chat: PrivateChat) => {
    ...
    try {
      ...
      if (error) {
        console.error('[SalaVirtual] Error loading private messages:', error);
        if (isMounted.current) {
          setPrivateChatMessages([]);
        }
      } else {
      ...
      console.error('[SalaVirtual] Error opening private chat:', error);
    }
  }, [user, localId, markPrivateMessagesAsRead]);
  ...
  const handleViewProfile = useCallback(async () => {
    ...
    try {
      ...
      console.error('[SalaVirtual] Error navigating:', error);
    }
  }, [selectedUser, localId, router, closeBottomSheet, activeTab]);
  ...
  const inputContainerBottomPadding =
    Platform.OS === 'ios'
      ...
      : isKeyboardVisible ? 8 : Math.max(insets.bottom + 8, 16);
