 ...
 * - ✅ DEDUPLICATE MESSAGES: Filter out duplicate message IDs before rendering
 * - ✅ UNIQUE KEYS: Ensure all messages have unique IDs in the array
 * - ✅ OPTIMISTIC UI FIX: Remove optimistic messages when real messages arrive
 * - ✅ SYNC FIX: Prevent duplicate messages from being added during sync
 * 
 * PROBLEMA IDENTIFICADO (CRITICAL):
 * - React Native error: "Encountered two children with the same key"
 * - Duplicate message IDs in the messages array
 * - Optimistic messages not being replaced by real messages
 * - Sync adding duplicate messages from database
 * 
 * SOLUCIÓN v10.3 (DEDUPLICATION):
 * 1. DEDUPLICATE BEFORE RENDER:
 *    - Filter messages array to ensure unique IDs
 *    - Use useMemo to deduplicate efficiently
 *    - Warn when duplicates are detected
 * 
 * 2. OPTIMISTIC UI CLEANUP:
 *    - Remove optimistic messages when real messages arrive
 *    - Check for existing IDs before adding new messages
 *    - Prevent double-adding messages from sync
 * 
 * 3. SYNC DEDUPLICATION:
 *    - Filter out messages that already exist in the array
 *    - Final deduplication pass after all updates
 *    - Log warnings when duplicates are detected
 * 
 * ═══════════════════════════════════════════════════════════════════════════
 */

console.log("✅ SALA VIRTUAL v10.3 - DUPLICATE KEY FIX IMPLEMENTED - Deduplication + optimistic UI cleanup");
...
function SalaVirtualEnhancedScreen() {
  ...
  // ✅ CRITICAL FIX v10.3: Deduplicate messages before rendering
  // This prevents the "Encountered two children with the same key" error
  const mensajesUnicos = useMemo(() => {
    console.log('[SalaVirtual v10.3] 🔍 Deduplicating messages array...');
    console.log('[SalaVirtual v10.3] 📊 Total messages before deduplication:', messages.length);
    
    const seenIds = new Set<string>();
    const deduplicated = messages.filter((mensaje, index) => {
      if (seenIds.has(mensaje.id)) {
        console.warn('[SalaVirtual v10.3] ⚠️ Duplicate message ID detected and removed:', mensaje.id);
        return false;
      }
      seenIds.add(mensaje.id);
      return true;
    });
    
    console.log('[SalaVirtual v10.3] ✅ Messages after deduplication:', deduplicated.length);
    console.log('[SalaVirtual v10.3] 🗑️ Duplicates removed:', messages.length - deduplicated.length);
    
    return deduplicated;
  }, [messages]);

  // ✅ CRITICAL FIX v10.3: Deduplicate private chat messages
  const privateChatMessagesUnicos = useMemo(() => {
    console.log('[SalaVirtual v10.3] 🔍 Deduplicating private chat messages...');
    console.log('[SalaVirtual v10.3] 📊 Total private messages before deduplication:', privateChatMessages.length);
    
    const seenIds = new Set<string>();
    const deduplicated = privateChatMessages.filter((mensaje, index) => {
      if (seenIds.has(mensaje.id)) {
        console.warn('[SalaVirtual v10.3] ⚠️ Duplicate private message ID detected and removed:', mensaje.id);
        return false;
      }
      seenIds.add(mensaje.id);
      return true;
    ...
    console.log('[SalaVirtual v10.3] 🗑️ Duplicates removed:', privateChatMessages.length - deduplicated.length);
  ...
  const fetchUserProfile = useCallback(async (userId: string): Promise<UserProfile | null> => {
    try {
      ...
      console.log('[SalaVirtual v10.3] 👤 Name:', data.nombre);
      console.log('[SalaVirtual v10.3] 🖼️ Avatar:', data.avatar || 'NO AVATAR');
      console.log('[SalaVirtual v10.3] 📝 Username:', data.username || 'NO USERNAME');
  ...
  const contentPaddingBottom = useMemo(() => {
    ...
    console.log('[SalaVirtual v10.3] 📏 Content padding bottom:', totalPadding, 'px (keyboard:', isKeyboardVisible ? 'open' : 'closed', ', height:', keyboardHeight, ')');
  ...
  const inputContainerBottom = useMemo(() => {
    ...
    console.log('[SalaVirtual v10.3] 📐 Input container bottom:', bottomValue, 
  ...
  const inputContainerPaddingBottom = useMemo(() => {
    ...
    console.log('[SalaVirtual v10.3] 📐 Input container paddingBottom:', paddingValue, 
  ...
  const handleLoginSuccess = useCallback(async () => {
    console.log('[SalaVirtual v10.3] ✅ Login successful - reloading room');
    if (isMounted.current) {
      ...
      >
        ...
        <View style={styles.tabsContainer}>
          ...
          >
            ...
            <Text
              style={[
                styles.tabTextI'll fix the duplicate key error by implementing the recommended solution - deduplicating the messages array before passing it to the FlatList.
...
function SalaVirtualEnhancedScreen() {
  ...
  const deduplicatedMessages = useMemo(() => {
    console.log('[SalaVirtual v10.3] 🔍 Deduplicating messages array...');
    console.log('[SalaVirtual v10.3] 📊 Original messages count:', messages.length);
    ...
    console.log('[SalaVirtual v10.3] ✅ Deduplicated messages count:', deduplicated.length);
    console.log('[SalaVirtual v10.3] 🗑️ Removed duplicates:', messages.length - deduplicated.length);
    
    return deduplicated;
  }, [messages]);

  // ✅ CRITICAL FIX v10.3: Deduplicate private chat messages
  const deduplicatedPrivateChatMessages = useMemo(() => {
    console.log('[SalaVirtual v10.3] 🔍 Deduplicating private chat messages...');
    console.log('[SalaVirtual v10.3] 📊 Original private messages count:', privateChatMessages.length);
    
    const seenIds = new Set<string>();
    const deduplicated = privateChatMessages.filter(msg => {
      if (seenIds.has(msg.id)) {
        console.warn('[SalaVirtual v10.3] ⚠️ Duplicate private message ID detected and removed:', msg.id);
        return false;
      }
    ...
    console.log('[SalaVirtual v10.3] ✅ Deduplicated private messages count:', deduplicated.length);
    console.log('[SalaVirtual v10.3] 🗑️ Removed duplicates:', privateChatMessages.length - deduplicated.length);
  ...
  const handleLoginSuccess = useCallback(async () => {
    ...
    await new Promise(resolve => setTimeout(resolve,Perfect! I can see the issue. The file is using `FlashList` and has duplicate message IDs in the `mensajes` array. I can see there's already some deduplication logic in place (v10.3), but the `keyExtractor` is still using a composite key. Let me check if there are any other issues and apply the proper fix.

Looking at the code, I can see:
1. Line 789 uses `keyExtractor={(item) => item.id}` for the main chat FlashList
2. There's already deduplication logic in the `syncMessages` function (v10.3)
3. The private chat uses `keyExtractor={(item, index) => `private-${item.id}-${index}`}` which is correct

The issue is that the main chat FlashList at line 789 is using just `item.id`, but there's a comment at line 786 showing it should use a composite key. Let me fix this:
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * 🚨 SALA VIRTUAL v10.4 - DUPLICATE KEY FIX
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * 🎉 v10.4 CHANGES (CRITICAL DUPLICATE KEY FIX):
 * - 🔑 FIXED: Changed keyExtractor to use composite key (id + index)
 * - ✅ PREVENTS: "Encountered two children with the same key" error
 * - 🛡️ MAINTAINS: All v10.3 deduplication logic as defense-in-depth
 * - 📊 ENSURES: React can properly track and render all messages
 * 
 * PROBLEMA IDENTIFICADO:
 * - FlatList/FlashList showing error: "Encountered two children with the same key"
 * - Using only item.id as key when duplicate IDs exist in array
 * - React cannot properly track components with duplicate keys
 * 
 * SOLUCIÓN v10.4:
 * - Changed keyExtractor from `(item) => item.id` to `(item, index) => ${item.id}-${index}`
 * - Composite key ensures uniqueness even if duplicate IDs slip through
 * - Maintains all existing deduplication logic as primary defense
 * - Composite key is fallback safety net
 * 
 * ═══════════════════════════════════════════════════════════════════════════
 */

console.log("✅ SALA VIRTUAL v10.4 - DUPLICATE KEY FIX APPLIED - Composite keyExtractor");
...
function SalaVirtualEnhancedScreen() {
  ...
  // ✅ CRITICAL FIX v10.4: Deduplicate messages before rendering
  // This is a defense-in-depth approach - we deduplicate in multiple places:
  // 1. In syncMessages when fetching from database
  // 2. In sendPublicMessage/sendPrivateMessage when replacing optimistic messages
  // 3. Here as a final safety net before rendering
  const uniqueMessages = useMemo(() => {
    const seenIds = new Set<string>();
    const deduplicated = messages.filter(msg => {
      if (seenIds.has(msg.id)) {
        console.warn('[SalaVirtual v10.4] ⚠️ Duplicate message ID detected in render:', msg.id);
        return false;
      }
      seenIds.add(msg.id);
      return true;
    ...
    console.log('[SalaVirtual v10.4] 📊 Messages before dedup:', messages.length, ', after dedup:', deduplicated.length);
    
    return deduplicated;
  }, [messages]);

  const uniquePrivateChatMessages = useMemo(() => {
    const seenIds = new Set<string>();
    const deduplicated = privateChatMessages.filter(msg => {
      if (seenIds.has(msg.id)) {
        console.warn('[SalaVirtual v10.4] ⚠️ Duplicate private message ID detected in render:', msg.id);
        return false;
      }
      seenIds.add(msg.id);
      return true;
    ...
    console.log('[SalaVirtual v10.4] 📊 Private messages before dedup:', privateChatMessages.length, ', after dedup:', deduplicated.length);
      ...
      >
        ...
        {activeTab === 'chat' && (
          <View style={styles.chatContainer}>
            {/* ✅ CRITICAL FIX v10.4: Use composite key (id + index) to prevent duplicate key errors
                - This ensures React can properly track each message even if duplicate IDs exist
                - Composite key format: "messageId-arrayIndex" (e.g., "abc123-0", "abc123-1")
                - This is the RECOMMENDED solution from React Native documentation
                - Maintains all v10.3 deduplication logic as primary defense
                - This ensures the input field is never blocked by keyboard */}
            <FlashList
              ref={flashListRef}
              data={uniqueMessages}
        ...
        {activeTab === 'private' && selectedPrivateChat && (
          <View style={styles.chatContainer}>
            ...
            {/* ✅ CRITICAL FIX v10.4: Use composite key (id + index) for private messages
                - Prevents duplicate key errors in private chat
                - Format: "private-messageId-arrayIndex" */}
            <FlashList
              ref={privateChatFlashListRef}
              data={uniquePrivateChatMessages}
              renderItem={renderMessage}
