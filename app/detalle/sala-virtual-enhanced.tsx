...
/**
 * ✅ SALA VIRTUAL v3.0 - PANTALLA COMPLETA CON ICONO X DE CIERRE
 * 
 * CAMBIOS v3.0:
 * - ✅ FIXED: Ahora se abre en PANTALLA COMPLETA (NO modal) en todas las plataformas
 * - ✅ FIXED: Añadido icono X de cierre en la esquina superior derecha
 * - ✅ FIXED: El icono X es visible y funcional en iOS y Android
 * - ✅ FIXED: Navegación contextual preservada (volver a la pestaña correcta)
 * - ✅ RESULTADO: Experiencia consistente - pantalla completa con cierre intuitivo
 */
...
export default function SalaVirtualEnhancedScreen() {
  ...
  console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN CONTEXTUAL: returnTab param:', returnTab);
  console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN CONTEXTUAL: Initializing activeTab with:', initialTab);
  ...
  const loadReadMessagesFromStorage = useCallback(async (localId: string, userId: string): Promise<Set<string>> => {
    try {
      ...
      if (stored) {
        ...
        console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Loaded read partners from storage:', readPartners);
        return new Set(readPartners);
      }
      
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: No stored read partners found');
      return new Set();
    } catch (error) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ FIX PUNTO AZUL: Error loading from storage:', error);
      return new Set();
    }
  }, [getReadMessagesKey]);
  ...
  const saveReadMessagesToStorage = useCallback(async (localId: string, userId: string, partnerId: string) => {
    try {
      ...
      if (!readPartners.includes(partnerId)) {
        ...
        console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Saved read status for partner:', partnerId);
      }
    } catch (error) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ FIX PUNTO AZUL: Error saving to storage:', error);
    }
  }, [getReadMessagesKey]);

  const fetchUserProfile = useCallback(async (userId: string): Promise<UserProfile | null> => {
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🔍 FIX 1: Fetching user profile from database for userId:', userId);
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ FIX 1: Error fetching user profile:', error);
        return null;
      }

      console.log('[SalaVirtual Enhanced v3.0] ✅ FIX 1: User profile fetched successfully');
      console.log('[SalaVirtual Enhanced v3.0] 👤 Name:', data.nombre);
      console.log('[SalaVirtual Enhanced v3.0] 🖼️ Avatar:', data.avatar || 'NO AVATAR - USARÁ DEGRADADO');
      console.log('[SalaVirtual Enhanced v3.0] 📝 Username:', data.username || 'NO USERNAME');
      ...
      console.error('[SalaVirtual Enhanced v3.0] ❌ FIX 1: Error in fetchUserProfile:', error);
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    if (Platform.OS === 'android') {
      console.log('[SalaVirtual Enhanced v3.0] 🎹 FIX 2: Setting up Android keyboard listeners');
      
      const keyboardDidShowListener = Keyboard.addListener('keyboardDidShow', (e) => {
        const height = e.endCoordinates.height;
        console.log('[SalaVirtual Enhanced v3.0] 🎹 FIX 2: Keyboard shown, height:', height);
        setKeyboardHeight(height);
        
        setTimeout(() => {
          if (selectedPrivateChat) {
            console.log('[SalaVirtual Enhanced v3.0] 🎹 FIX 2: Scrolling private chat to bottom');
            privateChatListRef.current?.scrollToEnd({ animated: true });
          } else {
            console.log('[SalaVirtual Enhanced v3.0] 🎹 FIX 2: Scrolling public chat to bottom');
      ...
      const keyboardDidHideListener = Keyboard.addListener('keyboardDidHide', () => {
        console.log('[SalaVirtual Enhanced v3.0] 🎹 FIX 2: Keyboard hidden');
        setKeyboardHeight(0);
      });
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status === 'granted') {
        ...
        console.log('[SalaVirtual Enhanced v3.0] ✅ User location obtained');
      }
    })();
  }, []);
  ...
  useFocusEffect(
    useCallback(() => {
      if (params.returnTab) {
        console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN CON useFocusEffect: Screen gained focus');
  ...
  const checkClosingTime = useCallback(() => {
    ...
    if (estadoLocal.estaAbierto && estadoLocal.tiempoRestante) {
      ...
      console.log('[SalaVirtual Enhanced v3.0] ⏰ Time until closing:', totalMinutes, 'minutes');
  ...
  const loadLocalData = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ No localId provided');
      setLoading(false);
      return;
    }

    try {
      console.log('[SalaVirtual Enhanced v3.0] 🏠 Loading local data for:', localId);
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error loading local:', error);
        setLoading(false);
        return;
      }

      console.log('[SalaVirtual Enhanced v3.0] ✅ Local loaded:', data.nombre);
      setLocal(data);
      ...
      if (!isOpen) {
        console.log('[SalaVirtual Enhanced v3.0] 🔒 Local is closed');
        setLocalClosed(true);
        setLoading(false);
      } else {
        console.log('[SalaVirtual Enhanced v3.0] ✅ Local is open');
        setLocalClosed(false);
      }
    } catch (error) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ Error:', error);
      setLoading(false);
    }
  }, [localId]);
  ...
  const checkUserCheckin = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🔍 Checking if user is checked in...');
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error checking checkin:', error);
        return false;
      }

      const checkedIn = !!data;
      console.log('[SalaVirtual Enhanced v3.0] ✅ User checked in status:', checkedIn);
  ...
  const handleCheckIn = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🚪 User checking in...');
      setCheckingIn(true);
      setIsCheckedIn(true);
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error inserting checkin:', error);
        setIsCheckedIn(false);
        throw new Error('No se pudo entrar en la sala');
      }

      console.log('[SalaVirtual Enhanced v3.0] ✅ User checked in successfully');
      
      setCheckingIn(false);
      return true;
    } catch (error: any) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ Error during checkin:', error);
      setCheckingIn(false);
      return false;
    }
  }, [user, localId]);
  ...
  const handleCheckOut = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🚪 User checking out...');
      ...
      console.log('[SalaVirtual Enhanced v3.0] ✅ User checked out successfully');
      ...
      console.error('[SalaVirtual Enhanced v3.0] ❌ Error checking out:', error);
    }
  }, [user, localId, router]);

  const loadMessages = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ No localId for loadMessages');
      return;
    }
    
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🔥 LOADING INITIAL MESSAGES');
      console.log('[SalaVirtual Enhanced v3.0] 📍 Local ID:', localId);
      ...
      console.log('[SalaVirtual Enhanced v3.0] 📦 RAW DATA from Supabase:', JSON.stringify(data, null, 2));
      console.log('[SalaVirtual Enhanced v3.0] ❌ RAW ERROR from Supabase:', JSON.stringify(error, null, 2));
      
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error loading initial messages:', error);
        setLoading(false);
        return;
      }

      if (!data) {
        console.log('[SalaVirtual Enhanced v3.0] ⚠️ Data is null, setting empty array');
        setMessages([]);
        setLoading(false);
        return;
      }
      ...
      if (data.length === 0) {
        console.log('[SalaVirtual Enhanced v3.0] ⚠️ No messages found');
        setMessages([]);
        setLoading(false);
        return;
      }

      console.log('[SalaVirtual Enhanced v3.0] ✅ DATOS RECIBIDOS CORRECTAMENTE');
      console.log('[SalaVirtual Enhanced v3.0] 📦 Número de mensajes:', data.length);

      const formattedMessages: Message[] = data
        .filter(msg => {
          if (!msg.usuario) {
            console.warn('[SalaVirtual Enhanced v3.0] ⚠️ Message without user data:', msg.id);
            return false;
          }
          return true;
        })
      ...
      console.log('[SalaVirtual Enhanced v3.0] ✅ Formatted', formattedMessages.length, 'public messages');
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔑 Tracking', messageIdsRef.current.size, 'message IDs');

      setMessages(formattedMessages);
      
      if (formattedMessages.length > 0) {
        lastPublicMessageTimestampRef.current = formattedMessages[formattedMessages.length - 1].created_at;
        console.log('[SalaVirtual Enhanced v3.0] 📅 Last public message timestamp:', lastPublicMessageTimestampRef.current);
      }
      
      setLoading(false);
      
      setTimeout(() => {
        console.log('[SalaVirtual Enhanced v3.0] 📜 Scrolling to bottom');
        flatListRef.current?.scrollToEnd({ animated: false });
      }, 300);
    } catch (error) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ Error loading messages:', error);
      setLoading(false);
    }
  }, [localId]);

  const triggerReceivedAnimation = useCallback((messageText: string, tipo: string) => {
    console.log('[SalaVirtual Enhanced v3.0] 🎬 Triggering received animation for tipo:', tipo);
    
    if (tipo !== 'privado') {
      console.log('[SalaVirtual Enhanced v3.0] ⏭️ Not a private message, skipping animation');
      return;
    }
    ...
    console.log('[SalaVirtual Enhanced v3.0] 🎬 Showing received animation with emoji:', emoji);
  ...
  const syncMessages = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🔄 Syncing messages...');
      ...
      if (lastPublicMessageTimestampRef.current) {
        ...
        console.log('[SalaVirtual Enhanced v3.0] 📅 Fetching messages after:', lastPublicMessageTimestampRef.current);
      } else {
        console.log('[SalaVirtual Enhanced v3.0] 📅 Fetching all messages (no timestamp set)');
      }

      const { data: publicData, error: publicError } = await publicQuery;

      if (publicError) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error syncing public messages:', publicError);
      } else if (publicData && publicData.length > 0) {
        console.log('[SalaVirtual Enhanced v3.0] 📨 Found', publicData.length, 'NEW public messages');
        ...
        const uniqueNewMessages = newMessages.filter(msg => {
          if (messageIdsRef.current.has(msg.id)) {
            console.log('[SalaVirtual Enhanced v3.0] ⏭️ Skipping duplicate message ID:', msg.id);
            return false;
          }
          return true;
        });
        ...
        if (uniqueNewMessages.length > 0) {
          console.log('[SalaVirtual Enhanced v3.0] ✅ Adding', uniqueNewMessages.length, 'unique new messages to UI');
          ...
          uniqueNewMessages.forEach(msg => {
            ...
            if (msg.usuario_id === user.id) {
              ...
              console.log('[SalaVirtual Enhanced v3.0] ✅ Removed pending message:', pendingId);
            }
            
            if (msg.usuario_id !== user.id && msg.tipo === 'privado') {
              console.log('[SalaVirtual Enhanced v3.0] 🎬 Received private message from another user!');
              console.log('[SalaVirtual Enhanced v3.0] 👤 From:', msg.usuario.nombre);
              console.log('[SalaVirtual Enhanced v3.0] 💬 Content:', msg.contenido);
              triggerReceivedAnimation(msg.contenido, msg.tipo);
            }
          });
          
          console.log('[SalaVirtual Enhanced v3.0] 🔑 Now tracking', messageIdsRef.current.size, 'message IDs');
          ...
          setMessages(prev => {
            ...
            console.log('[SalaVirtual Enhanced v3.0] 📊 Total messages in UI after update:', updated.length);
          ...
          console.log('[SalaVirtual Enhanced v3.0] 📅 Updated last timestamp to:', lastPublicMessageTimestampRef.current);
          
          setTimeout(() => {
            console.log('[SalaVirtual Enhanced v3.0] 📜 Scrolling to new messages');
            flatListRef.current?.scrollToEnd({ animated: true });
          }, 100);
        } else {
          console.log('[SalaVirtual Enhanced v3.0] ℹ️ No new unique messages to add');
        }
      } else {
        console.log('[SalaVirtual Enhanced v3.0] ℹ️ No new public messages found');
      }
      ...
      if (privateError) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error syncing private messages:', privateError);
      } else if (privateData && privateData.length > 0) {
        console.log('[SalaVirtual Enhanced v3.0] 📨 Found', privateData.length, 'new private messages');
        
        if (privateData.length > 0) {
          lastPrivateMessageTimestampRef.current = privateData[privateData.length - 1].created_at;
        }
        
        console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Reloading private chats to update unread counts');
        loadPrivateChats();
          ...
          if (relevantMessages.length > 0) {
            ...
            setPrivateChatMessages(prev => {
              ...
              if (uniqueNew.length > 0) {
                console.log('[SalaVirtual Enhanced v3.0] ✅ Adding', uniqueNew.length, 'new private messages to UI');
                ...
                uniqueNew.forEach(msg => {
                  ...
                  if (msg.usuario_id !== user.id && msg.tipo === 'privado') {
                    console.log('[SalaVirtual Enhanced v3.0] 🎬 Received private message!');
                    console.log('[SalaVirtual Enhanced v3.0] 👤 From:', msg.usuario.nombre);
                    console.log('[SalaVirtual Enhanced v3.0] 💬 Content:', msg.contenido);
                    triggerReceivedAnimation(msg.contenido, msg.tipo);
                  }
      ...
      console.error('[SalaVirtual Enhanced v3.0] ❌ Error syncing messages:', error);
    }
  }, [localId, user, selectedPrivateChat, triggerReceivedAnimation]);

  useEffect(() => {
    if (!localId || !user || !isCheckedIn) {
      console.log('[SalaVirtual Enhanced v3.0] ⏸️ Polling not started - missing requirements');
      return;
    }

    console.log('[SalaVirtual Enhanced v3.0] 🔥 STARTING ULTRA-AGGRESSIVE MESSAGE POLLING (every 1.5 seconds)');
    ...
    return () => {
      if (messageSyncIntervalRef.current) {
        console.log('[SalaVirtual Enhanced v3.0] 🛑 Stopping message polling');
  ...
  const updateActiveUsers = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🔄 Updating active users list...');
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error loading active users:', error);
        return;
      }
      ...
      console.log('[SalaVirtual Enhanced v3.0] 👥 Found', users.length, 'active users');
      ...
      if (user) {
        ...
        console.log('[SalaVirtual Enhanced v3.0] ✅ Current user moved to first position');
      }
  ...
  const markPrivateMessagesAsRead = useCallback(async (partnerId: string) => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Marking private messages as read from:', partnerId);
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Updating WHERE leido = false AND recipient_id = user.id AND usuario_id = partnerId');
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ FIX PUNTO AZUL: Error marking messages as read:', error);
      } else {
        console.log('[SalaVirtual Enhanced v3.0] ✅ FIX PUNTO AZUL: Messages marked as read in database');
      }
      
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Saving read status to AsyncStorage for:', partnerId);
      await saveReadMessagesToStorage(localId, user.id, partnerId);
      
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Updating unread counter IMMEDIATELY in frontend state');
      setPrivateChats(prev => 
        prev.map(chat => {
          if (chat.userId === partnerId) {
            console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Setting unreadCount to 0 for user:', partnerId);
            return { ...chat, unreadCount: 0 };
          }
          return chat;
        })
      );
      console.log('[SalaVirtual Enhanced v3.0] ✅ FIX PUNTO AZUL: Unread counter set to 0 for user:', partnerId);
    } catch (error) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ FIX PUNTO AZUL: Error:', error);
    }
  ...
  const loadPrivateChats = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 🔄 Loading private chats...');
      
      const readPartners = await loadReadMessagesFromStorage(localId, user.id);
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Read partners from storage:', Array.from(readPartners));
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error loading private chats:', error);
        return;
      }
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Counting ONLY unread messages WHERE leido = false AND recipient_id = user.id');
      ...
      (privateMessages || []).forEach(msg => {
        ...
        if (msg.recipient_id === user.id && msg.usuario_id !== user.id && msg.leido === false) {
          if (!readPartners.has(partnerId)) {
            ...
            console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Unread message from', partnerId, '- count:', currentCount + 1);
          } else {
            console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Skipping count for', partnerId, '- already marked as read in storage');
          }
        }
      ...
      console.log('[SalaVirtual Enhanced v3.0] ✅ Private chats loaded:', chats.length);
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Total unread messages (respecting storage):', 
  ...
  const handleTypingStart = useCallback(() => {
    ...
    console.log('[SalaVirtual Enhanced v3.0] ⌨️ FIX 4: User started typing to:', selectedPrivateChat.userId);
  ...
  const handleTypingStop = useCallback(() => {
    ...
    console.log('[SalaVirtual Enhanced v3.0] ⌨️ FIX 4: User stopped typing to:', selectedPrivateChat.userId);
  ...
  const subscribeToTypingEvents = useCallback(() => {
    ...
    console.log('[SalaVirtual Enhanced v3.0] ⌨️ FIX 4: Setting up typing indicator subscription');

    const typingChannel = supabase
      .channel(`typing_events_${localId}_${user.id}_${Date.now()}`)
      .on('broadcast', { event: 'typing_start' }, (payload: any) => {
        console.log('[SalaVirtual Enhanced v3.0] ⌨️ FIX 4: Received typing_start:', payload);
        
        if (payload.payload.recipientId === user.id && selectedPrivateChat?.userId === payload.payload.userId) {
          console.log('[SalaVirtual Enhanced v3.0] ⌨️ FIX 4: Partner is typing...');
          setTypingUsers(prev => {
      ...
      .on('broadcast', { event: 'typing_stop' }, (payload: any) => {
        console.log('[SalaVirtual Enhanced v3.0] ⌨️ FIX 4: Received typing_stop:', payload);
        
        if (payload.payload.recipientId === user.id) {
          console.log('[SalaVirtual Enhanced v3.0] ⌨️ FIX 4: Partner stopped typing');
          setTypingUsers(prev => {
            const newSet = new Set(prev);
      ...
      .subscribe((status) => {
        console.log('[SalaVirtual Enhanced v3.0] ⌨️ FIX 4: Typing channel status:', status);
      });

    typingChannelRef.current = typingChannel;

    return () => {
      console.log('[SalaVirtual Enhanced v3.0] 🔌 Unsubscribing from typing channel');
  ...
  const subscribeToUpdates = useCallback(() => {
    ...
    console.log('[SalaVirtual Enhanced v3.0] 📡 Setting up real-time subscriptions...');
    ...
    const chatChannel = supabase
      ...
      .on(
        ...
        async (payload) => {
          console.log('[SalaVirtual Enhanced v3.0] 📨 Real-time INSERT event received:', payload);
          
          const newRecord = payload.new as any;
          
          if (newRecord.usuario_id === user.id) {
            console.log('[SalaVirtual Enhanced v3.0] ⏭️ Skipping own message (already in UI optimistically)');
            return;
          }

          console.log('[SalaVirtual Enhanced v3.0] 🔄 Triggering immediate sync for new message from other user');
          syncMessages();
        }
      )
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual Enhanced v3.0] 🗑️ Message deleted from DB:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual Enhanced v3.0] 🔄 Message updated in DB:', payload);
          
          const updatedRecord = payload.new as any;
          
          if (updatedRecord.tipo === 'privado' && updatedRecord.leido === true) {
            console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Message marked as read via real-time, reloading chats');
            setTimeout(() => {
              loadPrivateChats();
            }, 300);
          }
      ...
      .subscribe((status) => {
        console.log('[SalaVirtual Enhanced v3.0] 📡 Chat channel status:', status);
        
        if (status === 'SUBSCRIBED') {
          console.log('[SalaVirtual Enhanced v3.0] ✅ Real-time subscription active');
        } else if (status === 'CHANNEL_ERROR') {
          console.warn('[SalaVirtual Enhanced v3.0] ⚠️ Real-time subscription error - polling will handle all sync');
        } else if (status === 'TIMED_OUT') {
          console.warn('[SalaVirtual Enhanced v3.0] ⏱️ Real-time subscription timed out - polling will handle all sync');
        }
      });
    ...
    const checkinsChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual Enhanced v3.0] 👤 Check-in event:', payload.eventType);
          updateActiveUsers();
        }
      )
      .subscribe((status) => {
        console.log('[SalaVirtual Enhanced v3.0] 📡 Checkins channel status:', status);
      });
    ...
    return () => {
      console.log('[SalaVirtual Enhanced v3.0] 🔌 Unsubscribing from channels');
  ...
  useEffect(() => {
    ...
    console.log('[SalaVirtual Enhanced v3.0] 🚀 INITIALIZING VIRTUAL ROOM');

    const init = async () => {
      console.log('[SalaVirtual Enhanced v3.0] 1️⃣ Loading local data...');
      await loadLocalData();
      
      console.log('[SalaVirtual Enhanced v3.0] 2️⃣ Checking user check-in status...');
      const checkedIn = await checkUserCheckin();
      
      if (!checkedIn && !localClosed && user) {
        console.log('[SalaVirtual Enhanced v3.0] 3️⃣ User not checked in, checking in now...');
        const success = await handleCheckIn();
        if (!success) {
          console.error('[SalaVirtual Enhanced v3.0] ❌ Check-in failed, aborting initialization');
          return;
        }
      }
      ...
      console.log('[SalaVirtual Enhanced v3.0] 4️⃣ Loading initial messages...');
      await loadMessages();
      
      console.log('[SalaVirtual Enhanced v3.0] 5️⃣ Setting up real-time subscriptions...');
      subscribeToUpdates();
      
      console.log('[SalaVirtual Enhanced v3.0] 6️⃣ Setting up typing events...');
      subscribeToTypingEvents();
      
      console.log('[SalaVirtual Enhanced v3.0] 7️⃣ Updating active users...');
      await updateActiveUsers();
      
      console.log('[SalaVirtual Enhanced v3.0] 8️⃣ Loading private chats...');
      await loadPrivateChats();
      
      console.log('[SalaVirtual Enhanced v3.0] ✅ INITIALIZATION COMPLETE');
    };

    init();

    return () => {
      console.log('[SalaVirtual Enhanced v3.0] 🧹 Cleanup: Checking out user');
      handleCheckOut();
    };
  }, [localId]);
  ...
  useEffect(() => {
    if (activeTab === 'private' && user && localId) {
      console.log('[SalaVirtual Enhanced v3.0] 🔄 Private tab active, reloading chats');
      loadPrivateChats();
    }
  ...
  useEffect(() => {
    if (selectedPrivateChat && user && localId) {
      console.log('[SalaVirtual Enhanced v3.0] 🔄 FIX PUNTO AZUL: selectedPrivateChat changed, syncing state...');
      console.log('[SalaVirtual Enhanced v3.0] 🔄 FIX PUNTO AZUL: Partner ID:', selectedPrivateChat.userId);
      
      const syncProfile = async () => {
        console.log('[SalaVirtual Enhanced v3.0] 🔄 FIX 1: Fetching fresh profile for partner...');
        const profile = await fetchUserProfile(selectedPrivateChat.userId);
        
        if (profile) {
          console.log('[SalaVirtual Enhanced v3.0] ✅ FIX 1: Profile fetched and updated');
          console.log('[SalaVirtual Enhanced v3.0] 🖼️ FIX 1: Avatar:', profile.avatar || 'NO AVATAR');
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔄 FIX PUNTO AZUL: Marking messages as read...');
  ...
  const sendPublicMessage = useCallback(async (content: string) => {
    if (!user || !localId || !content.trim()) {
      console.log('[SalaVirtual Enhanced v3.0] ⚠️ Cannot send message - missing requirements');
      return;
    }

    try {
      console.log('[SalaVirtual Enhanced v3.0] 📤 Sending public message:', content);
      setSending(true);
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔥 FIX 1: Fetching user profile from database...');
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔥 FIX 1: Using profile data from database');
      console.log('[SalaVirtual Enhanced v3.0] 👤 User profile:', currentUserProfile.nombre);
      console.log('[SalaVirtual Enhanced v3.0] 🖼️ Avatar URL:', currentUserProfile.avatar || 'NO AVATAR');
      ...
      console.log('[SalaVirtual Enhanced v3.0] ✨ FIX 1: Adding message optimistically with avatar from database');
      console.log('[SalaVirtual Enhanced v3.0] 👤 User:', optimisticMsg.usuario.nombre, '| Avatar:', !!optimisticMsg.usuario.avatar);
      ...
      console.log('[SalaVirtual Enhanced v3.0] 💾 Saving message to database...');
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error saving message to DB:', error);
        
        messageIdsRef.current.delete(messageId);
        setMessages(prev => prev.filter(m => m.id !== messageId));
        pendingMessageIds.current.delete(pendingId);
      } else {
        console.log('[SalaVirtual Enhanced v3.0] ✅ Public message saved to DB with id:', insertedMessage.id);
        ...
        setMessages(prev => {
          ...
          if (withoutOptimistic.some(m => m.id === realMessage.id)) {
            console.log('[SalaVirtual Enhanced v3.0] ℹ️ Real message already in UI from polling');
  ...
  const triggerFloatingReaction = useCallback((emoji: string) => {
    ...
    console.log(`[SalaVirtual Enhanced v3.0] 🎉 Floating reaction triggered: ${emoji}`);
  }, []);
  ...
  const sendPredefinedMessage = useCallback(async (recipientId: string, messageText: string) => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 📤 Sending predefined message:', messageText, 'to:', recipientId);
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔥 FIX 1: Fetching user profile from database for predefined message...');
      const userProfile = await fetchUserProfile(user.id);
      ...
      if (existingChatIndex === -1) {
        ...
        console.log('[SalaVirtual Enhanced v3.0] ✨ Creating new private chat optimistically');
        setPrivateChats(prev => [newChat, ...prev]);
      } else {
        console.log('[SalaVirtual Enhanced v3.0] ✨ Updating existing private chat optimistically');
        setPrivateChats(prev => {
          const updatedChat = {
      ...
      console.log('[SalaVirtual Enhanced v3.0] 💾 FIX 3: Saving with tipo = "privado" (NOT "rapido")');
      ...
      if (insertError) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ FIX 3: Error saving private message to DB:', insertError);
        console.error('[SalaVirtual Enhanced v3.0] ❌ FIX 3: Error code:', insertError.code);
        console.error('[SalaVirtual Enhanced v3.0] ❌ FIX 3: Error message:', insertError.message);
      } else {
        console.log('[SalaVirtual Enhanced v3.0] ✅ FIX 3: Private message saved to database with tipo = "privado"');
      ...
      console.log(`[SalaVirtual Enhanced v3.0] ✅ Message sent to ${recipientName}`);
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔄 Switching to private conversations tab');
      ...
      console.error('[SalaVirtual Enhanced v3.0] ❌ Error sending predefined message:', error);
  ...
  const closeBottomSheet = useCallback(() => {
    console.log('[SalaVirtual Enhanced v3.0] 📋 Closing bottom sheet');
  ...
  const sendPrivateMessage = useCallback(async (recipientId: string, content: string) => {
    ...
    try {
      console.log('[SalaVirtual Enhanced v3.0] 📤 Sending private message to:', recipientId);
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔥 FIX 1: Fetching user profile from database for private message...');
      const userProfile = await fetchUserProfile(user.id);
      ...
      console.log('[SalaVirtual Enhanced v3.0] 🔥 FIX 1: Using profile data from database for private message');
      ...
      console.log('[SalaVirtual Enhanced v3.0] ✨ Adding private message optimistically to UI');
      ...
      if (insertError) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error saving private message to DB:', insertError);
        setPrivateChatMessages(prev => prev.filter(m => m.id !== messageId));
        pendingMessageIds.current.delete(pendingId);
      } else {
        console.log('[SalaVirtual Enhanced v3.0] ✅ Private message saved to database with id:', insertedMessage.id);
      ...
      console.log('[SalaVirtual Enhanced v3.0] ✅ Private message sent');
    } catch (error) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ Error sending private message:', error);
    }
  ...
  const openPrivateChat = useCallback(async (chat: PrivateChat) => {
    ...
    try {
      ...
      console.log('[SalaVirtual Enhanced v3.0] 💬 Opening private chat with:', displayName);
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Unread count before:', chat.unreadCount);
      
      setSelectedPrivateChat(chat);
      
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Marking messages as read in database...');
      await markPrivateMessagesAsRead(chat.userId);
      
      console.log('[SalaVirtual Enhanced v3.0] 🔵 FIX PUNTO AZUL: Unread count set to 0 in frontend');
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error loading private messages:', error);
        return;
      }
      ...
      console.log('[SalaVirtual Enhanced v3.0] ✅ Private messages loaded:', formattedMessages.length);
      ...
      console.error('[SalaVirtual Enhanced v3.0] ❌ Error opening private chat:', error);
    }
  }, [user, localId, activeUsers, markPrivateMessagesAsRead]);

  const closePrivateChat = useCallback(() => {
    console.log('[SalaVirtual Enhanced v3.0] 💬 Closing private chat');
  ...
  const handleDeleteMessage = useCallback(async (message: Message) => {
    if (!user || message.usuario_id !== user.id) {
      console.log('[SalaVirtual Enhanced v3.0] ⚠️ Cannot delete message - not owner');
      return;
    }

    console.log('[SalaVirtual Enhanced v3.0] 🗑️ Showing delete confirmation for message:', message.id);
  ...
  const confirmDeleteMessage = useCallback(async () => {
    ...
    try {
      setDeleting(true);
      console.log('[SalaVirtual Enhanced v3.0] 🗑️ Deleting message:', messageToDelete.id);
      ...
      if (error) {
        console.error('[SalaVirtual Enhanced v3.0] ❌ Error deleting message:', error);
        ...
        console.log('[SalaVirtual Enhanced v3.0] ✅ Message deleted successfully');
  ...
  const cancelDeleteMessage = useCallback(() => {
    console.log('[SalaVirtual Enhanced v3.0] ❌ Delete cancelled');
    setShowDeleteModal(false);
    setMessageToDelete(null);
  }, []);
  ...
  const handleUserPress = async (selectedUser: ActiveUser) => {
    ...
    console.log('[SalaVirtual Enhanced v3.0] 👤 User pressed:', displayName);
    
    if (selectedUser.id === user?.id) {
      console.log('[SalaVirtual Enhanced v3.0] ⚠️ Cannot interact with self');
      return;
    }
    
    console.log('[SalaVirtual Enhanced v3.0] 🔥 FIX 1: Fetching user profile for bottom sheet cover...');
    const profile = await fetchUserProfile(selectedUser.id);
    
    if (profile) {
      console.log('[SalaVirtual Enhanced v3.0] ✅ FIX 1: Profile fetched for bottom sheet');
      console.log('[SalaVirtual Enhanced v3.0] 🖼️ FIX 1: Cover photo (avatar):', profile.avatar || 'NO AVATAR - USARÁ DEGRADADO');
      setSelectedUserProfile(profile);
    } else {
      console.log('[SalaVirtual Enhanced v3.0] ⚠️ FIX 1: Could not fetch profile, using cached data');
    ...
    console.log('[SalaVirtual Enhanced v3.0] 📋 Opening bottom sheet for user');
  ...
  const handleViewProfile = useCallback(async () => {
    if (!selectedUser) {
      console.log('[SalaVirtual Enhanced v3.0] ⚠️ NAVEGACIÓN: No selected user');
      return;
    }
    
    console.log('[SalaVirtual Enhanced v3.0] 🚀 NAVEGACIÓN: Starting profile navigation');
    console.log('[SalaVirtual Enhanced v3.0] 👤 NAVEGACIÓN: Target user ID:', selectedUser.id);
    console.log('[SalaVirtual Enhanced v3.0] 👤 NAVEGACIÓN: Target user name:', selectedUser.nombre);
    console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN: Current active tab:', activeTab);
    console.log('[SalaVirtual Enhanced v3.0] 🏠 NAVEGACIÓN: Current local ID:', localId);
    
    console.log('[SalaVirtual Enhanced v3.0] 📋 NAVEGACIÓN: Step 1 - Closing bottom sheet to hide modal...');
    closeBottomSheet();
    setSelectedPrivateChat(null);
    
    console.log('[SalaVirtual Enhanced v3.0] ⏳ NAVEGACIÓN: Waiting for bottom sheet animation to complete (300ms)...');
    await new Promise(resolve => setTimeout(resolve, 300));
    console.log('[SalaVirtual Enhanced v3.0] ✅ NAVEGACIÓN: Bottom sheet closed and hidden');
    
    console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN: Step 2 - Navigating to profile with context params...');
    console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN: Params to send:');
    console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN:   - userId:', selectedUser.id);
    console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN:   - from: sala-virtual');
    console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN:   - returnTab:', activeTab);
    console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN:   - localId:', localId);
    ...
    try {
      ...
      console.log('[SalaVirtual Enhanced v3.0] ✅ NAVEGACIÓN: Navigation executed successfully');
      console.log('[SalaVirtual Enhanced v3.0] 🎉 NAVEGACIÓN: Profile will open ABOVE the virtual room modal (thanks to presentation: card)');
      console.log('[SalaVirtual Enhanced v3.0] 🎉 NAVEGACIÓN: Bottom sheet is now hidden, so modal will NOT overlap profile');
    } catch (error) {
      console.error('[SalaVirtual Enhanced v3.0] ❌ NAVEGACIÓN: Error navigating:', error);
    }
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onLongPress={() => {
          if (isOwnMessage) {
            console.log('[SalaVirtual Enhanced v3.0] 🗑️ Long press on own message');
      ...
      >
        {!isOwnMessage && (
          <TouchableOpacity
            ...
            onPress={() => {
              console.log('[SalaVirtual Enhanced v3.0] 👤 Avatar clicked');
        ...
        {isOwnMessage && (
          <TouchableOpacity
            style={[styles.messageAvatar, { width: avatarSize, height: avatarSize }]}
            onPress={() => {
              console.log('[SalaVirtual Enhanced v3.0] 👤 Navigating to own profile');
              router.push('/perfil');
            }}
          >
            {item.usuario.avatar ? (
  ...
  const renderUserItem = ({ item, index }: { item: ActiveUser; index: number }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          console.log('[SalaVirtual Enhanced v3.0] 👤 Grid user card pressed:', displayName);
          if (isCurrentUser) {
            console.log('[SalaVirtual Enhanced v3.0] 👤 Navigating to own profile');
            router.push('/perfil');
          } else {
  ...
  if (loading) {
    console.log('[SalaVirtual Enhanced v3.0] ⏳ Showing loading state');
    return (
      <LinearGradient
  ...
  console.log('[SalaVirtual Enhanced v3.0] 🎹 FIX 2: Chat list padding bottom:', chatListPaddingBottom);
          ...
          <View style={[
            ...
            <View style={styles.tabBar}>
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual Enhanced v3.0] 🔄 Switching to Chat tab');
                  setActiveTab('chat');
                  if (selectedPrivateChat) {
                    closePrivateChat();
                  }
              ...
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual Enhanced v3.0] 🔄 Switching to Users tab');
                  setActiveTab('users');
                  if (selectedPrivateChat) {
                    closePrivateChat();
                  }
              ...
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual Enhanced v3.0] 🔄 Switching to private tab');
                  setActiveTab('private');
                  loadPrivateChats();
                }}
                activeOpacity={0.7}
          ...
          {activeTab === 'chat' ? (
            <React.Fragment>
              <FlatList
                ...
                onContentSizeChange={() => {
                  console.log('[SalaVirtual Enhanced v3.0] 🎹 FIX 2: Content size changed, scrolling to bottom');
                  flatListRef.current?.scrollToEnd({ animated: true });
                }}
                ListEmptyComponent={
              ...
              <View style={[
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    console.log('[SalaVirtual Enhanced v3.0] ⚡ Toggling quick messages:', !showQuickMessages);
              ...
              {selectedPrivateChat ? (
                <React.Fragment>
                  <View style={[
                    ...
                    <TouchableOpacity
                      onPress={async () => {
                        console.log('[SalaVirtual Enhanced v3.0] 🚀 NAVEGACIÓN: Profile button pressed in private chat header');
                        console.log('[SalaVirtual Enhanced v3.0] 👤 NAVEGACIÓN: Partner ID:', selectedPrivateChat.userId);
                        console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN: Current active tab:', activeTab);
                        
                        const targetUserId = selectedPrivateChat.userId;
                        
                        console.log('[SalaVirtual Enhanced v3.0] 💬 NAVEGACIÓN: Closing private chat...');
                        closePrivateChat();
                        
                        await new Promise(resolve => setTimeout(resolve, 300));
                        
                        console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN: Executing router.push() to profile');
                        console.log('[SalaVirtual Enhanced v3.0] 🎯 NAVEGACIÓN: Params: from=sala-virtual, returnTab=' + activeTab + ', localId=' + localId);
                        try {
                          router.push({
                            pathname: '/perfil/usuario',
                            params: {
                          ...
                          console.log('[SalaVirtual Enhanced v3.0] ✅ NAVEGACIÓN: Navigation complete');
                          console.log('[SalaVirtual Enhanced v3.0] 🎉 NAVEGACIÓN: Profile will open ABOVE the virtual room modal');
                        } catch (error) {
                          console.error('[SalaVirtual Enhanced v3.0] ❌ NAVEGACIÓN: Error navigating:', error);
                        }
                      }}
                  ...
                  <FlatList
                    ...
                    onContentSizeChange={() => {
                      console.log('[SalaVirtual Enhanced v3.0] 🎹 FIX 2: Private chat content size changed, scrolling to bottom');
                      privateChatListRef.current?.scrollToEnd({ animated: true });
                    }}
                    onLayout={() => {
