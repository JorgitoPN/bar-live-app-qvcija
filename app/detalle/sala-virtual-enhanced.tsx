...
// ⚠️ SALA VIRTUAL v3.7 - FIXES: Header icons redesigned, keyboard handling improved
console.log("⚠️ SALA VIRTUAL v3.7 - HEADER ICONS REDESIGNED & KEYBOARD FIX");
...
/**
 * ✅ SALA VIRTUAL v3.7 - HEADER ICONS REDESIGNED & KEYBOARD FIX
 * 
 * CAMBIOS v3.7:
 * - ✅ Header icons completamente rediseñados con contenedores circulares
 * - ✅ Iconos con fondo semitransparente y bordes para mejor visibilidad
 * - ✅ Icono X de cierre ahora visible y correctamente alineado
 * - ✅ KeyboardAvoidingView mejorado con mejor comportamiento
 * - ✅ Campo de texto siempre visible cuando aparece el teclado
 * - ✅ Scroll automático al fondo cuando aparece el teclado
 */
...
export default function SalaVirtualEnhancedScreen() {
  ...
  console.log('[SalaVirtual v3.7] 🎯 INITIAL TAB from params:', initialTab);
  ...
  const loadReadMessagesFromStorage = useCallback(async (localId: string, userId: string): Promise<Set<string>> => {
    try {
      ...
      if (stored) {
        ...
        console.log('[SalaVirtual v3.7] 🔵 Loaded read partners from storage:', readPartners);
        return new Set(readPartners);
      }
      
      console.log('[SalaVirtual v3.7] 🔵 No stored read partners found');
      return new Set();
    } catch (error) {
      console.error('[SalaVirtual v3.7] ❌ Error loading from storage:', error);
      return new Set();
    }
  }, [getReadMessagesKey]);
  ...
  const saveReadMessagesToStorage = useCallback(async (localId: string, userId: string, partnerId: string) => {
    try {
      ...
      if (!readPartners.includes(partnerId)) {
        ...
        console.log('[SalaVirtual v3.7] 🔵 Saved read status for partner:', partnerId);
      }
    } catch (error) {
      console.error('[SalaVirtual v3.7] ❌ Error saving to storage:', error);
    }
  }, [getReadMessagesKey]);

  const fetchUserProfile = useCallback(async (userId: string): Promise<UserProfile | null> => {
    try {
      console.log('[SalaVirtual v3.7] 🔍 Fetching user profile from database for userId:', userId);
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error fetching user profile:', error);
        return null;
      }

      console.log('[SalaVirtual v3.7] ✅ User profile fetched successfully');
      console.log('[SalaVirtual v3.7] 👤 Name:', data.nombre);
      console.log('[SalaVirtual v3.7] 🖼️ Avatar:', data.avatar || 'NO AVATAR');
      console.log('[SalaVirtual v3.7] 📝 Username:', data.username || 'NO USERNAME');
      ...
      console.error('[SalaVirtual v3.7] ❌ Error in fetchUserProfile:', error);
      return null;
    }
  }, []);

  // ✅ FIX v3.7: Improved keyboard handling with auto-scroll
  useEffect(() => {
    console.log('[SalaVirtual v3.7] 🎹 Setting up keyboard listeners for', Platform.OS);
    ...
    const showSubscription = Keyboard.addListener(keyboardShowEvent, (e) => {
      ...
      console.log('[SalaVirtual v3.7] 🎹 Keyboard shown, height:', height);
      ...
      setTimeout(() => {
        if (selectedPrivateChat) {
          console.log('[SalaVirtual v3.7] 🎹 Scrolling private chat to bottom');
          privateChatListRef.current?.scrollToEnd({ animated: true });
        } else {
          console.log('[SalaVirtual v3.7] 🎹 Scrolling public chat to bottom');
    ...
    const hideSubscription = Keyboard.addListener(keyboardHideEvent, () => {
      console.log('[SalaVirtual v3.7] 🎹 Keyboard hidden');
      setKeyboardHeight(0);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status === 'granted') {
        ...
        console.log('[SalaVirtual v3.7] ✅ User location obtained');
      }
    })();
  }, []);
  ...
  const checkClosingTime = useCallback(() => {
    ...
    if (estadoLocal.estaAbierto && estadoLocal.tiempoRestante) {
      ...
      console.log('[SalaVirtual v3.7] ⏰ Time until closing:', totalMinutes, 'minutes');
  ...
  const loadLocalData = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual v3.7] ❌ No localId provided');
      setLoading(false);
      return;
    }

    try {
      console.log('[SalaVirtual v3.7] 🏠 Loading local data for:', localId);
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error loading local:', error);
        setLoading(false);
        return;
      }

      console.log('[SalaVirtual v3.7] ✅ Local loaded:', data.nombre);
      setLocal(data);
      ...
      if (!isOpen) {
        console.log('[SalaVirtual v3.7] 🔒 Local is closed');
        setLocalClosed(true);
        setLoading(false);
      } else {
        console.log('[SalaVirtual v3.7] ✅ Local is open');
        setLocalClosed(false);
      }
    } catch (error) {
      console.error('[SalaVirtual v3.7] ❌ Error:', error);
      setLoading(false);
    }
  }, [localId]);
  ...
  const checkUserCheckin = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 🔍 Checking if user is checked in...');
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('[SalaVirtual v3.7] ❌ Error checking checkin:', error);
        return false;
      }

      const checkedIn = !!data;
      console.log('[SalaVirtual v3.7] ✅ User checked in status:', checkedIn);
  ...
  const handleCheckIn = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 🚪 User checking in...');
      setCheckingIn(true);
      setIsCheckedIn(true);
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error inserting checkin:', error);
        setIsCheckedIn(false);
        throw new Error('No se pudo entrar en la sala');
      }

      console.log('[SalaVirtual v3.7] ✅ User checked in successfully');
      
      setCheckingIn(false);
      return true;
    } catch (error: any) {
      console.error('[SalaVirtual v3.7] ❌ Error during checkin:', error);
      setCheckingIn(false);
      return false;
    }
  }, [user, localId]);
  ...
  const handleCheckOut = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 🚪 User checking out, navigating to local details...');
      ...
      console.log('[SalaVirtual v3.7] ✅ User checked out successfully');
      ...
      console.error('[SalaVirtual v3.7] ❌ Error checking out:', error);
    }
  }, [user, localId, router]);

  const loadMessages = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual v3.7] ❌ No localId for loadMessages');
      return;
    }
    
    try {
      console.log('[SalaVirtual v3.7] 🔥 LOADING INITIAL MESSAGES');
      console.log('[SalaVirtual v3.7] 📍 Local ID:', localId);
      ...
      console.log('[SalaVirtual v3.7] 📦 RAW DATA from Supabase:', JSON.stringify(data, null, 2));
      console.log('[SalaVirtual v3.7] ❌ RAW ERROR from Supabase:', JSON.stringify(error, null, 2));
      
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error loading initial messages:', error);
        setLoading(false);
        return;
      }

      if (!data) {
        console.log('[SalaVirtual v3.7] ⚠️ Data is null, setting empty array');
        setMessages([]);
        setLoading(false);
        return;
      }
      ...
      if (data.length === 0) {
        console.log('[SalaVirtual v3.7] ⚠️ No messages found');
        setMessages([]);
        setLoading(false);
        return;
      }

      console.log('[SalaVirtual v3.7] ✅ DATOS RECIBIDOS CORRECTAMENTE');
      console.log('[SalaVirtual v3.7] 📦 Número de mensajes:', data.length);

      const formattedMessages: Message[] = data
        .filter(msg => {
          if (!msg.usuario) {
            console.warn('[SalaVirtual v3.7] ⚠️ Message without user data:', msg.id);
            return false;
          }
          return true;
        })
      ...
      console.log('[SalaVirtual v3.7] ✅ Formatted', formattedMessages.length, 'public messages');
      ...
      console.log('[SalaVirtual v3.7] 🔑 Tracking', messageIdsRef.current.size, 'message IDs');

      setMessages(formattedMessages);
      
      if (formattedMessages.length > 0) {
        lastPublicMessageTimestampRef.current = formattedMessages[formattedMessages.length - 1].created_at;
        console.log('[SalaVirtual v3.7] 📅 Last public message timestamp:', lastPublicMessageTimestampRef.current);
      }
      
      setLoading(false);
      
      setTimeout(() => {
        console.log('[SalaVirtual v3.7] 📜 Scrolling to bottom');
        flatListRef.current?.scrollToEnd({ animated: false });
      }, 300);
    } catch (error) {
      console.error('[SalaVirtual v3.7] ❌ Error loading messages:', error);
      setLoading(false);
    }
  }, [localId]);

  const triggerReceivedAnimation = useCallback((messageText: string, tipo: string) => {
    console.log('[SalaVirtual v3.7] 🎬 Triggering received animation for tipo:', tipo);
    
    if (tipo !== 'privado') {
      console.log('[SalaVirtual v3.7] ⏭️ Not a private message, skipping animation');
      return;
    }
    ...
    console.log('[SalaVirtual v3.7] 🎬 Showing received animation with emoji:', emoji);
  ...
  const syncMessages = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 🔄 Syncing messages...');
      ...
      if (lastPublicMessageTimestampRef.current) {
        ...
        console.log('[SalaVirtual v3.7] 📅 Fetching messages after:', lastPublicMessageTimestampRef.current);
      } else {
        console.log('[SalaVirtual v3.7] 📅 Fetching all messages (no timestamp set)');
      }

      const { data: publicData, error: publicError } = await publicQuery;

      if (publicError) {
        console.error('[SalaVirtual v3.7] ❌ Error syncing public messages:', publicError);
      } else if (publicData && publicData.length > 0) {
        console.log('[SalaVirtual v3.7] 📨 Found', publicData.length, 'NEW public messages');
        ...
        const uniqueNewMessages = newMessages.filter(msg => {
          if (messageIdsRef.current.has(msg.id)) {
            console.log('[SalaVirtual v3.7] ⏭️ Skipping duplicate message ID:', msg.id);
            return false;
          }
          return true;
        });
        ...
        if (uniqueNewMessages.length > 0) {
          console.log('[SalaVirtual v3.7] ✅ Adding', uniqueNewMessages.length, 'unique new messages to UI');
          ...
          uniqueNewMessages.forEach(msg => {
            ...
            if (msg.usuario_id === user.id) {
              ...
              console.log('[SalaVirtual v3.7] ✅ Removed pending message:', pendingId);
            }
            
            if (msg.usuario_id !== user.id && msg.tipo === 'privado') {
              console.log('[SalaVirtual v3.7] 🎬 Received private message from another user!');
              console.log('[SalaVirtual v3.7] 👤 From:', msg.usuario.nombre);
              console.log('[SalaVirtual v3.7] 💬 Content:', msg.contenido);
              triggerReceivedAnimation(msg.contenido, msg.tipo);
            }
          });
          
          console.log('[SalaVirtual v3.7] 🔑 Now tracking', messageIdsRef.current.size, 'message IDs');
          ...
          setMessages(prev => {
            ...
            console.log('[SalaVirtual v3.7] 📊 Total messages in UI after update:', updated.length);
          ...
          console.log('[SalaVirtual v3.7] 📅 Updated last timestamp to:', lastPublicMessageTimestampRef.current);
          
          setTimeout(() => {
            console.log('[SalaVirtual v3.7] 📜 Scrolling to new messages');
            flatListRef.current?.scrollToEnd({ animated: true });
          }, 100);
        } else {
          console.log('[SalaVirtual v3.7] ℹ️ No new unique messages to add');
        }
      } else {
        console.log('[SalaVirtual v3.7] ℹ️ No new public messages found');
      }
      ...
      if (privateError) {
        console.error('[SalaVirtual v3.7] ❌ Error syncing private messages:', privateError);
      } else if (privateData && privateData.length > 0) {
        console.log('[SalaVirtual v3.7] 📨 Found', privateData.length, 'new private messages');
        
        if (privateData.length > 0) {
          lastPrivateMessageTimestampRef.current = privateData[privateData.length - 1].created_at;
        }
        
        console.log('[SalaVirtual v3.7] 🔵 Reloading private chats to update unread counts');
        loadPrivateChats();
          ...
          if (relevantMessages.length > 0) {
            ...
            setPrivateChatMessages(prev => {
              ...
              if (uniqueNew.length > 0) {
                console.log('[SalaVirtual v3.7] ✅ Adding', uniqueNew.length, 'new private messages to UI');
                ...
                uniqueNew.forEach(msg => {
                  ...
                  if (msg.usuario_id !== user.id && msg.tipo === 'privado') {
                    console.log('[SalaVirtual v3.7] 🎬 Received private message!');
                    console.log('[SalaVirtual v3.7] 👤 From:', msg.usuario.nombre);
                    console.log('[SalaVirtual v3.7] 💬 Content:', msg.contenido);
                    triggerReceivedAnimation(msg.contenido, msg.tipo);
                  }
      ...
      console.error('[SalaVirtual v3.7] ❌ Error syncing messages:', error);
    }
  }, [localId, user, selectedPrivateChat, triggerReceivedAnimation]);

  useEffect(() => {
    if (!localId || !user || !isCheckedIn) {
      console.log('[SalaVirtual v3.7] ⏸️ Polling not started - missing requirements');
      return;
    }

    console.log('[SalaVirtual v3.7] 🔥 STARTING MESSAGE POLLING (every 1.5 seconds)');
    ...
    return () => {
      if (messageSyncIntervalRef.current) {
        console.log('[SalaVirtual v3.7] 🛑 Stopping message polling');
  ...
  const uniqueActiveUsers = useMemo(() => {
    ...
    console.log('[SalaVirtual v3.7] 🔍 Filtering duplicate users...');
    console.log('[SalaVirtual v3.7] 📊 Total users before filtering:', activeUsers.length);
    
    const seenUserIds = new Set<string>();
    const uniqueUsers = activeUsers.filter(user => {
      if (seenUserIds.has(user.id)) {
        console.log('[SalaVirtual v3.7] ⚠️ Duplicate user found and removed:', user.nombre, user.username);
        return false;
      }
    ...
    console.log('[SalaVirtual v3.7] ✅ Unique users after filtering:', uniqueUsers.length);
    console.log('[SalaVirtual v3.7] 📋 User IDs:', uniqueUsers.map(u => u.username || u.nombre).join(', '));
  ...
  const updateActiveUsers = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 🔄 Updating active users list...');
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error loading active users:', error);
        return;
      }
      ...
      console.log('[SalaVirtual v3.7] 👥 Found', users.length, 'active users (before deduplication)');
      ...
      if (user) {
        ...
        console.log('[SalaVirtual v3.7] ✅ Current user moved to first position');
      }
  ...
  const markPrivateMessagesAsRead = useCallback(async (partnerId: string) => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 🔵 Marking private messages as read from:', partnerId);
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error marking messages as read:', error);
      } else {
        console.log('[SalaVirtual v3.7] ✅ Messages marked as read in database');
      }
      
      console.log('[SalaVirtual v3.7] 🔵 Saving read status to AsyncStorage for:', partnerId);
      await saveReadMessagesToStorage(localId, user.id, partnerId);
      
      console.log('[SalaVirtual v3.7] 🔵 Updating unread counter IMMEDIATELY in frontend state');
      setPrivateChats(prev => 
        prev.map(chat => {
          if (chat.userId === partnerId) {
            console.log('[SalaVirtual v3.7] 🔵 Setting unreadCount to 0 for user:', partnerId);
            return { ...chat, unreadCount: 0 };
          }
          return chat;
        })
      );
      console.log('[SalaVirtual v3.7] ✅ Unread counter set to 0 for user:', partnerId);
    } catch (error) {
      console.error('[SalaVirtual v3.7] ❌ Error:', error);
    }
  ...
  const loadPrivateChats = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 🔄 Loading private chats...');
      
      const readPartners = await loadReadMessagesFromStorage(localId, user.id);
      console.log('[SalaVirtual v3.7] 🔵 Read partners from storage:', Array.from(readPartners));
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error loading private chats:', error);
        return;
      }
      ...
      console.log('[SalaVirtual v3.7] 🔵 Counting ONLY unread messages WHERE leido = false AND recipient_id = user.id');
      ...
      (privateMessages || []).forEach(msg => {
        ...
        if (msg.recipient_id === user.id && msg.usuario_id !== user.id && msg.leido === false) {
          if (!readPartners.has(partnerId)) {
            ...
            console.log('[SalaVirtual v3.7] 🔵 Unread message from', partnerId, '- count:', currentCount + 1);
          } else {
            console.log('[SalaVirtual v3.7] 🔵 Skipping count for', partnerId, '- already marked as read in storage');
          }
        }
      ...
      console.log('[SalaVirtual v3.7] ✅ Private chats loaded:', chats.length);
      console.log('[SalaVirtual v3.7] 🔵 Total unread messages (respecting storage):', 
  ...
  const handleTypingStart = useCallback(() => {
    ...
    console.log('[SalaVirtual v3.7] ⌨️ User started typing to:', selectedPrivateChat.userId);
  ...
  const handleTypingStop = useCallback(() => {
    ...
    console.log('[SalaVirtual v3.7] ⌨️ User stopped typing to:', selectedPrivateChat.userId);
  ...
  const subscribeToTypingEvents = useCallback(() => {
    ...
    const typingChannel = supabase
      ...
      .on('broadcast', { event: 'typing_stop' }, (payload: any) => {
        console.log('[SalaVirtual v3.7] ⌨️ Received typing_stop:', payload);
        
        if (payload.payload.recipientId === user.id) {
          console.log('[SalaVirtual v3.7] ⌨️ Partner stopped typing');
          setTypingUsers(prev => {
            const newSet = new Set(prev);
      ...
      .subscribe((status) => {
        console.log('[SalaVirtual v3.7] ⌨️ Typing channel status:', status);
      });

    typingChannelRef.current = typingChannel;

    return () => {
      console.log('[SalaVirtual v3.7] 🔌 Unsubscribing from typing channel');
  ...
  const subscribeToUpdates = useCallback(() => {
    ...
    console.log('[SalaVirtual v3.7] 📡 Setting up real-time subscriptions...');
    ...
    const chatChannel = supabase
      ...
      .on(
        ...
        async (payload) => {
          console.log('[SalaVirtual v3.7] 📨 Real-time INSERT event received:', payload);
          
          const newRecord = payload.new as any;
          
          if (newRecord.usuario_id === user.id) {
            console.log('[SalaVirtual v3.7] ⏭️ Skipping own message (already in UI optimistically)');
            return;
          }

          console.log('[SalaVirtual v3.7] 🔄 Triggering immediate sync for new message from other user');
          syncMessages();
        }
      )
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual v3.7] 🗑️ Message deleted from DB:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual v3.7] 🔄 Message updated in DB:', payload);
          
          const updatedRecord = payload.new as any;
          
          if (updatedRecord.tipo === 'privado' && updatedRecord.leido === true) {
            console.log('[SalaVirtual v3.7] 🔵 Message marked as read via real-time, reloading chats');
            setTimeout(() => {
              loadPrivateChats();
            }, 300);
          }
      ...
      .subscribe((status) => {
        console.log('[SalaVirtual v3.7] 📡 Chat channel status:', status);
        
        if (status === 'SUBSCRIBED') {
          console.log('[SalaVirtual v3.7] ✅ Real-time subscription active');
        } else if (status === 'CHANNEL_ERROR') {
          console.warn('[SalaVirtual v3.7] ⚠️ Real-time subscription error - polling will handle all sync');
        } else if (status === 'TIMED_OUT') {
          console.warn('[SalaVirtual v3.7] ⏱️ Real-time subscription timed out - polling will handle all sync');
        }
      });
    ...
    const checkinsChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual v3.7] 👤 Check-in event:', payload.eventType);
          updateActiveUsers();
        }
      )
      .subscribe((status) => {
        console.log('[SalaVirtual v3.7] 📡 Checkins channel status:', status);
      });
    ...
    return () => {
      console.log('[SalaVirtual v3.7] 🔌 Unsubscribing from channels');
  ...
  useEffect(() => {
    ...
    console.log('[SalaVirtual v3.7] 🚀 INITIALIZING VIRTUAL ROOM');

    const init = async () => {
      console.log('[SalaVirtual v3.7] 1️⃣ Loading local data...');
      await loadLocalData();
      
      console.log('[SalaVirtual v3.7] 2️⃣ Checking user check-in status...');
      const checkedIn = await checkUserCheckin();
      
      if (!checkedIn && !localClosed && user) {
        console.log('[SalaVirtual v3.7] 3️⃣ User not checked in, checking in now...');
        const success = await handleCheckIn();
        if (!success) {
          console.error('[SalaVirtual v3.7] ❌ Check-in failed, aborting initialization');
          return;
        }
      }
      ...
      console.log('[SalaVirtual v3.7] 4️⃣ Loading initial messages...');
      await loadMessages();
      
      console.log('[SalaVirtual v3.7] 5️⃣ Setting up real-time subscriptions...');
      subscribeToUpdates();
      
      console.log('[SalaVirtual v3.7] 6️⃣ Setting up typing events...');
      subscribeToTypingEvents();
      
      console.log('[SalaVirtual v3.7] 7️⃣ Updating active users...');
      await updateActiveUsers();
      
      console.log('[SalaVirtual v3.7] 8️⃣ Loading private chats...');
      await loadPrivateChats();
      
      console.log('[SalaVirtual v3.7] ✅ INITIALIZATION COMPLETE');
    };

    init();

    return () => {
      console.log('[SalaVirtual v3.7] 🧹 Cleanup: Checking out user');
      handleCheckOut();
    };
  }, [localId]);
  ...
  useEffect(() => {
    if (activeTab === 'private' && user && localId) {
      console.log('[SalaVirtual v3.7] 🔄 Private tab active, reloading chats');
      loadPrivateChats();
    }
  ...
  useEffect(() => {
    if (selectedPrivateChat && user && localId) {
      console.log('[SalaVirtual v3.7] 🔄 selectedPrivateChat changed, syncing state...');
      console.log('[SalaVirtual v3.7] 🔄 Partner ID:', selectedPrivateChat.userId);
      
      const syncProfile = async () => {
        console.log('[SalaVirtual v3.7] 🔄 Fetching fresh profile for partner...');
        const profile = await fetchUserProfile(selectedPrivateChat.userId);
        
        if (profile) {
          console.log('[SalaVirtual v3.7] ✅ Profile fetched and updated');
          console.log('[SalaVirtual v3.7] 🖼️ Avatar:', profile.avatar || 'NO AVATAR');
      ...
      console.log('[SalaVirtual v3.7] 🔄 Marking messages as read...');
  ...
  const sendPublicMessage = useCallback(async (content: string) => {
    if (!user || !localId || !content.trim()) {
      console.log('[SalaVirtual v3.7] ⚠️ Cannot send message - missing requirements');
      return;
    }

    try {
      console.log('[SalaVirtual v3.7] 📤 Sending public message:', content);
      setSending(true);
      ...
      console.log('[SalaVirtual v3.7] 🔥 Fetching user profile from database...');
      ...
      console.log('[SalaVirtual v3.7] 🔥 Using profile data from database');
      console.log('[SalaVirtual v3.7] 👤 User profile:', currentUserProfile.nombre);
      console.log('[SalaVirtual v3.7] 🖼️ Avatar URL:', currentUserProfile.avatar || 'NO AVATAR');
      ...
      console.log('[SalaVirtual v3.7] ✨ Adding message optimistically with avatar from database');
      console.log('[SalaVirtual v3.7] 👤 User:', optimisticMsg.usuario.nombre, '| Avatar:', !!optimisticMsg.usuario.avatar);
      ...
      console.log('[SalaVirtual v3.7] 💾 Saving message to database...');
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error saving message to DB:', error);
        
        messageIdsRef.current.delete(messageId);
        setMessages(prev => prev.filter(m => m.id !== messageId));
        pendingMessageIds.current.delete(pendingId);
      } else {
        console.log('[SalaVirtual v3.7] ✅ Public message saved to DB with id:', insertedMessage.id);
        ...
        setMessages(prev => {
          ...
          if (withoutOptimistic.some(m => m.id === realMessage.id)) {
            console.log('[SalaVirtual v3.7] ℹ️ Real message already in UI from polling');
  ...
  const triggerFloatingReaction = useCallback((emoji: string) => {
    ...
    console.log(`[SalaVirtual v3.7] 🎉 Floating reaction triggered: ${emoji}`);
  }, []);
  ...
  const sendPredefinedMessage = useCallback(async (recipientId: string, messageText: string) => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 📤 Sending predefined message:', messageText, 'to:', recipientId);
      ...
      console.log('[SalaVirtual v3.7] 🔥 Fetching user profile from database for predefined message...');
      const userProfile = await fetchUserProfile(user.id);
      ...
      if (existingChatIndex === -1) {
        ...
        console.log('[SalaVirtual v3.7] ✨ Creating new private chat optimistically');
        setPrivateChats(prev => [newChat, ...prev]);
      } else {
        console.log('[SalaVirtual v3.7] ✨ Updating existing private chat optimistically');
        setPrivateChats(prev => {
          const updatedChat = {
      ...
      console.log('[SalaVirtual v3.7] 💾 Saving with tipo = "privado"');
      ...
      if (insertError) {
        console.error('[SalaVirtual v3.7] ❌ Error saving private message to DB:', insertError);
      } else {
        console.log('[SalaVirtual v3.7] ✅ Private message saved to database with tipo = "privado"');
      ...
      console.log(`[SalaVirtual v3.7] ✅ Message sent to ${recipientName}`);
      ...
      console.log('[SalaVirtual v3.7] 🔄 Switching to private conversations tab');
      ...
      console.error('[SalaVirtual v3.7] ❌ Error sending predefined message:', error);
  ...
  const closeBottomSheet = useCallback(() => {
    console.log('[SalaVirtual v3.7] 📋 Closing bottom sheet');
  ...
  const sendPrivateMessage = useCallback(async (recipientId: string, content: string) => {
    ...
    try {
      console.log('[SalaVirtual v3.7] 📤 Sending private message to:', recipientId);
      ...
      console.log('[SalaVirtual v3.7] 🔥 Fetching user profile from database for private message...');
      const userProfile = await fetchUserProfile(user.id);
      ...
      console.log('[SalaVirtual v3.7] 🔥 Using profile data from database for private message');
      ...
      console.log('[SalaVirtual v3.7] ✨ Adding private message optimistically to UI');
      ...
      if (insertError) {
        ...
        console.log('[SalaVirtual v3.7] ✅ Private message saved to database with id:', insertedMessage.id);
      ...
      console.log('[SalaVirtual v3.7] ✅ Private message sent');
    } catch (error) {
      console.error('[SalaVirtual v3.7] ❌ Error sending private message:', error);
    }
  ...
  const openPrivateChat = useCallback(async (chat: PrivateChat) => {
    ...
    try {
      ...
      console.log('[SalaVirtual v3.7] 💬 Opening private chat with:', displayName);
      console.log('[SalaVirtual v3.7] 🔵 Unread count before:', chat.unreadCount);
      
      setSelectedPrivateChat(chat);
      
      console.log('[SalaVirtual v3.7] 🔵 Marking messages as read in database...');
      await markPrivateMessagesAsRead(chat.userId);
      
      console.log('[SalaVirtual v3.7] 🔵 Unread count set to 0 in frontend');
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error loading private messages:', error);
        return;
      }
      ...
      console.log('[SalaVirtual v3.7] ✅ Private messages loaded:', formattedMessages.length);
      ...
      console.error('[SalaVirtual v3.7] ❌ Error opening private chat:', error);
    }
  }, [user, localId, uniqueActiveUsers, markPrivateMessagesAsRead]);

  const closePrivateChat = useCallback(() => {
    console.log('[SalaVirtual v3.7] 💬 Closing private chat');
  ...
  const handleDeleteMessage = useCallback(async (message: Message) => {
    if (!user || message.usuario_id !== user.id) {
      console.log('[SalaVirtual v3.7] ⚠️ Cannot delete message - not owner');
      return;
    }

    console.log('[SalaVirtual v3.7] 🗑️ Showing delete confirmation for message:', message.id);
  ...
  const confirmDeleteMessage = useCallback(async () => {
    ...
    try {
      setDeleting(true);
      console.log('[SalaVirtual v3.7] 🗑️ Deleting message:', messageToDelete.id);
      ...
      if (error) {
        console.error('[SalaVirtual v3.7] ❌ Error deleting message:', error);
        ...
        console.log('[SalaVirtual v3.7] ✅ Message deleted successfully');
  ...
  const cancelDeleteMessage = useCallback(() => {
    console.log('[SalaVirtual v3.7] ❌ Delete cancelled');
    setShowDeleteModal(false);
    setMessageToDelete(null);
  }, []);
  ...
  const handleUserPress = async (selectedUser: ActiveUser) => {
    ...
    console.log('[SalaVirtual v3.7] 👤 User pressed:', displayName);
    
    if (selectedUser.id === user?.id) {
      console.log('[SalaVirtual v3.7] ⚠️ Cannot interact with self');
      return;
    }
    
    console.log('[SalaVirtual v3.7] 🔥 Fetching user profile for bottom sheet cover...');
    const profile = await fetchUserProfile(selectedUser.id);
    
    if (profile) {
      console.log('[SalaVirtual v3.7] ✅ Profile fetched for bottom sheet');
      console.log('[SalaVirtual v3.7] 🖼️ Cover photo (avatar):', profile.avatar || 'NO AVATAR');
      setSelectedUserProfile(profile);
    } else {
      console.log('[SalaVirtual v3.7] ⚠️ Could not fetch profile, using cached data');
    ...
    console.log('[SalaVirtual v3.7] 📋 Opening bottom sheet for user');
  ...
  const handleViewProfile = useCallback(async () => {
    if (!selectedUser) {
      console.log('[SalaVirtual v3.7] ⚠️ No selected user');
      return;
    }
    
    console.log('[SalaVirtual v3.7] 🚀 Starting profile navigation');
    console.log('[SalaVirtual v3.7] 👤 Target user ID:', selectedUser.id);
    console.log('[SalaVirtual v3.7] 🎯 Current active tab:', activeTab);
    console.log('[SalaVirtual v3.7] 🏠 Current local ID:', localId);
    
    console.log('[SalaVirtual v3.7] 📋 Step 1 - Closing bottom sheet...');
    closeBottomSheet();
    setSelectedPrivateChat(null);
    
    console.log('[SalaVirtual v3.7] ⏳ Waiting for bottom sheet animation (300ms)...');
    await new Promise(resolve => setTimeout(resolve, 300));
    console.log('[SalaVirtual v3.7] ✅ Bottom sheet closed');
    
    console.log('[SalaVirtual v3.7] 🎯 Step 2 - Navigating to profile...');
    ...
    try {
      ...
      console.log('[SalaVirtual v3.7] ✅ Navigation executed successfully');
    } catch (error) {
      console.error('[SalaVirtual v3.7] ❌ Error navigating:', error);
    }
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onLongPress={() => {
          if (isOwnMessage) {
            console.log('[SalaVirtual v3.7] 🗑️ Long press on own message');
      ...
      >
        {!isOwnMessage && (
          <TouchableOpacity
            ...
            onPress={() => {
              console.log('[SalaVirtual v3.7] 👤 Avatar clicked');
        ...
        {isOwnMessage && (
          <TouchableOpacity
            style={[styles.messageAvatar, { width: avatarSize, height: avatarSize }]}
            onPress={() => {
              console.log('[SalaVirtual v3.7] 👤 Navigating to own profile');
              router.push('/perfil');
            }}
          >
            {item.usuario.avatar ? (
  ...
  const renderUserItem = ({ item, index }: { item: ActiveUser; index: number }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          console.log('[SalaVirtual v3.7] 👤 Grid user card pressed:', displayName);
          if (isCurrentUser) {
            console.log('[SalaVirtual v3.7] 👤 Navigating to own profile');
            router.push('/perfil');
          } else {
  ...
  // ✅ FIX v3.7: Dynamic padding for chat list based on keyboard visibility
  const chatListPaddingBottom = useMemo(() => {
    if (Platform.OS === 'ios') {
      return isKeyboardVisible ? keyboardHeight : insets.bottom + 80;
    } else { // Android
      return isKeyboardVisible ? 20 : Math.max(insets.bottom + 160, 180);
    }
  }, [isKeyboardVisible, keyboardHeight, insets.bottom]);

  console.log('[SalaVirtual v3.7] 🎹 Chat list padding bottom:', chatListPaddingBottom);
  console.log('[SalaVirtual v3.7] 🎹 Keyboard visible:', isKeyboardVisible);
  console.log('[SalaVirtual v3.7] 🎹 Keyboard height:', keyboardHeight);
  ...
  if (loading) {
    console.log('[SalaVirtual v3.7] ⏳ Showing loading state');
    return (
      <LinearGradient
      ...
      >
        <Stack.Screen
          options={{
            ...
            headerTitleAlign: 'center',
            // ✅ FIX v3.7: Redesigned header left icon with circular container
            headerLeft: () => (
              <TouchableOpacity 
            ...
            // ✅ FIX v3.7: Redesigned header right icons with circular containers
            headerRight: () => (
          ...
          <View style={[
            ...
            <View style={styles.tabBar}>
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual v3.7] 🔄 Switching to Chat tab');
                  setActiveTab('chat');
                  if (selectedPrivateChat) {
                    closePrivateChat();
                  }
              ...
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual v3.7] 🔄 Switching to Users tab');
                  setActiveTab('users');
                  if (selectedPrivateChat) {
                    closePrivateChat();
                  }
              ...
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual v3.7] 🔄 Switching to private tab');
                  setActiveTab('private');
                  loadPrivateChats();
                }}
                activeOpacity={0.7}
          ...
          {activeTab === 'chat' ? (
            <React.Fragment>
              <FlatList
                ...
                onContentSizeChange={() => {
                  console.log('[SalaVirtual v3.7] 🎹 Content size changed, scrolling to bottom');
                  flatListRef.current?.scrollToEnd({ animated: true });
                }}
                ListEmptyComponent={
              ...
              <View style={[
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    console.log('[SalaVirtual v3.7] ⚡ Toggling quick messages:', !showQuickMessages);
              ...
              {selectedPrivateChat ? (
                <React.Fragment>
                  <View style={[
                    ...
                    <TouchableOpacity
                      onPress={async () => {
                        console.log('[SalaVirtual v3.7] 🚀 Profile button pressed in private chat header');
                        ...
                        console.log('[SalaVirtual v3.7] 🎯 Executing router.push() to profile');
                        try {
                          router.push({
                            pathname: '/perfil/usuario',
                            params: {
                          ...
                          console.log('[SalaVirtual v3.7] ✅ Navigation complete');
                        } catch (error) {
                          console.error('[SalaVirtual v3.7] ❌ Error navigating:', error);
                        }
                      }}
                  ...
                  <FlatList
                    ...
                    onContentSizeChange={() => {
                      console.log('[SalaVirtual v3.7] 🎹 Private chat content size changed, scrolling to bottom');
                      privateChatListRef.current?.scrollToEnd({ animated: true });
                    }}
                    onLayout={() => {
...
const styles = StyleSheet.create({
  // ✅ FIX v3.7: KeyboardAvoidingView container
  ...
  // ✅ FIX v3.7: Redesigned header icon containers with circular backgrounds
  headerIconContainer: {
