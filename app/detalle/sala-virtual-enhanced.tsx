...
// ✅ SALA VIRTUAL v5.8 - ANDROID INPUT POSITIONING FIX v2
console.log("✅ SALA VIRTUAL v5.8 - ANDROID INPUT POSITIONING FIX v2");
 ...
 * CAMBIOS v5.8:
 * 
 * 1. ANDROID INPUT POSITIONING FIX v2:
 *    - ✅ FIXED: Text input now positioned with 16px margin above system buttons
 *    - ✅ RESULT: Input field is fully visible and accessible on Android
 *    - ✅ BEHAVIOR: When keyboard closed: 16px margin (comfortable spacing above buttons)
 *    - ✅ BEHAVIOR: When keyboard open: normal safe area padding (unchanged)
 *    - ✅ REASONING: 0px was too tight, 16px provides better visual spacing
 * 
 * Previous fixes maintained (v5.7):
...
export default function SalaVirtualEnhancedScreen() {
  ...
  console.log('[SalaVirtual v5.8] 🎯 INITIAL TAB from params:', initialTab);
  ...
  // ✅ v5.8: Listen to keyboard events
  useEffect(() => {
    console.log('[SalaVirtual v5.8] 🎹 Setting up keyboard listeners');
    
    const keyboardDidShowListener = Keyboard.addListener(
      'keyboardDidShow',
      (e) => {
        console.log('[SalaVirtual v5.8] ⬆️ Keyboard opened, height:', e.endCoordinates.height);
    ...
    const keyboardDidHideListener = Keyboard.addListener(
      'keyboardDidHide',
      () => {
        console.log('[SalaVirtual v5.8] ⬇️ Keyboard closed');
        setKeyboardHeight(0);
      }
    );

    return () => {
      console.log('[SalaVirtual v5.8] 🧹 Removing keyboard listeners');
  ...
  const loadReadMessagesFromStorage = useCallback(async (localId: string, userId: string): Promise<Set<string>> => {
    try {
      ...
      if (stored) {
        ...
        console.log('[SalaVirtual v5.8] 🔵 Loaded read partners from storage:', readPartners);
        return new Set(readPartners);
      }
      
      console.log('[SalaVirtual v5.8] 🔵 No stored read partners found');
      return new Set();
    } catch (error) {
      console.error('[SalaVirtual v5.8] ❌ Error loading from storage:', error);
      return new Set();
    }
  }, [getReadMessagesKey]);
  ...
  const saveReadMessagesToStorage = useCallback(async (localId: string, userId: string, partnerId: string) => {
    try {
      ...
      if (!readPartners.includes(partnerId)) {
        ...
        console.log('[SalaVirtual v5.8] 🔵 Saved read status for partner:', partnerId);
      }
    } catch (error) {
      console.error('[SalaVirtual v5.8] ❌ Error saving to storage:', error);
    }
  }, [getReadMessagesKey]);

  const fetchUserProfile = useCallback(async (userId: string): Promise<UserProfile | null> => {
    try {
      console.log('[SalaVirtual v5.8] 🔍 Fetching user profile from database for userId:', userId);
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error fetching user profile:', error);
        return null;
      }

      console.log('[SalaVirtual v5.8] ✅ User profile fetched successfully');
      console.log('[SalaVirtual v5.8] 👤 Name:', data.nombre);
      console.log('[SalaVirtual v5.8] 🖼️ Avatar:', data.avatar || 'NO AVATAR');
      console.log('[SalaVirtual v5.8] 📝 Username:', data.username || 'NO USERNAME');
      ...
      console.error('[SalaVirtual v5.8] ❌ Error in fetchUserProfile:', error);
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      console.log('[SalaVirtual v5.8] 🌓 Checking day/night mode:', newMode);
      setMode(newMode);
    }, 60000);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status === 'granted') {
        ...
        console.log('[SalaVirtual v5.8] ✅ User location obtained');
      }
    })();
  }, []);
  ...
  const checkClosingTime = useCallback(() => {
    ...
    if (estadoLocal.estaAbierto && estadoLocal.tiempoRestante) {
      ...
      console.log('[SalaVirtual v5.8] ⏰ Time until closing:', totalMinutes, 'minutes');
  ...
  const loadLocalData = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual v5.8] ❌ No localId provided');
      setLoading(false);
      return;
    }

    try {
      console.log('[SalaVirtual v5.8] 🏠 Loading local data for:', localId);
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error loading local:', error);
        setLoading(false);
        return;
      }

      console.log('[SalaVirtual v5.8] ✅ Local loaded:', data.nombre);
      setLocal(data);
      ...
      if (!isOpen) {
        console.log('[SalaVirtual v5.8] 🔒 Local is closed');
        setLocalClosed(true);
        setLoading(false);
      } else {
        console.log('[SalaVirtual v5.8] ✅ Local is open');
        setLocalClosed(false);
      }
    } catch (error) {
      console.error('[SalaVirtual v5.8] ❌ Error:', error);
      setLoading(false);
    }
  }, [localId]);
  ...
  const checkUserCheckin = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v5.8] 🔍 Checking if user is checked in...');
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('[SalaVirtual v5.8] ❌ Error checking checkin:', error);
        return false;
      }

      const checkedIn = !!data;
      console.log('[SalaVirtual v5.8] ✅ User checked in status:', checkedIn);
  ...
  const handleCheckIn = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v5.8] 🚪 User checking in...');
      setCheckingIn(true);
      setIsCheckedIn(true);
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error inserting checkin:', error);
        setIsCheckedIn(false);
        throw new Error('No se pudo entrar en la sala');
      }

      console.log('[SalaVirtual v5.8] ✅ User checked in successfully');
      
      setCheckingIn(false);
      return true;
    } catch (error: any) {
      console.error('[SalaVirtual v5.8] ❌ Error during checkin:', error);
      setCheckingIn(false);
      return false;
    }
  }, [user, localId]);
  ...
  /**
   * ✅ FIX v5.6: NAVIGATION LOOP FIX
   * 
   * Changed from router.push() to router.replace() to remove virtual room from stack.
   * This prevents the navigation loop where:
   * - Virtual Room → Close → Local Details → Close → Virtual Room (WRONG)
   * 
   * Now the flow is:
   * - Virtual Room → Close (replace) → Local Details → Close → Previous screen (CORRECT)
   */
  const handleCheckOut = useCallback(async () => {
    if (!user || !localId) return;

    try {
      console.log('[SalaVirtual v5.8] 🚪 User checking out, navigating to local details...');
      console.log('[SalaVirtual v5.8] 🔧 FIX v5.6: Using router.replace() to remove virtual room from stack');
      ...
      console.log('[SalaVirtual v5.8] ✅ User checked out successfully');

      // ✅ FIX v5.6: Use replace instead of push to remove virtual room from navigation stack
      router.replace({
        pathname: '/detalle/local',
        params: { id: localId }
      });
      
      console.log('[SalaVirtual v5.8] ✅ Navigation executed with replace() - virtual room removed from stack');
    } catch (error) {
      console.error('[SalaVirtual v5.8] ❌ Error checking out:', error);
    }
  }, [user, localId, router]);

  const loadMessages = useCallback(async () => {
    if (!localId) {
      console.error('[SalaVirtual v5.8] ❌ No localId for loadMessages');
      return;
    }
    
    try {
      console.log('[SalaVirtual v5.8] 🔥 LOADING INITIAL MESSAGES');
      console.log('[SalaVirtual v5.8] 📍 Local ID:', localId);
      ...
      console.log('[SalaVirtual v5.8] 📦 RAW DATA from Supabase:', JSON.stringify(data, null, 2));
      console.log('[SalaVirtual v5.8] ❌ RAW ERROR from Supabase:', JSON.stringify(error, null, 2));
      
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error loading initial messages:', error);
        setLoading(false);
        return;
      }

      if (!data) {
        console.log('[SalaVirtual v5.8] ⚠️ Data is null, setting empty array');
        setMessages([]);
        setLoading(false);
        return;
      }
      ...
      if (data.length === 0) {
        console.log('[SalaVirtual v5.8] ⚠️ No messages found');
        setMessages([]);
        setLoading(false);
        return;
      }

      console.log('[SalaVirtual v5.8] ✅ DATOS RECIBIDOS CORRECTAMENTE');
      console.log('[SalaVirtual v5.8] 📦 Número de mensajes:', data.length);

      const formattedMessages: Message[] = data
        .filter(msg => {
          if (!msg.usuario) {
            console.warn('[SalaVirtual v5.8] ⚠️ Message without user data:', msg.id);
            return false;
          }
          return true;
        })
      ...
      console.log('[SalaVirtual v5.8] ✅ Formatted', formattedMessages.length, 'public messages');
      ...
      console.log('[SalaVirtual v5.8] 🔑 Tracking', messageIdsRef.current.size, 'message IDs');

      setMessages(formattedMessages);
      
      if (formattedMessages.length > 0) {
        lastPublicMessageTimestampRef.current = formattedMessages[formattedMessages.length - 1].created_at;
        console.log('[SalaVirtual v5.8] 📅 Last public message timestamp:', lastPublicMessageTimestampRef.current);
      }
      
      setLoading(false);
      
      setTimeout(() => {
        console.log('[SalaVirtual v5.8] 📜 Scrolling to bottom');
        flatListRef.current?.scrollToEnd({ animated: false });
      }, 300);
    } catch (error) {
      console.error('[SalaVirtual v5.8] ❌ Error loading messages:', error);
      setLoading(false);
    }
  }, [localId]);

  const triggerReceivedAnimation = useCallback((messageText: string, tipo: string) => {
    console.log('[SalaVirtual v5.8] 🎬 Triggering received animation for tipo:', tipo);
    
    if (tipo !== 'privado') {
      console.log('[SalaVirtual v5.8] ⏭️ Not a private message, skipping animation');
      return;
    }
    ...
    console.log('[SalaVirtual v5.8] 🎬 Showing received animation with emoji:', emoji);
  ...
  const syncMessages = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v5.8] 🔄 Syncing messages...');
      ...
      if (lastPublicMessageTimestampRef.current) {
        ...
        console.log('[SalaVirtual v5.8] 📅 Fetching messages after:', lastPublicMessageTimestampRef.current);
      } else {
        console.log('[SalaVirtual v5.8] 📅 Fetching all messages (no timestamp set)');
      }

      const { data: publicData, error: publicError } = await publicQuery;

      if (publicError) {
        console.error('[SalaVirtual v5.8] ❌ Error syncing public messages:', publicError);
      } else if (publicData && publicData.length > 0) {
        console.log('[SalaVirtual v5.8] 📨 Found', publicData.length, 'NEW public messages');
        ...
        const uniqueNewMessages = newMessages.filter(msg => {
          if (messageIdsRef.current.has(msg.id)) {
            console.log('[SalaVirtual v5.8] ⏭️ Skipping duplicate message ID:', msg.id);
            return false;
          }
          return true;
        });
        ...
        if (uniqueNewMessages.length > 0) {
          console.log('[SalaVirtual v5.8] ✅ Adding', uniqueNewMessages.length, 'unique new messages to UI');
          ...
          uniqueNewMessages.forEach(msg => {
            ...
            if (msg.usuario_id === user.id) {
              ...
              console.log('[SalaVirtual v5.8] ✅ Removed pending message:', pendingId);
            }
            
            if (msg.usuario_id !== user.id && msg.tipo === 'privado') {
              console.log('[SalaVirtual v5.8] 🎬 Received private message from another user!');
              console.log('[SalaVirtual v5.8] 👤 From:', msg.usuario.nombre);
              console.log('[SalaVirtual v5.8] 💬 Content:', msg.contenido);
              triggerReceivedAnimation(msg.contenido, msg.tipo);
            }
          });
          
          console.log('[SalaVirtual v5.8] 🔑 Now tracking', messageIdsRef.current.size, 'message IDs');
          ...
          setMessages(prev => {
            ...
            console.log('[SalaVirtual v5.8] 📊 Total messages in UI after update:', updated.length);
          ...
          console.log('[SalaVirtual v5.8] 📅 Updated last timestamp to:', lastPublicMessageTimestampRef.current);
          
          setTimeout(() => {
            console.log('[SalaVirtual v5.8] 📜 Scrolling to new messages');
            flatListRef.current?.scrollToEnd({ animated: true });
          }, 100);
        } else {
          console.log('[SalaVirtual v5.8] ℹ️ No new unique messages to add');
        }
      } else {
        console.log('[SalaVirtual v5.8] ℹ️ No new public messages found');
      }
      ...
      if (privateError) {
        console.error('[SalaVirtual v5.8] ❌ Error syncing private messages:', privateError);
      } else if (privateData && privateData.length > 0) {
        console.log('[SalaVirtual v5.8] 📨 Found', privateData.length, 'new private messages');
        
        if (privateData.length > 0) {
          lastPrivateMessageTimestampRef.current = privateData[privateData.length - 1].created_at;
        }
        
        console.log('[SalaVirtual v5.8] 🔵 Reloading private chats to update unread counts');
        loadPrivateChats();
          ...
          if (relevantMessages.length > 0) {
            ...
            setPrivateChatMessages(prev => {
              ...
              if (uniqueNew.length > 0) {
                console.log('[SalaVirtual v5.8] ✅ Adding', uniqueNew.length, 'new private messages to UI');
                ...
                uniqueNew.forEach(msg => {
                  ...
                  if (msg.usuario_id !== user.id && msg.tipo === 'privado') {
                    console.log('[SalaVirtual v5.8] 🎬 Received private message!');
                    console.log('[SalaVirtual v5.8] 👤 From:', msg.usuario.nombre);
                    console.log('[SalaVirtual v5.8] 💬 Content:', msg.contenido);
                    triggerReceivedAnimation(msg.contenido, msg.tipo);
                  }
      ...
      console.error('[SalaVirtual v5.8] ❌ Error syncing messages:', error);
    }
  }, [localId, user, selectedPrivateChat, triggerReceivedAnimation]);

  useEffect(() => {
    if (!localId || !user || !isCheckedIn) {
      console.log('[SalaVirtual v5.8] ⏸️ Polling not started - missing requirements');
      return;
    }

    console.log('[SalaVirtual v5.8] 🔥 STARTING MESSAGE POLLING (every 1.5 seconds)');
    ...
    return () => {
      if (messageSyncIntervalRef.current) {
        console.log('[SalaVirtual v5.8] 🛑 Stopping message polling');
  ...
  const uniqueActiveUsers = useMemo(() => {
    ...
    console.log('[SalaVirtual v5.8] 🔍 Filtering duplicate users...');
    console.log('[SalaVirtual v5.8] 📊 Total users before filtering:', activeUsers.length);
    
    const seenUserIds = new Set<string>();
    const uniqueUsers = activeUsers.filter(user => {
      if (seenUserIds.has(user.id)) {
        console.log('[SalaVirtual v5.8] ⚠️ Duplicate user found and removed:', user.nombre, user.username);
        return false;
      }
    ...
    console.log('[SalaVirtual v5.8] ✅ Unique users after filtering:', uniqueUsers.length);
    console.log('[SalaVirtual v5.8] 📋 User IDs:', uniqueUsers.map(u => u.username || u.nombre).join(', '));
  ...
  const updateActiveUsers = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v5.8] 🔄 Updating active users list...');
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error loading active users:', error);
        return;
      }
      ...
      console.log('[SalaVirtual v5.8] 👥 Found', users.length, 'active users (before deduplication)');
      ...
      if (user) {
        ...
        console.log('[SalaVirtual v5.8] ✅ Current user moved to first position');
      }
  ...
  const markPrivateMessagesAsRead = useCallback(async (partnerId: string) => {
    ...
    try {
      console.log('[SalaVirtual v5.8] 🔵 Marking private messages as read from:', partnerId);
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error marking messages as read:', error);
      } else {
        console.log('[SalaVirtual v5.8] ✅ Messages marked as read in database');
      }
      
      console.log('[SalaVirtual v5.8] 🔵 Saving read status to AsyncStorage for:', partnerId);
      await saveReadMessagesToStorage(localId, user.id, partnerId);
      
      console.log('[SalaVirtual v5.8] 🔵 Updating unread counter IMMEDIATELY in frontend state');
      setPrivateChats(prev => 
        prev.map(chat => {
          if (chat.userId === partnerId) {
            console.log('[SalaVirtual v5.8] 🔵 Setting unreadCount to 0 for user:', partnerId);
            return { ...chat, unreadCount: 0 };
          }
          return chat;
        })
      );
      console.log('[SalaVirtual v5.8] ✅ Unread counter set to 0 for user:', partnerId);
    } catch (error) {
      console.error('[SalaVirtual v5.8] ❌ Error:', error);
    }
  ...
  const loadPrivateChats = useCallback(async () => {
    ...
    try {
      console.log('[SalaVirtual v5.8] 🔄 Loading private chats...');
      
      const readPartners = await loadReadMessagesFromStorage(localId, user.id);
      console.log('[SalaVirtual v5.8] 🔵 Read partners from storage:', Array.from(readPartners));
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error loading private chats:', error);
        return;
      }
      ...
      console.log('[SalaVirtual v5.8] 🔵 Counting ONLY unread messages WHERE leido = false AND recipient_id = user.id');
      ...
      (privateMessages || []).forEach(msg => {
        ...
        if (msg.recipient_id === user.id && msg.usuario_id !== user.id && msg.leido === false) {
          if (!readPartners.has(partnerId)) {
            ...
            console.log('[SalaVirtual v5.8] 🔵 Unread message from', partnerId, '- count:', currentCount + 1);
          } else {
            console.log('[SalaVirtual v5.8] 🔵 Skipping count for', partnerId, '- already marked as read in storage');
          }
        }
      ...
      console.log('[SalaVirtual v5.8] ✅ Private chats loaded:', chats.length);
      console.log('[SalaVirtual v5.8] 🔵 Total unread messages (respecting storage):', 
  ...
  const handleTypingStart = useCallback(() => {
    ...
    console.log('[SalaVirtual v5.8] ⌨️ User started typing to:', selectedPrivateChat.userId);
  ...
  const handleTypingStop = useCallback(() => {
    ...
    console.log('[SalaVirtual v5.8] ⌨️ User stopped typing to:', selectedPrivateChat.userId);
  ...
  const subscribeToTypingEvents = useCallback(() => {
    ...
    const typingChannel = supabase
      ...
      .on('broadcast', { event: 'typing_stop' }, (payload: any) => {
        console.log('[SalaVirtual v5.8] ⌨️ Received typing_stop:', payload);
        
        if (payload.payload.recipientId === user.id) {
          console.log('[SalaVirtual v5.8] ⌨️ Partner stopped typing');
          setTypingUsers(prev => {
            const newSet = new Set(prev);
      ...
      .subscribe((status) => {
        console.log('[SalaVirtual v5.8] ⌨️ Typing channel status:', status);
      });

    typingChannelRef.current = typingChannel;

    return () => {
      console.log('[SalaVirtual v5.8] 🔌 Unsubscribing from typing channel');
  ...
  const subscribeToUpdates = useCallback(() => {
    ...
    console.log('[SalaVirtual v5.8] 📡 Setting up real-time subscriptions...');
    ...
    const chatChannel = supabase
      ...
      .on(
        ...
        async (payload) => {
          console.log('[SalaVirtual v5.8] 📨 Real-time INSERT event received:', payload);
          
          const newRecord = payload.new as any;
          
          if (newRecord.usuario_id === user.id) {
            console.log('[SalaVirtual v5.8] ⏭️ Skipping own message (already in UI optimistically)');
            return;
          }

          console.log('[SalaVirtual v5.8] 🔄 Triggering immediate sync for new message from other user');
          syncMessages();
        }
      )
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual v5.8] 🗑️ Message deleted from DB:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual v5.8] 🔄 Message updated in DB:', payload);
          
          const updatedRecord = payload.new as any;
          
          if (updatedRecord.tipo === 'privado' && updatedRecord.leido === true) {
            console.log('[SalaVirtual v5.8] 🔵 Message marked as read via real-time, reloading chats');
            setTimeout(() => {
              loadPrivateChats();
            }, 300);
          }
      ...
      .subscribe((status) => {
        console.log('[SalaVirtual v5.8] 📡 Chat channel status:', status);
        
        if (status === 'SUBSCRIBED') {
          console.log('[SalaVirtual v5.8] ✅ Real-time subscription active');
        } else if (status === 'CHANNEL_ERROR') {
          console.warn('[SalaVirtual v5.8] ⚠️ Real-time subscription error - polling will handle all sync');
        } else if (status === 'TIMED_OUT') {
          console.warn('[SalaVirtual v5.8] ⏱️ Real-time subscription timed out - polling will handle all sync');
        }
      });
    ...
    const checkinsChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('[SalaVirtual v5.8] 👤 Check-in event:', payload.eventType);
          updateActiveUsers();
        }
      )
      .subscribe((status) => {
        console.log('[SalaVirtual v5.8] 📡 Checkins channel status:', status);
      });
    ...
    return () => {
      console.log('[SalaVirtual v5.8] 🔌 Unsubscribing from channels');
  ...
  useEffect(() => {
    ...
    console.log('[SalaVirtual v5.8] 🚀 INITIALIZING VIRTUAL ROOM');

    const init = async () => {
      console.log('[SalaVirtual v5.8] 1️⃣ Loading local data...');
      await loadLocalData();
      
      console.log('[SalaVirtual v5.8] 2️⃣ Checking user check-in status...');
      const checkedIn = await checkUserCheckin();
      
      if (!checkedIn && !localClosed && user) {
        console.log('[SalaVirtual v5.8] 3️⃣ User not checked in, checking in now...');
        const success = await handleCheckIn();
        if (!success) {
          console.error('[SalaVirtual v5.8] ❌ Check-in failed, aborting initialization');
          return;
        }
      }
      ...
      console.log('[SalaVirtual v5.8] 4️⃣ Loading initial messages...');
      await loadMessages();
      
      console.log('[SalaVirtual v5.8] 5️⃣ Setting up real-time subscriptions...');
      subscribeToUpdates();
      
      console.log('[SalaVirtual v5.8] 6️⃣ Setting up typing events...');
      subscribeToTypingEvents();
      
      console.log('[SalaVirtual v5.8] 7️⃣ Updating active users...');
      await updateActiveUsers();
      
      console.log('[SalaVirtual v5.8] 8️⃣ Loading private chats...');
      await loadPrivateChats();
      
      console.log('[SalaVirtual v5.8] ✅ INITIALIZATION COMPLETE');
    };

    init();

    return () => {
      console.log('[SalaVirtual v5.8] 🧹 Cleanup: Checking out user');
      handleCheckOut();
    };
  }, [localId]);
  ...
  useEffect(() => {
    if (activeTab === 'private' && user && localId) {
      console.log('[SalaVirtual v5.8] 🔄 Private tab active, reloading chats');
      loadPrivateChats();
    }
  ...
  useEffect(() => {
    if (selectedPrivateChat && user && localId) {
      console.log('[SalaVirtual v5.8] 🔄 selectedPrivateChat changed, syncing state...');
      console.log('[SalaVirtual v5.8] 🔄 Partner ID:', selectedPrivateChat.userId);
      
      const syncProfile = async () => {
        console.log('[SalaVirtual v5.8] 🔄 Fetching fresh profile for partner...');
        const profile = await fetchUserProfile(selectedPrivateChat.userId);
        
        if (profile) {
          console.log('[SalaVirtual v5.8] ✅ Profile fetched and updated');
          console.log('[SalaVirtual v5.8] 🖼️ Avatar:', profile.avatar || 'NO AVATAR');
      ...
      console.log('[SalaVirtual v5.8] 🔄 Marking messages as read...');
  ...
  const sendPublicMessage = useCallback(async (content: string) => {
    if (!user || !localId || !content.trim()) {
      console.log('[SalaVirtual v5.8] ⚠️ Cannot send message - missing requirements');
      return;
    }

    try {
      console.log('[SalaVirtual v5.8] 📤 Sending public message:', content);
      setSending(true);
      ...
      console.log('[SalaVirtual v5.8] 🔥 Fetching user profile from database...');
      ...
      console.log('[SalaVirtual v5.8] 🔥 Using profile data from database');
      console.log('[SalaVirtual v5.8] 👤 User profile:', currentUserProfile.nombre);
      console.log('[SalaVirtual v5.8] 🖼️ Avatar URL:', currentUserProfile.avatar || 'NO AVATAR');
      ...
      console.log('[SalaVirtual v5.8] ✨ Adding message optimistically with avatar from database');
      console.log('[SalaVirtual v5.8] 👤 User:', optimisticMsg.usuario.nombre, '| Avatar:', !!optimisticMsg.usuario.avatar);
      ...
      console.log('[SalaVirtual v5.8] 💾 Saving message to database...');
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error saving message to DB:', error);
        
        messageIdsRef.current.delete(messageId);
        setMessages(prev => prev.filter(m => m.id !== messageId));
        pendingMessageIds.current.delete(pendingId);
      } else {
        console.log('[SalaVirtual v5.8] ✅ Public message saved to DB with id:', insertedMessage.id);
        ...
        setMessages(prev => {
          ...
          if (withoutOptimistic.some(m => m.id === realMessage.id)) {
            console.log('[SalaVirtual v5.8] ℹ️ Real message already in UI from polling');
  ...
  const triggerFloatingReaction = useCallback((emoji: string) => {
    ...
    console.log(`[SalaVirtual v5.8] 🎉 Floating reaction triggered: ${emoji}`);
  }, []);
  ...
  const sendPredefinedMessage = useCallback(async (recipientId: string, messageText: string) => {
    ...
    try {
      console.log('[SalaVirtual v5.8] 📤 Sending predefined message:', messageText, 'to:', recipientId);
      ...
      console.log('[SalaVirtual v5.8] 🔥 Fetching user profile from database for predefined message...');
      const userProfile = await fetchUserProfile(user.id);
      ...
      if (existingChatIndex === -1) {
        ...
        console.log('[SalaVirtual v5.8] ✨ Creating new private chat optimistically');
        setPrivateChats(prev => [newChat, ...prev]);
      } else {
        console.log('[SalaVirtual v5.8] ✨ Updating existing private chat optimistically');
        setPrivateChats(prev => {
          const updatedChat = {
      ...
      console.log('[SalaVirtual v5.8] 💾 Saving with tipo = "privado"');
      ...
      if (insertError) {
        console.error('[SalaVirtual v5.8] ❌ Error saving private message to DB:', insertError);
      } else {
        console.log('[SalaVirtual v5.8] ✅ Private message saved to database with tipo = "privado"');
      ...
      console.log(`[SalaVirtual v5.8] ✅ Message sent to ${recipientName}`);
      ...
      console.log('[SalaVirtual v5.8] 🔄 Switching to private conversations tab');
      ...
      console.error('[SalaVirtual v5.8] ❌ Error sending predefined message:', error);
  ...
  const closeBottomSheet = useCallback(() => {
    console.log('[SalaVirtual v5.8] 📋 Closing bottom sheet');
  ...
  const sendPrivateMessage = useCallback(async (recipientId: string, content: string) => {
    ...
    try {
      console.log('[SalaVirtual v5.8] 📤 Sending private message to:', recipientId);
      ...
      console.log('[SalaVirtual v5.8] 🔥 Fetching user profile from database for private message...');
      const userProfile = await fetchUserProfile(user.id);
      ...
      console.log('[SalaVirtual v5.8] 🔥 Using profile data from database for private message');
      ...
      console.log('[SalaVirtual v5.8] ✨ Adding private message optimistically to UI');
      ...
      if (insertError) {
        ...
        console.log('[SalaVirtual v5.8] ✅ Private message saved to database with id:', insertedMessage.id);
      ...
      console.log('[SalaVirtual v5.8] ✅ Private message sent');
    } catch (error) {
      console.error('[SalaVirtual v5.8] ❌ Error sending private message:', error);
    }
  ...
  const openPrivateChat = useCallback(async (chat: PrivateChat) => {
    ...
    try {
      ...
      console.log('[SalaVirtual v5.8] 💬 Opening private chat with:', displayName);
      console.log('[SalaVirtual v5.8] 🔵 Unread count before:', chat.unreadCount);
      
      setSelectedPrivateChat(chat);
      
      console.log('[SalaVirtual v5.8] 🔵 Marking messages as read in database...');
      await markPrivateMessagesAsRead(chat.userId);
      
      console.log('[SalaVirtual v5.8] 🔵 Unread count set to 0 in frontend');
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error loading private messages:', error);
        return;
      }
      ...
      console.log('[SalaVirtual v5.8] ✅ Private messages loaded:', formattedMessages.length);
      ...
      console.error('[SalaVirtual v5.8] ❌ Error opening private chat:', error);
    }
  }, [user, localId, uniqueActiveUsers, markPrivateMessagesAsRead]);

  const closePrivateChat = useCallback(() => {
    console.log('[SalaVirtual v5.8] 💬 Closing private chat');
  ...
  const handleDeleteMessage = useCallback(async (message: Message) => {
    if (!user || message.usuario_id !== user.id) {
      console.log('[SalaVirtual v5.8] ⚠️ Cannot delete message - not owner');
      return;
    }

    console.log('[SalaVirtual v5.8] 🗑️ Showing delete confirmation for message:', message.id);
  ...
  const confirmDeleteMessage = useCallback(async () => {
    ...
    try {
      setDeleting(true);
      console.log('[SalaVirtual v5.8] 🗑️ Deleting message:', messageToDelete.id);
      ...
      if (error) {
        console.error('[SalaVirtual v5.8] ❌ Error deleting message:', error);
        ...
        console.log('[SalaVirtual v5.8] ✅ Message deleted successfully');
  ...
  const cancelDeleteMessage = useCallback(() => {
    console.log('[SalaVirtual v5.8] ❌ Delete cancelled');
    setShowDeleteModal(false);
    setMessageToDelete(null);
  }, []);
  ...
  const handleUserPress = async (selectedUser: ActiveUser) => {
    ...
    console.log('[SalaVirtual v5.8] 👤 User pressed:', displayName);
    
    if (selectedUser.id === user?.id) {
      console.log('[SalaVirtual v5.8] ⚠️ Cannot interact with self');
      return;
    }
    
    console.log('[SalaVirtual v5.8] 🔥 Fetching user profile for bottom sheet cover...');
    const profile = await fetchUserProfile(selectedUser.id);
    
    if (profile) {
      console.log('[SalaVirtual v5.8] ✅ Profile fetched for bottom sheet');
      console.log('[SalaVirtual v5.8] 🖼️ Cover photo (avatar):', profile.avatar || 'NO AVATAR');
      setSelectedUserProfile(profile);
    } else {
      console.log('[SalaVirtual v5.8] ⚠️ Could not fetch profile, using cached data');
    ...
    console.log('[SalaVirtual v5.8] 📋 Opening bottom sheet for user');
  ...
  const handleViewProfile = useCallback(async () => {
    if (!selectedUser) {
      console.log('[SalaVirtual v5.8] ⚠️ No selected user');
      return;
    }
    
    console.log('[SalaVirtual v5.8] 🚀 Starting profile navigation');
    console.log('[SalaVirtual v5.8] 👤 Target user ID:', selectedUser.id);
    console.log('[SalaVirtual v5.8] 🎯 Current active tab:', activeTab);
    console.log('[SalaVirtual v5.8] 🏠 Current local ID:', localId);
    
    console.log('[SalaVirtual v5.8] 📋 Step 1 - Closing bottom sheet...');
    closeBottomSheet();
    setSelectedPrivateChat(null);
    
    console.log('[SalaVirtual v5.8] ⏳ Waiting for bottom sheet animation (300ms)...');
    await new Promise(resolve => setTimeout(resolve, 300));
    console.log('[SalaVirtual v5.8] ✅ Bottom sheet closed');
    
    console.log('[SalaVirtual v5.8] 🎯 Step 2 - Navigating to profile...');
    ...
    try {
      ...
      console.log('[SalaVirtual v5.8] ✅ Navigation executed successfully');
    } catch (error) {
      console.error('[SalaVirtual v5.8] ❌ Error navigating:', error);
    }
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onLongPress={() => {
          if (isOwnMessage) {
            console.log('[SalaVirtual v5.8] 🗑️ Long press on own message');
      ...
      >
        {!isOwnMessage && (
          <TouchableOpacity
            ...
            onPress={() => {
              console.log('[SalaVirtual v5.8] 👤 Avatar clicked');
        ...
        {isOwnMessage && (
          <TouchableOpacity
            style={[styles.messageAvatar, { width: avatarSize, height: avatarSize }]}
            onPress={() => {
              console.log('[SalaVirtual v5.8] 👤 Navigating to own profile');
              router.push('/perfil');
            }}
          >
            {item.usuario.avatar ? (
  ...
  const renderUserItem = ({ item, index }: { item: ActiveUser; index: number }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          console.log('[SalaVirtual v5.8] 👤 Grid user card pressed:', displayName);
          if (isCurrentUser) {
            console.log('[SalaVirtual v5.8] 👤 Navigating to own profile');
            router.push('/perfil');
          } else {
  ...
  // ✅ FIX v5.8: Dynamic padding based on keyboard state with CORRECT Android positioning
  // When keyboard is closed on Android: position with 16px margin above system buttons
  ...
  const contentPaddingBottom = useMemo(() => {
    ...
    // ✅ ANDROID FIX v5.8: 16px margin when keyboard closed (comfortable spacing above system buttons)
    ...
    console.log('[SalaVirtual v5.8] 📏 Content padding bottom:', totalPadding, '| Keyboard height:', keyboardHeight, '| Safe area:', safeAreaBottom);
  ...
  const handleLoginSuccess = useCallback(async () => {
    console.log('[SalaVirtual v5.8] ✅ Login successful - reloading room');
    setShowLoginModal(false);
    
    console.log('[SalaVirtual v5.8] 🔄 Waiting for auth state to update...');
    await new Promise(resolve => setTimeout(resolve, 500));
    
    console.log('[SalaVirtual v5.8] 🔄 Reloading local data...');
    await loadLocalData();
    
    console.log('[SalaVirtual v5.8] 🚪 Checking in user...');
    const success = await handleCheckIn();
    
    if (success) {
      console.log('[SalaVirtual v5.8] ✅ User checked in, loading messages...');
      await loadMessages();
      subscribeToUpdates();
      subscribeToTypingEvents();
      await updateActiveUsers();
  ...
  if (loading) {
    console.log('[SalaVirtual v5.8] ⏳ Showing loading state');
    return (
      <LinearGradient
          ...
          <View style={[
            ...
            <View style={styles.tabBar}>
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual v5.8] 🔄 Switching to Chat tab');
                  setActiveTab('chat');
                  if (selectedPrivateChat) {
                    closePrivateChat();
                  }
              ...
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual v5.8] 🔄 Switching to Users tab');
                  setActiveTab('users');
                  if (selectedPrivateChat) {
                    closePrivateChat();
                  }
              ...
              <TouchableOpacity
                style={styles.tab}
                onPress={() => {
                  console.log('[SalaVirtual v5.8] 🔄 Switching to private tab');
                  setActiveTab('private');
                  loadPrivateChats();
                }}
                activeOpacity={0.7}
          ...
          {activeTab === 'chat' ? (
            <React.Fragment>
              <FlatList
                ...
                onContentSizeChange={() => {
                  console.log('[SalaVirtual v5.8] 📜 Content size changed, scrolling to bottom');
                  flatListRef.current?.scrollToEnd({ animated: true });
                }}
                ListEmptyComponent={
              ...
              <View style={[
                ...
                { 
                  backgroundColor: themeColors.cardBg, 
                  borderTopColor: themeColors.cardBorder,
                  // ✅ FIX v5.8: 16px margin when keyboard closed on Android (comfortable spacing above system buttons)
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    console.log('[SalaVirtual v5.8] ⚡ Toggling quick messages:', !showQuickMessages);
                ...
                <TextInput
                  ...
                  onFocus={() => {
                    console.log('[SalaVirtual v5.8] 🎹 Input focused - keyboard will appear');
                  }}
                  multiline
                  maxLength={500}
                />
              ...
              {selectedPrivateChat ? (
                <React.Fragment>
                  <View style={[
                    ...
                    <TouchableOpacity
                      onPress={async () => {
                        console.log('[SalaVirtual v5.8] 🚀 Profile button pressed in private chat header');
                        ...
                        console.log('[SalaVirtual v5.8] 🎯 Executing router.push() to profile');
                        try {
                          router.push({
                            pathname: '/perfil/usuario',
                            params: {
                          ...
                          console.log('[SalaVirtual v5.8] ✅ Navigation complete');
                        } catch (error) {
                          console.error('[SalaVirtual v5.8] ❌ Error navigating:', error);
                        }
                      }}
                  ...
                  <FlatList
                    ...
                    onContentSizeChange={() => {
                      console.log('[SalaVirtual v5.8] 📜 Private chat content size changed, scrolling to bottom');
                      privateChatListRef.current?.scrollToEnd({ animated: true });
                    }}
                    onLayout={() => {
                  ...
                  <View style={[
                    ...
                    <TextInput
                      ...
                      onFocus={() => {
                        console.log('[SalaVirtual v5.8] 🎹 Private input focused - keyboard will appear');
                      }}
                      multiline
                      maxLength={500}
                    />
