...
export default function ResetPasswordWebScreen() {
  ...
  const checkSession = async () => {
    try {
      console.log('[ResetPasswordWeb] 🔍 Verificando sesión de recuperación...');
      
      // For web, check URL hash parameters
      if (Platform.OS === 'web' && typeof window !== 'undefined') {
        const hash = window.location.hash;
        console.log('[ResetPasswordWeb] 📋 URL hash:', hash);
        ...
        if (hash) {
          ...
          console.log('[ResetPasswordWeb] 📋 Hash params:', {
          ...
          if (type === 'recovery' && accessToken) {
            console.log('[ResetPasswordWeb] 🔐 Estableciendo sesión de recuperación...');
            ...
            if (sessionError) {
              console.error('[ResetPasswordWeb] ❌ Error estableciendo sesión:', sessionError);
            ...
            if (data.session) {
              console.log('[ResetPasswordWeb] ✅ Sesión establecida para:', data.session.user.email);
      ...
      if (error || !session) {
        console.log('[ResetPasswordWeb] ⚠️ No hay sesión válida');
        setHasValidSession(false);
        setCheckingSession(false);
        return;
      }

      console.log('[ResetPasswordWeb] ✅ Sesión válida para:', session.user.email);
      setUserEmail(session.user.email || '');
      setHasValidSession(true);
      setCheckingSession(false);
    } catch (error: any) {
      console.error('[ResetPasswordWeb] ❌ Error inesperado:', error);
      setHasValidSession(false);
      setCheckingSession(false);
    }
  };
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      console.log('═══════════════════════════════════════════════════════');
      console.log('[ResetPasswordWeb] 🔄 ACTUALIZANDO CONTRASEÑA');
      console.log('═══════════════════════════════════════════════════════');
      ...
      if (error) {
        console.error('[ResetPasswordWeb] ❌ Error:', error);
        alert(`No se pudo actualizar tu contraseña: ${error.message}`);
        return;
      }

      console.log('[ResetPasswordWeb] ✅ Contraseña actualizada para:', data.user?.email);
      ...
      try {
        ...
        if (session) {
          ...
          console.log('[ResetPasswordWeb] ✅ Correo de confirmación enviado');
        }
      } catch (emailError) {
        console.error('[ResetPasswordWeb] ⚠️ Error enviando correo de confirmación:', emailError);
        // Don't fail the whole process if email fails
      }

      setPasswordUpdated(true);
    } catch (error: any) {
      console.error('[ResetPasswordWeb] ❌ Excepción:', error);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.centerContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (Platform.OS === 'web') {
                window.location.href = 'https://barliveapp.es/auth/recuperar-password-v6';
              }
            }}
          >
          ...
          <View style={styles.successContainer}>
            ...
            <View style={styles.infoCard}>
              ...
              <Text style={styles.infoCardText}>
                Hemos enviado un correo de confirmación a {userEmail} para informarte del cambio.
              </Text>
            </View>
            ...
            >
              <Text style={styles.closeButtonText}>Cerrar página</Text>
            </TouchableOpacity>
            ...
            <View style={styles.securityNote}>
              ...
              <Text style={styles.securityNoteText}>
                Si no realizaste este cambio, contacta inmediatamente con soporte@barliveapp.es
              </Text>
            </View>
          </View>
        </ScrollView>
