...
/**
 * 🔐 SECURE REGISTRATION SCREEN v1.2 - INTELLIGENT REDIRECT FLOW ENHANCED
 * 
 * NEW CHANGES v1.2:
 * - ✅ IMPROVED: Better redirect path handling and parsing
 * - ✅ FIXED: Passes redirect parameter correctly to verification screen
 * - ✅ ENHANCED: More robust navigation after successful registration
 * 
 * Previous changes maintained (v1.1):
...
export default function SecureRegistrationScreen() {
  ...
  // ✅ v1.2: Read redirect parameter
  const redirectPath = params.redirect as string | undefined;
  
  console.log('[SecureRegistration v1.2] 🔄 Redirect path from params:', redirectPath);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      console.log('[SecureRegistration v1.2] 🔐 Creating secure account:', normalizedEmail);
      ...
      if (authError) {
        console.error('[SecureRegistration v1.2] ❌ Registration error:', authError);
          ...
          Alert.alert(
            ...
            [
              { text: 'Cancelar', style: 'cancel' },
              {
                text: 'Iniciar sesión',
                onPress: () => {
                  // ✅ v1.2: Pass redirect parameter to login screen
                  if (redirectPath) {
                    router.replace({
      ...
      console.log('[SecureRegistration v1.2] ✅ Account created successfully:', authData.user.id);
        ...
        [
          {
            text: 'Entendido',
            onPress: () => {
              // ✅ v1.2: INTELLIGENT REDIRECT - Pass redirect path to verification screen
              if (redirectPath) {
                console.log('[SecureRegistration v1.2] 🎯 Will redirect to saved path after verification:', redirectPath);
                router.replace({
                ...
                console.log('[SecureRegistration v1.2] 🏠 No redirect path, standard verification flow');
                router.replace({
                  pathname: '/auth/verificar-email',
      ...
      console.error('[SecureRegistration v1.2] ❌ Unexpected error:', error);
