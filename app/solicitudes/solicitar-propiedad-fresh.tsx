...
import FreshDocumentUploader from '@/components/propiedad/FreshDocumentUploader';
 ...
 * - Usa FreshDocumentUploader (nuevo componente)
...
export default function SolicitarPropiedadFreshScreen() {
  ...
  console.log('[FreshScreen] 🎬 Pantalla inicializada');
  console.log('[FreshScreen] 📋 Tipo:', requestType);
  console.log('[FreshScreen] 👤 Usuario:', user?.nombre);
  ...
  const loadPreselectedLocal = useCallback(async (localId: string) => {
    try {
      console.log('[FreshScreen] 🔍 Cargando local:', localId);
      ...
      console.log('[FreshScreen] ✅ Local cargado:', data.nombre);
      setSelectedLocal(data);
      setCurrentStep(2);
    } catch (error) {
      console.error('[FreshScreen] ❌ Error:', error);
  ...
  const searchLocales = useCallback(async (query: string) => {
    ...
    try {
      ...
      console.log('[FreshScreen] 🔍 Buscando:', query);
      ...
      console.log('[FreshScreen] ✅ Encontrados:', data?.length || 0);
  ...
  const handleSelectLocal = async (local: LocalSearchResult) => {
    console.log('[FreshScreen] ✅ Local seleccionado:', local.nombre);
  ...
  const handleGetCurrentLocation = async () => {
    setLoading(true);
    try {
      console.log('[FreshScreen] 📍 Obteniendo ubicación...');
      ...
      console.log('[FreshScreen] ✅ Ubicación obtenida');
  ...
  const handleWebViewMessage = (event: any) => {
    try {
      ...
      if (data.type === 'location_selected') {
        console.log('[FreshScreen] 📍 Ubicación en mapa:', data.lat, data.lng);
...
<body>
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      console.log('[FreshScreen] 📤 Enviando solicitud...');
      ...
      if (requestType === 'reclamar_local') {
        ...
        console.log('[FreshScreen] 💾 Guardando solicitud');
        console.log('[FreshScreen] 📄 URL documento:', documentoUrl);
        ...
        console.log('[FreshScreen] ✅ Solicitud creada');
        ...
        console.log('[FreshScreen] 💾 Guardando nuevo local');
        console.log('[FreshScreen] 📄 URL documento:', documentoUrl);
      ...
      {searchQuery.length >= 3 && !searchingLocales && searchResults.length === 0 && (
        <View style={styles.noResultsContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              router.replace({
                pathname: '/solicitudes/solicitar-propiedad-fresh',
  ...
  const renderReclamarStep2 = () => (
    <View style={styles.stepContent}>
      ...
      {/* 🆕 NUEVO: Componente Fresh */}
      <FreshDocumentUploader
        onUploadComplete={(url) => {
          console.log('[FreshScreen] ✅ Documento recibido:', url);
          setDocumentoUrl(url);
        }}
    ...
    >
      ...
      >
        ...
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerSubtitle}>Sistema Fresh v5.0</Text>
        </View>
        <View style={{ width: 40 }} />
      </LinearGradient>
