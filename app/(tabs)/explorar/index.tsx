...
/**
 * ✅ EXPLORAR SCREEN v73.0 - CRITICAL FIXES FOR iOS AND ANDROID
 * 
...
export default function ExplorarScreen() {
  ...
  const userIsAdmin = useMemo(() => {
    ...
    console.log('[ExplorarScreen v73.0] Admin check for mode selector:', {
      email: user?.email,
      role: user?.rol_app,
      isAdmin,
  ...
  useFocusEffect(
    useCallback(() => {
      console.log('[ExplorarScreen v73.0] ⚡ Screen focused');
      setMostrarFiltros(false);
      setMostrarSelectorModo(false);
      
      return () => {
        console.log('[ExplorarScreen v73.0] Screen unfocused');
      };
    }, [])
  );
  ...
  const localesFiltradosCompletos = useMemo(() => {
    console.log('[ExplorarScreen v73.0] ⚡ Applying filters...');
    console.log('[ExplorarScreen v73.0] 📊 Total locales:', todosLosLocales.length);
    console.log('[ExplorarScreen v73.0] 🔍 Selected category:', categoriaSeleccionada);
    console.log('[ExplorarScreen v73.0] 🔍 Global filters:', globalFiltros);

    let localesFiltrados = todosLosLocales.filter(local => local.activo === true);
    console.log('[ExplorarScreen v73.0] ✅ After activo filter:', localesFiltrados.length);

    if (categoriaSeleccionada !== 'todos') {
      console.log('[ExplorarScreen v73.0] 🔍 Filtering by category:', categoriaSeleccionada);
      ...
      console.log(`[ExplorarScreen v73.0] ✅ After category filter: ${localesFiltrados.length} locales`);
    }
    ...
    if (busqueda) {
      ...
      console.log(`[ExplorarScreen v73.0] 🔍 After search filter: ${localesFiltrados.length} locales`);
    }
    ...
    if (globalFiltros.comunidad && globalFiltros.comunidad !== 'Todas las Comunidades') {
      ...
      console.log(`[ExplorarScreen v73.0] 🔍 After community filter: ${localesFiltrados.length} locales`);
    }

    if (globalFiltros.provincia) {
      localesFiltrados = localesFiltrados.filter(local => local.provincia === globalFiltros.provincia);
      console.log(`[ExplorarScreen v73.0] 🔍 After province filter: ${localesFiltrados.length} locales`);
    }
    ...
    if (globalFiltros.tipo && globalFiltros.tipo.length > 0) {
      ...
      console.log(`[ExplorarScreen v73.0] 🔍 After type filter: ${localesFiltrados.length} locales`);
    }
    ...
    if (globalFiltros.servicios && globalFiltros.servicios.length > 0) {
      ...
      console.log(`[ExplorarScreen v73.0] 🔍 After services filter: ${localesFiltrados.length} locales`);
    }
    ...
    if (globalFiltros.ambiente && globalFiltros.ambiente.length > 0 && !globalFiltros.ambiente.includes('cualquiera')) {
      ...
      console.log(`[ExplorarScreen v73.0] 🔍 After ambiente filter: ${localesFiltrados.length} locales`);
    }
    ...
    if (globalFiltros.clientela && globalFiltros.clientela.length > 0 && !globalFiltros.clientela.includes('cualquiera')) {
      ...
      console.log(`[ExplorarScreen v73.0] 🔍 After clientela filter: ${localesFiltrados.length} locales`);
    }
    ...
    if (userLocation) {
      console.log('[ExplorarScreen v73.0] 📍 User location:', userLocation);
      ...
      if (globalFiltros.distancia) {
        ...
        console.log(`[ExplorarScreen v73.0] 🔍 After distance filter (${globalFiltros.distancia}km): ${localesFiltrados.length} locales`);
      }

      console.log('[ExplorarScreen v73.0] 🧠 Applying FIXED sorting algorithm...');
      ...
      console.log('[ExplorarScreen v73.0] 📊 Open/Closed split:');
      ...
      console.log('[ExplorarScreen v73.0] ✅ FIXED SORTING APPLIED (OPEN FIRST) - Total locals:', localesFiltrados.length);
    } else {
      console.log('[ExplorarScreen v73.0] ⚠️ No user location available, sorting by open status, destacado and rating');
    ...
    console.log(`[ExplorarScreen v73.0] ⚡ Final filtered and sorted locals: ${localesFiltrados.length}`);
  ...
  const obtenerUbicacionUsuario = async () => {
    try {
      console.log('[ExplorarScreen v73.0] 🔍 Requesting location permissions...');
      
      // ✅ iOS FIX v73.0: Check if location services are enabled first
      let isAvailable = false;
      try {
        isAvailable = await Location.hasServicesEnabledAsync();
      } catch (error) {
        console.log('[ExplorarScreen v73.0] ⚠️ Error checking location services:', error);
        isAvailable = false;
      }
      
      if (!isAvailable) {
        console.log('[ExplorarScreen v73.0] ⚠️ Location services are disabled, using default location (Madrid)');
      ...
      try {
        ...
        console.log('[ExplorarScreen v73.0] ⚠️ Error requesting location permission:', error);
        status = 'denied';
      }
      
      if (status !== 'granted') {
        console.log('[ExplorarScreen v73.0] ⚠️ Location permission denied, using default location (Madrid)');
        setUserLocation({ lat: 40.4168, lng: -3.7038 });
        return;
      }

      console.log('[ExplorarScreen v73.0] ✅ Location permission granted, getting position...');
      ...
      try {
        ...
        console.log('[ExplorarScreen v73.0] 📍 User location obtained:', {
          lat: location.coords.latitude,
          lng: location.coords.longitude,
        });
      } catch (locationError: any) {
        console.error('[ExplorarScreen v73.0] ❌ Error getting current position:', {
        ...
        try {
          ...
          if (lastLocation) {
            ...
            console.log('[ExplorarScreen v73.0] 📍 Using last known location:', {
              lat: lastLocation.coords.latitude,
              lng: lastLocation.coords.longitude,
            });
          } else {
          ...
          console.log('[ExplorarScreen v73.0] ⚠️ Using default location (Madrid) due to error');
          setUserLocation({ lat: 40.4168, lng: -3.7038 });
        }
      }
    } catch (error: any) {
      console.error('[ExplorarScreen v73.0] ❌ Error in obtenerUbicacionUsuario:', {
  ...
  const cargarPromocionesActivas = async () => {
    try {
      console.log('[ExplorarScreen v73.0] 🔄 Loading active promotions...');
      ...
      if (subsError) {
        console.error('[ExplorarScreen v73.0] Error loading subscriptions:', subsError);
        return;
      }

      if (!suscripciones || suscripciones.length === 0) {
        console.log('[ExplorarScreen v73.0] No active subscriptions found');
      ...
      if (planesError) {
        console.error('[ExplorarScreen v73.0] Error loading plans:', planesError);
        return;
      }

      if (!planes || planes.length === 0) {
        console.log('[ExplorarScreen v73.0] No plans with promotions found');
        setActivePromotions(new Set());
        return;
      }
      ...
      console.log('[ExplorarScreen v73.0] 💰 Active promotions loaded:', promotedLocalIds.size);
    } catch (error) {
      console.error('[ExplorarScreen v73.0] Error in cargarPromocionesActivas:', error);
      setActivePromotions(new Set());
    }
  };
  ...
  const handleModoChange = (modo: ModoUsuario) => {
    console.log('[ExplorarScreen v73.0] Mode change:', modo);
    setCurrentMode(modo);
    setMostrarSelectorModo(false);
  };
...
const styles = StyleSheet.create({
  ...
  // ✅ ANDROID FIX v73.0: Normalized font size on Android (80% of iOS)
  headerTitle: {
  ...
  // ✅ ANDROID FIX v73.0: Category icon container sizes normalized on Android (85% of iOS)
  ...
  // ✅ ANDROID FIX v73.0: Category label sizes normalized on Android (80% of iOS)
  categoriaLabel: {
  ...
  // ✅ ANDROID FIX v73.0: Icon container sizes normalized on Android (85% of iOS)
  ...
  // ✅ ANDROID FIX v73.0: Text sizes normalized on Android (80% of iOS)
  claimLocalTitle: {
    fontSize: Platform.OS === 'ios' ? 14 : 11,
    fontWeight: '700',
    color: colors.text,
  ...
  // ✅ ANDROID FIX v73.0: Arrow container sizes normalized on Android (85% of iOS)
  claimLocalArrow: {
  ...
  // ✅ ANDROID FIX v73.0: Empty state text sizes normalized on Android (80% of iOS)
  emptyText: {
  ...
  // ✅ ANDROID FIX v73.0: Modal text sizes normalized on Android (80% of iOS)
  modalTitle: {
