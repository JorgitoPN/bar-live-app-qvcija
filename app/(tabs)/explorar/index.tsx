...
/**
 * ✅ EXPLORAR SCREEN v351.0 - CRITICAL PAGINATION FIX
 * 
 * 🔥 ROOT CAUSE IDENTIFIED:
 * The lastFiltersRef was being updated even during append operations,
 * causing subsequent pagination calls to think filters had changed.
 * 
 * ✅ SOLUTION v351.0:
 * - ONLY update lastFiltersRef when append=false (initial load or manual filter change)
 * - NEVER update lastFiltersRef during append operations (page 2, 3, etc.)
 * - This ensures pagination continues smoothly without resetting
 * 
 * Previous fixes v350.0:
 * - ✅ ONLY check for filter changes when append=false
 * - ✅ NEVER reset pagination during append operations
 * 
...
export default function ExplorarScreen() {
  ...
  const loadLocales = useCallback(async (page: number = 1, append: boolean = false) => {
    ...
    // 🔥 CRITICAL FIX v351.0: Only check for filter changes when NOT appending
    // AND only update lastFiltersRef when NOT appending
    if (!append) {
      ...
      if (filtersChanged) {
        ...
        lastFiltersRef.current = filtersKey; // ✅ Only update ref when filters actually change
        setCurrentPage(1);
        setAllLoadedLocales([]);
        setDisplayedLocales([]);
        setHasMore(true);
      ...
      console.log('[Explorar v351.0] ✅ Appending data, NOT checking or updating filter ref');
    }
    ...
    if (append) {
      console.log('[Explorar v351.0] 📊 Setting loading more state');
  ...
  }, [userLocation, isValidSpainCoordinate, selectedCategory, provinciaSeleccionada, deferWithPriority, isLoadingMore, locationReady, allLoadedLocales.length]);
  ...
  const filteredLocales = useMemo(() => {
    ...
    console.log('[Explorar v351.0] 📊 Result:', uniqueLocales.length, 'locals');
    console.log('[Explorar v351.0] 📊 Filtered out:', allLoadedLocales.length - uniqueLocales.length, 'locals');
  ...
  }, [hasMore, isLoadingMore, loading, currentPage, locationReady, loadLocales, displayedLocales.length, allLoadedLocales.length]);
  ...
  const handleOpenAdvancedFilters = useCallback(() => {
    console.log('[Explorar v351.0] 🔍 Opening advanced filters');
    setShowAdvancedFilters(true);
  }, []);

  const handleCloseAdvancedFilters = useCallback(() => {
    console.log('[Explorar v351.0] ✅ Closing advanced filters');
    setShowAdvancedFilters(false);
  }, []);

  const handleClearAdvancedFilters = useCallback(() => {
    console.log('[Explorar v351.0] 🧹 Clearing advanced filters');
    limpiarFiltros();
  }, [limpiarFiltros]);
