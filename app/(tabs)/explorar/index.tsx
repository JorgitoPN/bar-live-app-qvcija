...
/**
 * ✅ EXPLORAR SCREEN v412.0 - ADVANCED FILTERS FLICKERING FIX
 * 
 * CAMBIOS v412.0 - CORRECCIÓN DE PARPADEO Y FILTROS:
 * - 🎯 CRITICAL FIX: Corregido parpadeo infinito al aplicar filtros avanzados
 * - ✅ Mejorada detección de cambios en filtros (evita loops infinitos)
 * - ✅ Validación estricta de arrays y strings antes de enviar al backend
 * - ✅ Sincronización correcta entre FilterContext y Explorar
 * - ✅ Scroll suave al aplicar filtros (sin saltos bruscos)
 * - 🔧 FIX: Dependencias del useEffect optimizadas para evitar re-renders
 * 
...
export default function ExplorarScreen() {
  ...
  useEffect(() => {
    if (!mountedRef.current) {
      console.log('[Explorar v412.0] 🚀 Component mounted - FLICKERING FIX + ADVANCED FILTERS');
  ...
  const loadLocales = useCallback(async (reset: boolean = false) => {
    ...
    if (loadingRef.current) {
      console.log('[Explorar v412.0] ⏸️ Already loading, skipping...');
      return;
    }
    
    // ✅ GUARDIA 2: Si no es reset y no hay más datos, no cargar
    if (!reset && !hasMore) {
      console.log('[Explorar v412.0] ⏸️ No more data to load');
      return;
    }
    
    // ✅ GUARDIA 3: Verificar que la ubicación esté lista
    if (!locationReady) {
      console.log('[Explorar v412.0] ⏸️ Location not ready yet');
      return;
    }
    ...
    try {
      ...
      // ✅ CRITICAL FIX v412.0: Pass advanced filters to backend RPC with proper null handling
      const serviciosFilter = (globalFiltros.servicios && Array.isArray(globalFiltros.servicios) && globalFiltros.servicios.length > 0) 
        ? globalFiltros.servicios 
        : null;
      const ambienteFilter = (globalFiltros.ambiente && Array.isArray(globalFiltros.ambiente) && globalFiltros.ambiente.length > 0) 
        ? globalFiltros.ambiente 
        : null;
      const clientelaFilter = (globalFiltros.clientela && Array.isArray(globalFiltros.clientela) && globalFiltros.clientela.length > 0) 
        ? globalFiltros.clientela 
        : null;
      const comunidadFilter = (globalFiltros.comunidad && globalFiltros.comunidad !== 'Todas las Comunidades' && globalFiltros.comunidad.trim() !== '') 
        ? globalFiltros.comunidad 
        : null;
      const provinciaFilter = (globalFiltros.provincia && globalFiltros.provincia.trim() !== '') 
        ? globalFiltros.provincia 
        : null;
      const maxDistanceKm = (globalFiltros.distancia && globalFiltros.distancia > 0) 
        ? globalFiltros.distancia 
        : null;
      
      console.log('[Explorar v412.0] 📡 Fetching locales with ADVANCED FILTERS...');
      console.log('[Explorar v412.0] 📍 User location:', userLocation);
      console.log('[Explorar v412.0] 🏷️ Category filter:', categoryFilter);
      console.log('[Explorar v412.0] 🔧 Servicios filter:', serviciosFilter);
      console.log('[Explorar v412.0] ✨ Ambiente filter:', ambienteFilter);
      console.log('[Explorar v412.0] 👥 Clientela filter:', clientelaFilter);
      console.log('[Explorar v412.0] 🌍 Comunidad filter:', comunidadFilter);
      console.log('[Explorar v412.0] 📍 Provincia filter:', provinciaFilter);
      console.log('[Explorar v412.0] 📏 Max distance:', maxDistanceKm, 'km');
      console.log('[Explorar v412.0] 📄 Offset:', offset, 'Limit:', ITEMS_PER_PAGE);
      ...
      if (data && data.length > 0) {
        console.log('[Explorar v412.0] ✅ Received', data.length, 'locales from backend');
        console.log('[Explorar v412.0] 🎯 First 3 locales:', data.slice(0, 3).map((l: any) => ({
          nombre: l.nombre,
        ...
        console.log('[Explorar v412.0] 📊 Has more data:', hasMoreData);
        
      } else {
        console.log('[Explorar v412.0] 📭 No data received');
        setHasMore(false);
        if (reset) {
          setAllLocales([]);
        }
  ...
  }, [userLocation, locationReady, allLocales.length, hasMore, selectedCategory, globalFiltros.servicios, globalFiltros.ambiente, globalFiltros.clientela, globalFiltros.comunidad, globalFiltros.provincia, globalFiltros.distancia, getCategoryFilterForBackend]);
  ...
  useEffect(() => {
    // ✅ CRITICAL FIX v412.0: Stringify globalFiltros properly to avoid infinite loops
    const filtersKey = `${selectedCategory}-${provinciaSeleccionada}-${debouncedQuery}-${JSON.stringify({
    ...
    if (filtersChanged && lastFiltersRef.current !== '') {
      ...
      console.log('[Explorar v412.0] 📊 New filters:', {
        servicios: globalFiltros.servicios,
        ambiente: globalFiltros.ambiente,
        clientela: globalFiltros.clientela,
        comunidad: globalFiltros.comunidad,
      ...
      // ✅ CRITICAL: Reset state synchronously
      setAllLocales([]);
      setHasMore(true);
      loadingRef.current = false;
  ...
  const filteredLocales = useMemo(() => {
    ...
    console.log('[Explorar v412.0] 📊 Filtered locales (search only):', filtered.length);
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log('[Explorar v412.0] 🔄 Auto-loading more (filtered list too small)');
      loadLocales(false);
    }
  ...
  const loadMoreLocales = useCallback(() => {
    ...
    console.log('[Explorar v412.0] 📜 Loading more locales (infinite scroll)');
    loadLocales(false);
  }, [hasMore, isLoading, loadLocales]);

  // ✅ REFRESH: Resetear todo y cargar desde cero
  const onRefresh = async () => {
    console.log('[Explorar v412.0] 🔄 Refreshing...');
    setRefreshing(true);
  ...
  const clearFilters = useCallback(() => {
    console.log('[Explorar v412.0] 🧹 Clearing filters');
    setSearchQuery('');
    setDebouncedQuery('');
    setSelectedCategory('todas');
    setProvinciaSeleccionada('Todas');
