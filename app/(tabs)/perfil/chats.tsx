...
/**
 * ✅ CHATS SYSTEM v19.0 - ENHANCED DELETION WITH PROPER SYNC
 * 
 * Key improvements:
 * - Proper session validation before deletion
 * - Cascade deletion (messages first, then chats)
 * - Optimistic UI updates
 * - Database sync verification with retry logic
 * - Enhanced error handling
 * - Real-time reload after deletion
...
export default function ChatsScreen() {
  ...
  const loadChats = useCallback(async (forceRefresh: boolean = false) => {
    ...
    try {
      if (forceRefresh) {
        console.log('[Chats v19.0] 🔄 Force refreshing chats from database...');
      } else {
        console.log('[Chats v19.0] 🔍 Loading chats for user:', user.id);
      }
      ...
      if (chatsError) {
        console.error('[Chats v19.0] Error loading chats:', chatsError);
        return;
      }

      console.log('[Chats v19.0] ✅ Loaded', chatsData?.length || 0, 'chats from database');
      ...
      const chatsWithInfo = await Promise.all(
        (chatsData || []).map(async (chat) => {
          ...
          let userData;
          if (chat.local_id) {
            console.log('[Chats v19.0] 🏢 Chat', chat.id, 'is LOCAL-SPECIFIC, loading local info for:', chat.local_id);
            ...
            if (localData) {
              ...
              console.log('[Chats v19.0] ✅ Loaded local info:', localData.nombre);
            } else {
              console.error('[Chats v19.0] ❌ Failed to load local info for:', chat.local_id);
            }
          } else {
            console.log('[Chats v19.0] 👤 Chat', chat.id, 'is USER-TO-USER, loading user info for:', otroUsuarioId);
            ...
            if (userData) {
              console.log('[Chats v19.0] ✅ Loaded user info:', userData.nombre, 'username:', userData.username);
            }
          }
      ...
      console.log('[Chats v19.0] ✅ Processed all chats with info');
    } catch (error) {
      console.error('[Chats v19.0] Error:', error);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const handleOpenChat = useCallback(async (chatId: string, isLocalChat: boolean, localId?: string) => {
    ...
    console.log('[Chats v19.0] 🔥 Opening chat:', { chatId, isLocalChat, localId });
    ...
    try {
      ...
      console.error('[Chats v19.0] Error marking messages as read:', error);
    }

    // ✅ Route to local-specific chat if this is a local chat
    if (isLocalChat && localId) {
      console.log('[Chats v19.0] 🏢 Navigating to LOCAL-SPECIFIC chat');
      router.push(`/chat/conversacion?localId=${localId}&userId=${user.id}`);
    } else {
      console.log('[Chats v19.0] 👤 Navigating to USER-TO-USER chat');
  ...
  // ✅ CRITICAL FIX v19.0: COMPLETE REDESIGN - Enhanced deletion with proper session validation and database sync
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            console.log('[Chats v19.0] 🗑️ Starting deletion of', chatIdsToDelete.length, 'conversations');
            console.log('[Chats v19.0] 📋 Chat IDs to delete:', chatIdsToDelete);
            
            setDeleting(true);
            
            try {
              // ✅ CRITICAL FIX v19.0: Step 1 - Ensure valid session before deletion
              console.log('[Chats v19.0] 🔄 Step 1: Ensuring valid session...');
              const validSession = await ensureValidSession();
              
              if (!validSession || !validSession.user) {
                console.error('[Chats v19.0] ❌ No valid session available');
              ...
              console.log('[Chats v19.0] ✅ Step 1 complete: Valid session confirmed');
              
              // ✅ CRITICAL FIX v19.0: Step 2 - Optimistic UI update - remove chats from local state immediately
              console.log('[Chats v19.0] 🎯 Step 2: Optimistic UI update - removing chats from local state');
              setChats(prevChats => prevChats.filter(chat => !chatIdsToDelete.includes(chat.id)));
              console.log('[Chats v19.0] ✅ Step 2 complete: Chats removed from UI');
              
              // ✅ CRITICAL FIX v19.0: Step 3 - Delete messages first with proper error handling
              console.log('[Chats v19.0] 🗑️ Step 3: Deleting messages for all chats...');
              
              let totalMessagesDeleted = 0;
              let messagesErrorCount = 0;
              
              for (const chatId of chatIdsToDelete) {
                console.log('[Chats v19.0] 🗑️ Deleting messages for chat:', chatId);
                ...
                console.log('[Chats v19.0] 📊 Found', messageCount || 0, 'messages to delete for chat:', chatId);
                ...
                if (messagesError) {
                  console.error('[Chats v19.0] ❌ Error deleting messages for chat', chatId, ':', messagesError);
                  messagesErrorCount++;
                } else {
                  console.log('[Chats v19.0] ✅ Deleted', deletedCount || 0, 'messages for chat:', chatId);
                  totalMessagesDeleted += (deletedCount || 0);
                }
              }

              console.log('[Chats v19.0] ✅ Step 3 complete: Deleted', totalMessagesDeleted, 'messages total');
              if (messagesErrorCount > 0) {
                console.warn('[Chats v19.0] ⚠️ Failed to delete messages for', messagesErrorCount, 'chats');
              }

              console.log('[Chats v19.0] 🗑️ Step 4: Deleting chats from database...');

              // ✅ CRITICAL FIX v19.0: Step 4 - Delete chats in a single query with proper error handling
              ...
              if (chatsError) {
                console.error('[Chats v19.0] ❌ Error deleting chats:', chatsError);
                throw chatsError;
              }

              console.log('[Chats v19.0] ✅ Step 4 complete: Deleted', chatsCount || 0, 'chats from database');

              // ✅ CRITICAL FIX v19.0: Step 5 - Verify deletion by reloading from database with retry logic
              console.log('[Chats v19.0] 🔄 Step 5: Verifying deletion by reloading chats...');
              
              // Wait a moment for database to sync
              await new Promise(resolve => setTimeout(resolve, 2000));
              
              // Reload chats to ensure sync - with retry logic
              let retryCount = 0;
              const maxRetries = 3;
              let syncSuccess = false;
              
              while (retryCount < maxRetries && !syncSuccess) {
                console.log('[Chats v19.0] 🔄 Reload attempt', retryCount + 1, 'of', maxRetries);
                
                await loadChats(true); // Force refresh from database
                
                // Check if deleted chats are still in the list
                const currentChatIds = chats.map(c => c.id);
                const stillPresent = chatIdsToDelete.filter(id => currentChatIds.includes(id));
                
                if (stillPresent.length === 0) {
                  syncSuccess = true;
                  console.log('[Chats v19.0] ✅ Sync verified: All deleted chats removed from list');
                } else {
                  console.warn('[Chats v19.0] ⚠️ Sync incomplete:', stillPresent.length, 'chats still present');
                  retryCount++;
              ...
              if (!syncSuccess) {
                console.error('[Chats v19.0] ❌ Sync failed after', maxRetries, 'attempts');
                // Force one final reload
                await loadChats(true);
              }
              
              console.log('[Chats v19.0] ✅ Step 5 complete: Chats reloaded from database');
              
              console.log('[Chats v19.0] ✅ Successfully deleted', chatIdsToDelete.length, 'conversations');
              ...
              console.error('[Chats v19.0] ❌ Error deleting conversations:', error);
              Alert.alert('Error', 'No se pudieron eliminar algunas conversaciones. Recargando...');
              
              // ✅ CRITICAL FIX v19.0: Always reload chats on error to sync with database
              await loadChats(true);
            } finally {
              setDeleting(false);
            }
