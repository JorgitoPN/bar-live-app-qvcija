...
// ✅ CRITICAL FIX: Filtros de categorías con diseño consistente (solo emojis)
const CATEGORIAS = [
...
export default function FavoritosScreen() {
  ...
  const loadSavedLocales = useCallback(async () => {
    ...
    try {
      ...
      const locales = (data || [])
        ...
        .filter((local: any) => local && local.activo);
  ...
  const filteredLocales = useMemo(() => {
    return allSavedLocales.filter((local) => {
  ...
  }, [allSavedLocales, searchQuery, provinciaSeleccionada, selectedCategory]);
    ...
    return (
      <View style={styles.emptyState}>
        ...
        <Text style={[styles.emptyStateText, { fontSize: scaleFontSize(16) }]}>
          No tienes locales favoritos
        </Text>
        <Text style={[styles.emptyStateSubtext, { fontSize: scaleFontSize(14) }]}>
          Explora locales y guárdalos como favoritos
        </Text>
      </View>
    );
  };
