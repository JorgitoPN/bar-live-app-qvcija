...
interface UnifiedStoryViewerV10Props {
  visible: boolean;
  stories: Story[];
  initialIndex: number;
  onClose: () => void;
...
/**
 * ✅ UNIFIED STORY VIEWER V10.0 - INSTAGRAM-STYLE COMPLETE SYSTEM
 * 
 * Features:
 ...
 */
function UnifiedStoryViewerV10({
  visible,
  stories,
  initialIndex,
  onClose,
  ...
  console.log('[UnifiedStoryViewerV10] 🎭 V10.0 - Instagram-style viewer:', {
    interactionUserId,
    interactionLocalId,
  ...
  const markAsViewed = useCallback(async (storyId: string) => {
    ...
    if (viewDuration < MIN_VIEW_THRESHOLD) {
      console.log('[UnifiedStoryViewerV10] ⏭️ View threshold not met:', {
        viewDuration,
  ...
  const checkIfLiked = useCallback(async (storyId: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('[UnifiedStoryViewerV10] Error checking like status:', error);
        setIsLiked(false);
        return;
      }
  ...
  // ✅ INSTAGRAM LOGIC: Mark as viewed when leaving the story
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    if (visible) {
      console.log('[UnifiedStoryViewerV10] 🎬 Story viewer opened:', {
        initialIndex,
        totalStories: stories.length,
        duration,
      });
  ...
  const handleNext = useCallback(() => {
    console.log('[UnifiedStoryViewerV10] ⏭️ Next story:', {
      currentIndex,
      totalStories: stories.length,
    });

    // ✅ INSTAGRAM LOGIC: Mark current story as viewed before moving to next
    if (currentStory && interactionUserId && !isOwner) {
      markAsViewed(currentStory.id);
    }
    ...
    if (currentIndex < stories.length - 1) {
      ...
      console.log('[UnifiedStoryViewerV10] 🏁 Reached end of stories, auto-closing');
      onClose();
    }
  ...
  const handlePrevious = useCallback(() => {
    console.log('[UnifiedStoryViewerV10] ⏮️ Previous story:', {
      currentIndex,
    });
  ...
  const handleLongPressIn = useCallback(() => {
    console.log('[UnifiedStoryViewerV10] ⏸️ Story paused');
    isLongPressing.current = true;
    setIsPaused(true);
  }, []);

  const handleLongPressOut = useCallback(() => {
    console.log('[UnifiedStoryViewerV10] ▶️ Story resumed');
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (error) {
                console.error('[UnifiedStoryViewerV10] Error deleting story:', error);
                return;
              }
  ...
  const handleViewStoryStats = useCallback(async () => {
    ...
    try {
      ...
      console.error('[UnifiedStoryViewerV10] Error loading story stats:', error);
  ...
  const handleStoryLike = useCallback(async () => {
    ...
    try {
      ...
      console.error('[UnifiedStoryViewerV10] Error toggling story like:', error);
      setIsLiked(!newLikedState);
    }
  ...
  const handleSendStoryMessage = useCallback(async () => {
    ...
    try {
      ...
      console.error('[UnifiedStoryViewerV10] Error sending story message:', error);
    } finally {
        ...
        <View style={styles.container} {...panResponder.panHandlers}>
          {storyImageUrl ? (
            <Image
              ...
              resizeMode="contain"
              onLoadStart={() => {
                console.log('[UnifiedStoryViewerV10] 🖼️ Image loading started');
                setLoading(true);
              }}
              onLoadEnd={() => {
                console.log('[UnifiedStoryViewerV10] ✅ Image loaded successfully');
                setLoading(false);
              }}
              onError={() => {
                console.error('[UnifiedStoryViewerV10] ❌ Error loading image:', storyImageUrl);
                setLoading(false);
              }}
            />
          ) : (
...
export default memo(UnifiedStoryViewerV10);
