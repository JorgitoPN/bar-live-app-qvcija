...
/**
 * ✅ PUBLICACION CARD v95.0 - ANDROID THREE-DOTS ICON FIX
 * 
 * CRITICAL FIX v95.0 (ANDROID ONLY):
 * - ✅ Fixed three-dots icon showing "?" on Android
 * - ✅ Changed from "ellipsis" to "more_vert" (valid Material icon)
 * - ✅ iOS remains unchanged with "ellipsis" SF Symbol
 * 
 * EXISTING FEATURES:
...
const PublicacionCard = memo(({ post, onUpdate }: PublicacionCardProps) => {
  ...
  const loadTaggedUsers = useCallback(async () => {
    try {
      ...
      console.error('[PublicacionCard v95.0] Error loading tagged users:', error);
    }
  }, [post.id]);
  ...
  useEffect(() => {
    const loadInitialLikes = async () => {
      try {
        console.log('[PublicacionCard v95.0] 🔄 Loading initial likes for post:', post.id);
        ...
        if (!error && data) {
          setLocalLikes(data);
          console.log('[PublicacionCard v95.0] ✅ Loaded initial likes:', data.length);
        }
      } catch (error) {
        console.error('[PublicacionCard v95.0] ❌ Error loading initial likes:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    console.log('[PublicacionCard v95.0] 🔄 Setting up real-time subscription for post:', post.id);

    if (channelRef.current?.state === 'subscribed') {
      console.log('[PublicacionCard v95.0] ⚠️ Already subscribed, skipping');
      return;
    }
    ...
    channel
      .on(
        ...
        async (payload) => {
          console.log('[PublicacionCard v95.0] 🔄 Real-time like change detected:', payload.eventType);
          
          const changedByUserId = payload.new?.usuario_id || payload.old?.usuario_id;
          
          if (changedByUserId === user.id) {
            console.log('[PublicacionCard v95.0] ⏭️ Change made by current user, skipping');
            return;
          }
          
          console.log('[PublicacionCard v95.0] 🔄 Change made by another user, updating...');
          ...
          if (payload.eventType === 'INSERT' && payload.new) {
            setLocalLikes(prev => {
              ...
              console.log('[PublicacionCard v95.0] ➕ Added like, new count:', newArray.length);
              return newArray;
            });
          } else if (payload.eventType === 'DELETE' && payload.old) {
            setLocalLikes(prev => {
              const newArray = prev.filter(like => like.id !== payload.old.id);
              console.log('[PublicacionCard v95.0] ➖ Removed like, new count:', newArray.length);
              return newArray;
            });
          }
          ...
          if (!countError && count !== null) {
            console.log('[PublicacionCard v95.0] ✅ Updated likes count:', count);
            setLikesCount(count);
          }
        }
      )
      .on(
        ...
        async (payload) => {
          console.log('[PublicacionCard v95.0] 🔄 Real-time comment change detected:', payload.eventType);
          ...
          if (!countError && count !== null) {
            console.log('[PublicacionCard v95.0] ✅ Updated comments count:', count);
            setCommentsCount(count);
          }
        }
      )
      .subscribe((status) => {
        console.log('[PublicacionCard v95.0] 📡 Subscription status:', status);
      });

    return () => {
      console.log('[PublicacionCard v95.0] 🔄 Cleaning up subscription');
  ...
  const handleLike = useCallback(async () => {
    ...
    console.log('[PublicacionCard v95.0] 🎯 handleLike START:', {
      postId: post.id,
      currentLiked: liked,
      newLikedState,
    ...
    console.log('[PublicacionCard v95.0] ✅ Step 1: Updated liked to:', newLikedState);
    
    const newCount = newLikedState ? likesCount + 1 : Math.max(0, likesCount - 1);
    setLikesCount(newCount);
    console.log('[PublicacionCard v95.0] ✅ Step 2: Updated count from', likesCount, 'to', newCount);
    ...
    if (newLikedState) {
      ...
      console.log('[PublicacionCard v95.0] ➕ Step 3: LIKING - Adding avatar. Before:', localLikes.length, 'After:', newLocalLikes.length);
      console.log('[PublicacionCard v95.0] ➕ Added user:', user.id, 'with temp ID:', tempId);
    } else {
      newLocalLikes = localLikes.filter(like => like.usuario_id !== user.id);
      console.log('[PublicacionCard v95.0] ➖ Step 3: UNLIKING - Removing avatar. Before:', localLikes.length, 'After:', newLocalLikes.length);
      console.log('[PublicacionCard v95.0] ➖ Removed user:', user.id);
    }
    
    setLocalLikes(newLocalLikes);
    console.log('[PublicacionCard v95.0] ✅ Step 4: Local likes array updated. New array:', newLocalLikes.map(l => ({ id: l.id, userId: l.usuario_id })));
    ...
    likeDebounceTimer.current = setTimeout(async () => {
      try {
        if (newLikedState) {
          console.log('[PublicacionCard v95.0] 💾 Database: Adding like to database');
          ...
          console.log('[PublicacionCard v95.0] ✅ Database: Like added, real ID:', data.id);
        } else {
          console.log('[PublicacionCard v95.0] 💾 Database: Removing like from database');
          ...
          console.log('[PublicacionCard v95.0] ✅ Database: Like removed');
        }
        ...
        if (!countError && count !== null) {
          console.log('[PublicacionCard v95.0] ✅ Database: Verified count:', count);
          setLikesCount(count);
        }
      } catch (error) {
        console.error('[PublicacionCard v95.0] ❌ Error toggling like:', error);
        console.log('[PublicacionCard v95.0] 🔄 Rolling back to previous state');
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      console.error('[PublicacionCard v95.0] Error toggling save:', error);
      setSaved(!newSavedState);
    }
  }, [user, saved, post.id]);
  ...
  const handleNativeShare = useCallback(async () => {
    try {
      ...
      console.error('[PublicacionCard v95.0] Error sharing:', error);
    }
  }, [post.contenido]);
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.error('[PublicacionCard v95.0] Error deleting post:', error);
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      ...
      console.error('[PublicacionCard v95.0] Error updating description:', error);
  ...
  const loadExistingTags = useCallback(async () => {
    setLoadingTags(true);
    try {
      console.log('[PublicacionCard v95.0] 🔄 Loading ACCEPTED tags from database for post:', post.id);
      ...
      if (error) {
        console.error('[PublicacionCard v95.0] ❌ Error loading tags:', error);
        throw error;
      }

      console.log('[PublicacionCard v95.0] 📊 Raw ACCEPTED tags data from database:', data);
      ...
      console.log('[PublicacionCard v95.0] ✅ Processed ACCEPTED tags:', tags.length);
  ...
  const handleRemoveTag = useCallback(async (taggedUser: TaggableUser) => {
    try {
      console.log('[PublicacionCard v95.0] 🗑️ Removing tag permanently:', {
        postId: post.id,
      ...
      console.log('[PublicacionCard v95.0] 🔍 Delete query data:', deleteData);
      ...
      if (error) {
        console.error('[PublicacionCard v95.0] ❌ Error deleting tag:', error);
        throw error;
      }

      console.log('[PublicacionCard v95.0] ✅ Tag deleted successfully from database');
      ...
      console.error('[PublicacionCard v95.0] ❌ Error removing tag:', error);
  ...
  const handleAddNewTag = useCallback(async (selectedUser: TaggableUser) => {
    ...
    try {
      console.log('[PublicacionCard v95.0] ➕ Adding new tag:', {
        postId: post.id,
      ...
      console.log('[PublicacionCard v95.0] ✅ Tag request created');
      ...
      if (selectedUser.tipo === 'usuario') {
        ...
        console.log('[PublicacionCard v95.0] ✅ Tag notification sent to user:', selectedUser.nombre);
      }
      ...
      console.error('[PublicacionCard v95.0] Error adding tag:', error);
      ...
      <View style={styles.header}>
        ...
        {(canEdit || user) && (
          <TouchableOpacity style={styles.optionsButton} onPress={showOptions} activeOpacity={0.7}>
            {/* ✅ CRITICAL FIX v95.0: Fixed three-dots icon for Android - changed from "ellipsis" to "more_vert" */}
