...
export default function PostViewerModal({
  ...
  const refreshCurrentPost = useCallback(async () => {
    ...
    try {
      ...
      if (postError || !postData) {
        console.error('[PostViewerModal] Error refreshing post:', postError);
        return;
      }
      ...
      console.error('[PostViewerModal] Error refreshing post:', error);
    }
  }, [currentPostId]);
  ...
  const loadPosts = useCallback(async () => {
    try {
      ...
      if (!allPostIds || !Array.isArray(allPostIds) || allPostIds.length === 0) {
        console.error('[PostViewerModal] Invalid allPostIds in loadPosts:', allPostIds);
        setPosts([]);
        setLoading(false);
        return;
      }
  ...
  const attemptScrollToInitial = useCallback(() => {
    ...
    try {
      ...
      console.error('[PostViewerModal] ScrollToIndex error:', error);
  ...
  const getItemLayout = useCallback((data: any, index: number) => {
    const baseHeight = SCREEN_HEIGHT * 0.9;
    return {
      length: baseHeight,
  ...
  const handleScrollToIndexFailed = useCallback((info: any) => {
    if (flatListRef.current && !hasScrolledToInitial.current) {
      ...
      try {
        ...
        console.error('[PostViewerModal] Fallback scroll failed:', error);
      }
    }
  }, []);
