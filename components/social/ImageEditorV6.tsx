...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * 🚨 ANDROID-ONLY FIXES v157.0 - IMAGE EDITOR COMPLETELY DISABLED FOR ANDROID
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * CAMBIOS IMPLEMENTADOS v157.0 (ANDROID EXCLUSIVO):
 ...
 * - ✅ Usa requestAnimationFrame para asegurar que el callback se ejecuta correctamente
 * - ✅ VERIFICADO: Cambios aplicados en v157.0
 * 
 * ARCHIVOS MODIFICADOS:
 * - components/social/ImageEditorV6.tsx (este archivo)
 * - app/crear/publicacion.android.tsx (nuevo - sin editor)
 * - components/momento/MomentoUpload.android.tsx (nuevo - sin editor)
 * 
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
/**
 * ✅ IMAGE EDITOR v157.0 - ANDROID EDITOR COMPLETELY DISABLED
 * 
 ...
 * - ✅ Uses requestAnimationFrame to ensure callbacks execute properly
 * 
...
export default function ImageEditorV6({
  ...
  // ✅ CRITICAL FIX v157.0: ALL HOOKS DECLARED AT THE TOP (React rules compliant)
  ...
  // ✅ CRITICAL FIX v157.0: ANDROID ONLY - Skip editor completely, save image directly
  useEffect(() => {
    if (visible && imageUri && Platform.OS === 'android') {
      console.log('[ImageEditorV6 v157.0] 🤖 Android detected - skipping editor, saving directly');
      // Save image immediately without showing editor
      // Use requestAnimationFrame to ensure proper callback execution
  ...
  useEffect(() => {
    ...
    if (visible && imageUri) {
      console.log('[ImageEditorV6 v157.0] 🖼️ Loading image (iOS only):', imageUri);
      setImageLoaded(false);
      
      Image.getSize(
        imageUri,
        (width, height) => {
          console.log('[ImageEditorV6 v157.0] ✅ Image loaded:', { width, height });
        ...
        (error) => {
          console.error('[ImageEditorV6 v157.0] ❌ Error loading image:', error);
  ...
  // ✅ CRITICAL FIX v157.0: Animated style MUST be declared unconditionally
  ...
  const resetTransform = () => {
    console.log('[ImageEditorV6 v157.0] 🔄 Resetting all transformations');
  ...
  const handleRotateLeft = () => {
    console.log('[ImageEditorV6 v157.0] ↶ Rotating left');
    setRotation((rotation - 90) % 360);
  };

  const handleRotateRight = () => {
    console.log('[ImageEditorV6 v157.0] ↷ Rotating right');
    setRotation((rotation + 90) % 360);
  };

  const handleFlipHorizontal = () => {
    console.log('[ImageEditorV6 v157.0] ↔️ Flipping horizontal');
    setFlipHorizontal(!flipHorizontal);
  };

  const handleFlipVertical = () => {
    console.log('[ImageEditorV6 v157.0] ↕️ Flipping vertical');
  ...
  const applyEdits = async () => {
    ...
    setProcessing(true);
    try {
      console.log('[ImageEditorV6 v157.0] 🎨 Applying edits...');
      
      const actions: any[] = [];

      // Apply rotation
      if (rotation !== 0) {
        console.log('[ImageEditorV6 v157.0] ↻ Applying rotation:', rotation);
        actions.push({ rotate: rotation });
      }

      // Apply flips
      if (flipHorizontal) {
        console.log('[ImageEditorV6 v157.0] ↔️ Applying horizontal flip');
        actions.push({ flip: ImageManipulator.FlipType.Horizontal });
      }
      if (flipVertical) {
        console.log('[ImageEditorV6 v157.0] ↕️ Applying vertical flip');
      ...
      if (imageDimensions.width !== imageDimensions.height) {
        ...
        console.log('[ImageEditorV6 v157.0] ✂️ Cropping to square:', { size, originX, originY });
      ...
      console.log('[ImageEditorV6 v157.0] 🔧 Total actions to apply:', actions.length);
      ...
      console.log('[ImageEditorV6 v157.0] ✅ Edits applied successfully:', result.uri);
      
      onSave(result.uri);
      resetTransform();
    } catch (error) {
      console.error('[ImageEditorV6 v157.0] ❌ Error applying edits:', error);
  ...
  // ✅ CRITICAL FIX v157.0: ANDROID ONLY - Return null to prevent editor from showing
  // This check MUST come AFTER all hooks are declared
  if (Platform.OS === 'android') {
    console.log('[ImageEditorV6 v157.0] 🤖 Android detected - returning null (no editor)');
    return null;
  }
