...
/**
 * ✅ MOMENTO VIEWER v169.0 - PROFESSIONAL GESTURE CONTROLS + KEYBOARD FIXES OPTIMIZED
 * 
 * GESTURE SYSTEM:
 * 1. TAP (Short Press):
 ...
 * 
 * ✅ FIXES v169.0 (OPTIMIZED):
 * - PROBLEMA 1 RESUELTO: Android keyboard now properly lifts input field ~10% (optimized from 90%)
 * - PROBLEMA 2 RESUELTO: Send button sends message immediately WITHOUT closing keyboard first
 * - Message sends while keyboard is still open for fluid conversation
 * - paddingBottom set to 10px for ~10% lift (optimized for better UX)
 */
...
export default function MomentoViewer({
  ...
  const markAsViewed = useCallback(async (momentoId: string) => {
    ...
    try {
      ...
      console.error('[MomentoViewer v169.0] Error marking as viewed:', error);
    }
  }, [user]);
  ...
  const loadMomentos = useCallback(async () => {
    ...
    try {
      setLoading(true);
      console.log('[MomentoViewer v169.0] Loading momentos for:', { authorId, authorType });
      ...
      console.log('[MomentoViewer v169.0] Starting at index:', startIndex, 'of', momentosWithStatus.length);
      ...
      console.log('[MomentoViewer v169.0] ✅ Loaded momentos:', momentosWithStatus.length);
    } catch (error) {
      console.error('[MomentoViewer v169.0] Error loading momentos:', error);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      console.error('[MomentoViewer v169.0] Error toggling like:', error);
    }
  };
  ...
  const captureMomentoScreenshot = async (): Promise<string | null> => {
    ...
    try {
      console.log('[MomentoViewer v169.0] 📸 Capturing momento screenshot...');
      ...
      console.log('[MomentoViewer v169.0] ✅ Screenshot captured:', uri);
      return uri;
    } catch (error) {
      console.error('[MomentoViewer v169.0] Error capturing screenshot:', error);
      return null;
    }
  };

  const handleOpenMessageInput = () => {
    console.log('[MomentoViewer v169.0] 📝 Opening message input, pausing momento');
    setPaused(true);
  ...
  const handleCloseMessageInput = () => {
    console.log('[MomentoViewer v169.0] ❌ Closing message input, resuming momento');
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.log('[MomentoViewer v169.0] 📸 Starting momento message flow with text...');
      ...
      if (screenshotUri) {
        ...
        if (!uploadError && uploadData) {
          ...
          console.log('[MomentoViewer v169.0] ✅ Screenshot uploaded:', screenshotUrl);
        }
      }
      ...
      if (chatId) {
        ...
        console.log('[MomentoViewer v169.0] ✅ Momento message sent with screenshot and text');
      ...
      console.error('[MomentoViewer v169.0] Error creating chat:', error);
  ...
  const handleShowStats = async () => {
    ...
    console.log('[MomentoViewer v169.0] 📊 Opening stats, pausing momento');
    setPaused(true);
    ...
    try {
      ...
      console.error('[MomentoViewer v169.0] Error loading stats:', error);
    }
  };
  ...
  const submitMomentoReport = async (momentoId: string, reason: string) => {
    try {
      ...
      console.error('[MomentoViewer v169.0] Error reporting momento:', error);
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.error('[MomentoViewer v169.0] Error deleting momento:', error);
  ...
  const handleNext = useCallback(() => {
    console.log('[MomentoViewer v169.0] ➡️ Next momento');
    ...
    if (currentIndex < momentos.length - 1) {
      ...
      console.log('[MomentoViewer v169.0] End of momentos, closing');
      handleClose();
    }
  }, [currentIndex, momentos, progressAnims, markAsViewed]);

  const handlePrevious = useCallback(() => {
    console.log('[MomentoViewer v169.0] ⬅️ Previous momento');
  ...
  const handleClose = useCallback(() => {
    console.log('[MomentoViewer v169.0] ❌ Closing viewer');
  ...
  const handleTap = useCallback((locationX: number) => {
    if (isLongPressRef.current) {
      console.log('[MomentoViewer v169.0] Ignoring tap - was long press');
      return;
    }
    ...
    if (isRightSide) {
      console.log('[MomentoViewer v169.0] 👉 Tap right - Next');
      handleNext();
    } else {
      console.log('[MomentoViewer v169.0] 👈 Tap left - Previous');
      handlePrevious();
    }
  ...
  const handleLongPressStart = useCallback(() => {
    console.log('[MomentoViewer v169.0] 🛑 Long press detected - PAUSE');
  ...
  const handleLongPressEnd = useCallback(() => {
    console.log('[MomentoViewer v169.0] ▶️ Long press released - RESUME');
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (gestureState.dy > VERTICAL_SWIPE_THRESHOLD) {
          console.log('[MomentoViewer v169.0] ⬇️ Swipe down - Close');
        ...
        if (Math.abs(gestureState.dx) > SWIPE_THRESHOLD) {
          if (gestureState.dx < 0) {
            console.log('[MomentoViewer v169.0] ⬅️ Swipe left - Next user');
            handleNext();
          } else {
            console.log('[MomentoViewer v169.0] ➡️ Swipe right - Previous user');
            handlePrevious();
          }
          return;
        }
  ...
  useEffect(() => {
    if (visible && authorId) {
      console.log('[MomentoViewer v169.0] Opening viewer for:', { authorId, authorType });
      loadMomentos();
  ...
  useEffect(() => {
    if (!paused && !showMessageInput && !showStats && momentos.length > 0 && !loading && visible) {
      console.log('[MomentoViewer v169.0] ▶️ Starting/resuming progress for momento', currentIndex);
      ...
      console.log('[MomentoViewer v169.0] Progress:', currentProgress.toFixed(3), '- Remaining:', remainingDuration.toFixed(0), 'ms');

      progressStartTimeRef.current = Date.now();

      progressTimerRef.current = setTimeout(() => {
        console.log('[MomentoViewer v169.0] ⏱️ Timer completed - moving to next');
        handleNext();
      }, remainingDuration);
  ...
  if (!currentMomento) {
    console.error('[MomentoViewer v169.0] Current momento is undefined');
    handleClose();
    return null;
  }
          ...
          >
            <View style={[
              ...
              Platform.OS === 'android' && {
                ...
                // Lift input field ~10% above keyboard (optimized for better UX)
                paddingBottom: 10,
              }
            ]}>
              <TouchableOpacity 
              ...
              <View style={styles.messageInputBox}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    // Direct call to handleSendMessage - no Keyboard.dismiss(), no setTimeout
                    // Message sends instantly while keyboard remains open for continuous typing
                    console.log('[MomentoViewer v169.0] 📤 Send button pressed - sending immediately');
                    handleSendMessage();
                  }}
