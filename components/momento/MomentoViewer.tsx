...
/**
 * ✅ MOMENTO VIEWER v170.0 - PROFESSIONAL GESTURE CONTROLS + KEYBOARD FIXES FINAL
 * 
 * GESTURE SYSTEM:
 * 1. TAP (Short Press):
 ...
 * - paddingBottom: 90px for Android (90% lift as requested)
 */
...
export default function MomentoViewer({
  ...
  const markAsViewed = useCallback(async (momentoId: string) => {
    ...
    try {
      ...
      console.error('[MomentoViewer v170.0] Error marking as viewed:', error);
    }
  }, [user]);
  ...
  const loadMomentos = useCallback(async () => {
    ...
    try {
      setLoading(true);
      console.log('[MomentoViewer v170.0] Loading momentos for:', { authorId, authorType });
      ...
      console.log('[MomentoViewer v170.0] Starting at index:', startIndex, 'of', momentosWithStatus.length);
      ...
      console.log('[MomentoViewer v170.0] ✅ Loaded momentos:', momentosWithStatus.length);
    } catch (error) {
      console.error('[MomentoViewer v170.0] Error loading momentos:', error);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      console.error('[MomentoViewer v170.0] Error toggling like:', error);
    }
  };
  ...
  const captureMomentoScreenshot = async (): Promise<string | null> => {
    ...
    try {
      console.log('[MomentoViewer v170.0] 📸 Capturing momento screenshot...');
      ...
      console.log('[MomentoViewer v170.0] ✅ Screenshot captured:', uri);
      return uri;
    } catch (error) {
      console.error('[MomentoViewer v170.0] Error capturing screenshot:', error);
      return null;
    }
  };

  const handleOpenMessageInput = () => {
    console.log('[MomentoViewer v170.0] 📝 Opening message input, pausing momento');
    setPaused(true);
  ...
  const handleCloseMessageInput = () => {
    console.log('[MomentoViewer v170.0] ❌ Closing message input, resuming momento');
  ...
  const handleSendMessage = async () => {
    console.log('[MomentoViewer v170.0] 📤 handleSendMessage called - keyboard should stay open');
    ...
    try {
      ...
      console.log('[MomentoViewer v170.0] 📸 Starting momento message flow with text...');
      ...
      if (screenshotUri) {
        ...
        if (!uploadError && uploadData) {
          ...
          console.log('[MomentoViewer v170.0] ✅ Screenshot uploaded:', screenshotUrl);
        }
      }
      ...
      if (chatId) {
        ...
        console.log('[MomentoViewer v170.0] ✅ Momento message sent with screenshot and text');
        console.log('[MomentoViewer v170.0] ✅ Message sent successfully - keyboard was NOT dismissed');
      ...
      console.error('[MomentoViewer v170.0] Error creating chat:', error);
      Alert.alert('Error', 'No se pudo crear la conversación');
    } finally {
      setSendingMessage(false);
      console.log('[MomentoViewer v170.0] 📤 Send message flow completed');
    }
  };
  ...
  const handleShowStats = async () => {
    ...
    console.log('[MomentoViewer v170.0] 📊 Opening stats, pausing momento');
    setPaused(true);
    ...
    try {
      ...
      console.error('[MomentoViewer v170.0] Error loading stats:', error);
    }
  };
  ...
  const submitMomentoReport = async (momentoId: string, reason: string) => {
    try {
      ...
      console.error('[MomentoViewer v170.0] Error reporting momento:', error);
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.error('[MomentoViewer v170.0] Error deleting momento:', error);
  ...
  const handleNext = useCallback(() => {
    console.log('[MomentoViewer v170.0] ➡️ Next momento');
    ...
    if (currentIndex < momentos.length - 1) {
      ...
      console.log('[MomentoViewer v170.0] End of momentos, closing');
      handleClose();
    }
  }, [currentIndex, momentos, progressAnims, markAsViewed]);

  const handlePrevious = useCallback(() => {
    console.log('[MomentoViewer v170.0] ⬅️ Previous momento');
  ...
  const handleClose = useCallback(() => {
    console.log('[MomentoViewer v170.0] ❌ Closing viewer');
  ...
  const handleTap = useCallback((locationX: number) => {
    if (isLongPressRef.current) {
      console.log('[MomentoViewer v170.0] Ignoring tap - was long press');
      return;
    }
    ...
    if (isRightSide) {
      console.log('[MomentoViewer v170.0] 👉 Tap right - Next');
      handleNext();
    } else {
      console.log('[MomentoViewer v170.0] 👈 Tap left - Previous');
      handlePrevious();
    }
  ...
  const handleLongPressStart = useCallback(() => {
    console.log('[MomentoViewer v170.0] 🛑 Long press detected - PAUSE');
  ...
  const handleLongPressEnd = useCallback(() => {
    console.log('[MomentoViewer v170.0] ▶️ Long press released - RESUME');
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (gestureState.dy > VERTICAL_SWIPE_THRESHOLD) {
          console.log('[MomentoViewer v170.0] ⬇️ Swipe down - Close');
        ...
        if (Math.abs(gestureState.dx) > SWIPE_THRESHOLD) {
          if (gestureState.dx < 0) {
            console.log('[MomentoViewer v170.0] ⬅️ Swipe left - Next user');
            handleNext();
          } else {
            console.log('[MomentoViewer v170.0] ➡️ Swipe right - Previous user');
            handlePrevious();
          }
          return;
        }
  ...
  useEffect(() => {
    if (visible && authorId) {
      console.log('[MomentoViewer v170.0] Opening viewer for:', { authorId, authorType });
      loadMomentos();
  ...
  useEffect(() => {
    if (!paused && !showMessageInput && !showStats && momentos.length > 0 && !loading && visible) {
      console.log('[MomentoViewer v170.0] ▶️ Starting/resuming progress for momento', currentIndex);
      ...
      console.log('[MomentoViewer v170.0] Progress:', currentProgress.toFixed(3), '- Remaining:', remainingDuration.toFixed(0), 'ms');

      progressStartTimeRef.current = Date.now();

      progressTimerRef.current = setTimeout(() => {
        console.log('[MomentoViewer v170.0] ⏱️ Timer completed - moving to next');
        handleNext();
      }, remainingDuration);
  ...
  if (!currentMomento) {
    console.error('[MomentoViewer v170.0] Current momento is undefined');
    handleClose();
    return null;
  }
        ...
        {showMessageInput && (
          ...
          >
            <View style={[
              ...
              Platform.OS === 'android' && {
                // ✅ FIX PROBLEMA 1 (DEFINITIVO): Android keyboard adjustment
                // Lift input field ~90% above keyboard as requested
                paddingBottom: 90,
              }
            ]}>
              <TouchableOpacity 
              ...
              <View style={styles.messageInputBox}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    // ✅ FIX PROBLEMA 2 (DEFINITIVO): Send message immediately WITHOUT closing keyboard
