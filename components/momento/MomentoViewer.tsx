...
/**
 * ✅ MOMENTO VIEWER v156.0 - ANDROID FULLSCREEN PERFECTED
 * 
...
export default function MomentoViewer({
  ...
  const markAsViewed = useCallback(async (momentoId: string) => {
    ...
    try {
      ...
      console.error('[MomentoViewer v156.0] Error marking as viewed:', error);
    }
  }, [user]);
  ...
  const loadMomentos = useCallback(async () => {
    ...
    try {
      setLoading(true);
      console.log('[MomentoViewer v156.0] Loading momentos for:', { authorId, authorType });
      ...
      console.log('[MomentoViewer v156.0] Starting at index:', startIndex, 'of', momentosWithStatus.length);
      ...
      console.log('[MomentoViewer v156.0] ✅ Loaded momentos:', momentosWithStatus.length);
    } catch (error) {
      console.error('[MomentoViewer v156.0] Error loading momentos:', error);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      console.error('[MomentoViewer v156.0] Error toggling like:', error);
    }
  };
  ...
  const captureMomentoScreenshot = async (): Promise<string | null> => {
    ...
    try {
      console.log('[MomentoViewer v156.0] 📸 Capturing momento screenshot...');
      ...
      console.log('[MomentoViewer v156.0] ✅ Screenshot captured:', uri);
      return uri;
    } catch (error) {
      console.error('[MomentoViewer v156.0] Error capturing screenshot:', error);
      return null;
    }
  };

  const handleOpenMessageInput = () => {
    console.log('[MomentoViewer v156.0] 📝 Opening message input, pausing momento');
    setPaused(true);
  ...
  const handleCloseMessageInput = () => {
    console.log('[MomentoViewer v156.0] ❌ Closing message input, resuming momento');
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.log('[MomentoViewer v156.0] 📸 Starting momento message flow with text...');
      ...
      if (screenshotUri) {
        ...
        if (!uploadError && uploadData) {
          ...
          console.log('[MomentoViewer v156.0] ✅ Screenshot uploaded:', screenshotUrl);
        }
      }
      ...
      if (chatId) {
        ...
        console.log('[MomentoViewer v156.0] ✅ Momento message sent with screenshot and text');
      ...
      console.error('[MomentoViewer v156.0] Error creating chat:', error);
  ...
  const handleShowStats = async () => {
    ...
    console.log('[MomentoViewer v156.0] 📊 Opening stats, pausing momento');
    setPaused(true);
    ...
    try {
      ...
      console.error('[MomentoViewer v156.0] Error loading stats:', error);
    }
  };
  ...
  const submitMomentoReport = async (momentoId: string, reason: string) => {
    try {
      ...
      console.error('[MomentoViewer v156.0] Error reporting momento:', error);
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.error('[MomentoViewer v156.0] Error deleting momento:', error);
  ...
  const handlePressIn = () => {
    console.log('[MomentoViewer v156.0] 🛑 PAUSE - Touch detected');
  ...
  const handlePressOut = () => {
    console.log('[MomentoViewer v156.0] ▶️ RESUME - Touch released');
  ...
  useEffect(() => {
    if (visible && authorId) {
      console.log('[MomentoViewer v156.0] Opening viewer for:', { authorId, authorType });
      loadMomentos();
  ...
  useEffect(() => {
    if (!paused && !showMessageInput && !showStats && momentos.length > 0 && !loading && visible) {
      console.log('[MomentoViewer v156.0] ▶️ Starting/resuming progress for momento', currentIndex);
      ...
      console.log('[MomentoViewer v156.0] Progress:', currentProgress.toFixed(3), '- Remaining:', remainingDuration.toFixed(0), 'ms');

      progressStartTimeRef.current = Date.now();

      progressTimerRef.current = setTimeout(() => {
        console.log('[MomentoViewer v156.0] ⏱️ Timer completed - moving to next');
        handleNext();
      }, remainingDuration);
  ...
  if (!currentMomento) {
    console.error('[MomentoViewer v156.0] Current momento is undefined');
    handleClose();
    return null;
  }
