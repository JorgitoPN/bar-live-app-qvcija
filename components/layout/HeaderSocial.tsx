...
/**
 * ✅ HEADER SOCIAL v63.0 - STANDARDIZED DIMENSIONS
 * 
 * CRITICAL FIXES v63.0:
 * - ✅ Uses HEADER_DIMENSIONS from commonStyles for consistency
 * - ✅ All text and icon sizes reduced on Android (40% smaller)
...
export default function HeaderSocial({
  ...
  const loadUnreadCounts = useCallback(async () => {
    if (!user) {
      console.log('[HeaderSocial v63.0] ℹ️ No user, resetting counts to 0');
    ...
    try {
      console.log('[HeaderSocial v63.0] 🔄 Loading unread counts from database...');
      ...
      if (notifError) {
        console.error('[HeaderSocial v63.0] ❌ Error loading notifications count:', notifError);
      } else {
        setUnreadNotifications(notifCount || 0);
        console.log('[HeaderSocial v63.0] ✅ Unread notifications:', notifCount || 0);
      }
      ...
      if (chatsError) {
        console.error('[HeaderSocial v63.0] ❌ Error loading chats:', chatsError);
      } else if (chatsData) {
        let totalUnread = 0;
        ...
        console.log('[HeaderSocial v63.0] ✅ Unread messages:', totalUnread);
      }
    } catch (error) {
      console.error('[HeaderSocial v63.0] ❌ Error loading unread counts:', error);
    }
  }, [user]);
  ...
  useEffect(() => {
    ...
    console.log('[HeaderSocial v63.0] 🔄 Setting up real-time subscriptions for user:', user.id);
    ...
    const subscription = supabase
      ...
      .on(
        ...
        () => {
          console.log('[HeaderSocial v63.0] 🔔 Notification update detected, reloading count...');
          loadUnreadCounts();
        }
      )
      .on(
        ...
        (payload) => {
          console.log('[HeaderSocial v63.0] 💬 Message UPDATE detected:', payload.new);
          if (payload.new && (payload.new as any).leido === true) {
            console.log('[HeaderSocial v63.0] ✅ Message marked as read, reloading counts...');
            loadUnreadCounts();
          }
        }
      )
      .on(
        ...
        () => {
          console.log('[HeaderSocial v63.0] 💬 New message INSERT detected, reloading count...');
          loadUnreadCounts();
        }
      )
      .subscribe((status) => {
        console.log('[HeaderSocial v63.0] 📡 Subscription status:', status);
      });

    return () => {
      console.log('[HeaderSocial v63.0] 🔄 Cleaning up subscriptions');
  ...
  const performSearch = useCallback(async (query: string) => {
    ...
    try {
      console.log('[HeaderSocial v63.0] 🔍 Searching for:', query);
      ...
      if (usersError) {
        console.error('[HeaderSocial v63.0] ❌ Error searching users:', usersError);
      }

      console.log('[HeaderSocial v63.0] 🔍 Searching locals with query:', cleanQuery);
      ...
      if (localsError) {
        console.error('[HeaderSocial v63.0] ❌ Error searching locals:', localsError);
      }

      console.log('[HeaderSocial v63.0] ✅ Found locals:', localsData?.length || 0);
      ...
      console.log('[HeaderSocial v63.0] 📊 Search results:', results.length);
      setSearchResults(results);
    } catch (error) {
      console.error('[HeaderSocial v63.0] ❌ Error searching:', error);
    } finally {
  ...
  const handleSearchResultPress = (result: SearchResult) => {
    try {
      console.log('[HeaderSocial v63.0] 🔗 Navigating to:', result.type, result.id);
      ...
      if (result.type === 'user') {
        ...
        console.log('[HeaderSocial v63.0] 🏢 Navigating to local profile:', result.id);
        router.push(`/perfil/local?localId=${result.id}`);
      }
    } catch (error) {
      console.error('[HeaderSocial v63.0] ❌ Error navigating to profile:', error);
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerButtons}>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <IconSymbol ios_icon_name="message.fill" android_material_icon_name="message" size={Platform.OS === 'ios' ? 22 : 15} color={colors.headerText} />
              {unreadMessages > 0 && (
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <IconSymbol ios_icon_name="bell.fill" android_material_icon_name="notifications" size={Platform.OS === 'ios' ? 22 : 15} color={colors.headerText} />
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <IconSymbol ios_icon_name="magnifyingglass" android_material_icon_name="search" size={Platform.OS === 'ios' ? 22 : 15} color={colors.headerText} />
            </TouchableOpacity>
            ...
            {(onCreatePress || onCreatePost) && (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <IconSymbol ios_icon_name="plus" android_material_icon_name="add" size={Platform.OS === 'ios' ? 22 : 15} color={colors.headerText} />
              </TouchableOpacity>
            )}
          </View>
        </View>
        ...
        <View style={styles.searchContainer}>
          ...
          >
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <IconSymbol ios_icon_name="chevron.left" android_material_icon_name="arrow_back" size={Platform.OS === 'ios' ? 24 : 17} color={colors.headerText} />
            </TouchableOpacity>
            
            <View style={styles.searchInputContainer}>
              <IconSymbol ios_icon_name="magnifyingglass" android_material_icon_name="search" size={Platform.OS === 'ios' ? 20 : 13} color={colors.headerText} />
              <TextInput
              ...
              {searchQuery.length > 0 && (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <IconSymbol ios_icon_name="xmark.circle.fill" android_material_icon_name="cancel" size={Platform.OS === 'ios' ? 20 : 13} color={colors.headerText} />
                </TouchableOpacity>
              )}
            </View>
          </LinearGradient>
          ...
          <View style={styles.searchContent}>
            {searchLoading ? (
              ...
              <View style={styles.searchEmptyState}>
                <IconSymbol ios_icon_name="magnifyingglass" android_material_icon_name="search" size={Platform.OS === 'ios' ? 64 : 45} color={colors.textSecondary} />
...
const styles = StyleSheet.create({
  ...
  // ✅ ANDROID FIX v63.0: Significantly reduced font size on Android (40% smaller)
  headerTitle: {
  ...
  badge: {
    ...
    borderRadius: 12,
    minWidth: Platform.OS === 'ios' ? 22 : 18,
  ...
  badgeText: {
    ...
    fontWeight: '700',
    textAlign: 'center',
    lineHeight: Platform.OS === 'android' ? 10 : 10,
  ...
  searchResultAvatar: {
    width: Platform.OS === 'ios' ? 56 : 42,
    height: Platform.OS === 'ios' ? 56 : 42,
    borderRadius: Platform.OS === 'ios' ? 28 : 21,
    marginRight: 16,
  },
