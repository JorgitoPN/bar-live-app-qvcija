...
-- ✅ MIGRATION: Add advanced filters support to get_sorted_locales_by_proximity RPC
-- This migration adds support for servicios, ambiente, clientela, comunidad, provincia, and max_distance filters
...
BEGIN
  ...
  WITH locales_with_distance AS (
    ...
    WHERE l.activo = TRUE
      ...
      -- ✅ NEW: Servicios filter (AND logic - local must have ALL selected services)
      AND (
      ...
      -- ✅ NEW: Ambiente filter (OR logic - local must have AT LEAST ONE selected ambiente)
      AND (
      ...
      -- ✅ NEW: Clientela filter (OR logic - local must have AT LEAST ONE selected clientela)
      AND (
  ...
  locales_with_status AS (
    SELECT
      ...
      CASE
        ...
        ELSE (
          SELECT bool_or(
            v_current_time >= SPLIT_PART(horario::TEXT, '–', 1)::TIME
            AND v_current_time <= SPLIT_PART(horario::TEXT, '–', 2)::TIME
          )
    ...
    -- ✅ NEW: Distance filter (only if user location is provided)
    WHERE (
  ...
  locales_with_tier AS (
    SELECT
      ...
      CASE
        ...
        WHEN lws.has_schedule_info = FALSE
        THEN 3
...
-- Add comment
COMMENT ON FUNCTION get_sorted_locales_by_proximity IS 'Returns locales sorted by 5-tier system with advanced filters support (servicios, ambiente, clientela, comunidad, provincia, max_distance)';
